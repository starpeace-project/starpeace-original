...
interface
  ...
  type
    TPeriodType = (perHour, perDay, perMonth, perYear);
  ...
  type
    ...
    IModelFactory =
      interface
        ...
        function  NewTycoonRole( RoleId, name : string ) : TTycoon;
    ...
    TInvention =
      class( TMetaInstance )
        public
          constructor Create( aNumId : TInventionNumId; aName, aKind, aResp : string );
        private
        ...
        published
          ...
          property Resp  : string          read fResp;
    ...
    TMetaFacilityOption  = (mfcInTown, mfcGenerateName);
    ...
    TTycoon =
      class( TMoneyDealer )
        public
          constructor Create( anId : TTycoonId );
          destructor  Destroy; override;
        private
          fId              : TTycoonId;
...
implementation
  ...
  constructor TInvention.Create( aNumId : TInventionNumId; aName, aKind, aResp : string );
    begin
      inherited Create( GetInventionId( aNumId, aKind ) );
      fNumId := aNumId;
      fName  := aName;
      fKind  := aKind;
      fResp  := aResp;
  ...
  constructor TMetaFacility.Create( anId, aName : string; aVisualClass : TVisualClassId; aFacilityClass : CFacility );
    ...
    begin
      ...
      fInTown        := true;
      fOptions       := [mfcInTown, mfcGenerateName];
            ...
            if fToBeDemolished > 0
              then
                if fToBeDemolished = 1
                  then
                    begin
                      Company.GenMoney( Cost, accIdx_Compensations );
  ...
  function TFacility.AccessLevelOf( Tycoon : TTycoon ) : TAccessLevel;
    begin
      if (Company <> nil) and ((Tycoon.MasterRole = Company.Owner.MasterRole) or not Company.Privated)
        then result := acsFull
        ...
        case kind of
          sttMain :
            begin
              ...
              if (Company <> nil) and Company.Privated and (Company.Owner <> nil)
    ...
    begin
      ...
      while (FluidData.Q < Capacity) and (i < ConnectionCount) and not (Block.Facility.CriticalTrouble) do
        begin
          ...
          if not CollectExtra and ExInf.Connected
  ...
  constructor TTown.Create( aName : string; aXPos, aYPos : integer; aCluster : TCluster; aModelFactory : IModelFactory; aMailServer : IMailServer );
    ...
    begin
      ...
      fOrdinanceSet := [];
      fMayor        := aModelFactory.NewTycoonRole( tidRole_Mayor, 'Mayor of ' + aName );
        ...
        for i := 0 to pred(count) do
          begin
            ...
            try
              ...
              fInventionSet := fInventionSet + [Invention.fNumId];
            except
            end;
          end;
      end;
  ...
  procedure TCompanyDirection.Update;
    begin
      if round(fDemand) > 0
        then fSupport := fStrength/fDemand 
        else fSupport := 1;
      fDemand   := 0;
      fStrength := 0;
    end;
  ...
  procedure TTycoon.StoreToBackup( Writer : IBackupWriter );
    ...
    begin
      ...
      Writer.WriteLooseObject( 'Tutorial', fTutorial );
    end;
