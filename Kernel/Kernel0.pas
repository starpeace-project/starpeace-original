...
implementation
  ...
  procedure TFacility.SetStopped( Value : boolean );
    begin
      ...
      if CheckOpAuthenticity and (Value or not (mfcStopDisabled in MetaFacility.Options))
        then
          begin
            Lock;
            try
  ...
  procedure TFacility.RDOStartUpgrade;
    var
      Block : TBlock;
    begin
      Logs.Log( tidLog_Survival, TimeToStr(Now) + ' Facility Start Upgrade..' );
      if CheckOpAuthenticity
        then
          try
            Block := CurrBlock;
  ...
  procedure TFacility.RDODowngrade;
    var
      Block : TBlock;
    begin
      Logs.Log( tidLog_Survival, TimeToStr(Now) + ' Facility Downgrade..' );
      if CheckOpAuthenticity
        then
          try
            Block := CurrBlock;
  ...
  procedure TTycoon.EndOfPeriod( PeriodType : TPeriodType; PeriodCount : integer );
    ...
    begin
      ...
      try
        ...
        if (PeriodType = perHour) and (fTutorial <> nil) and not IsRole and IsOnline and HasValidCompany and fTutorial.Active and fTutorial.TimeToSimulate(PeriodCount)
          then
            begin
