    ...
    TMetaOutputEvaluator =
      class
        ...
        public
          ...
          property PrdWthrEnv      : integer     read fPrdWthrEnv      write fPrdWthrEnv;
        private
    ...
    TOutputEvaluator =
      class
        ...
        public
          ...
          function  BiasOpenLimit(Block : TBlock) : TBiasRatio; virtual;
          ...
          procedure GetInventionsEffect(Block : TBlock; var QEffect : TInventionEfficiencyEffect; var KEffect : integer; DsgnK : TPercent);
          procedure RecalculateInventionsEffect(Block : TBlock);
          procedure GetCompanyInputsEffect(Block : TBlock; var QEffect : single; var KEffect : single);
          function  WeatherEffect(Block : TBlock) : single;
        ...
        public
          function  FormatOutput(Block : TBlock; kind : TStatusKind; ShowName : boolean; ToTycoon : TTycoon ) : string; virtual;
        protected
        ...
        private
          ...
          function  EfficToLevels(effic : single) : single;
        public
          ...
          property  CompInpEff     : single     read fCompInpEff;
        public
    ...
    TEvaluatorPool =
      class
        ...
        private
          ...
          function GetCompInpEff     : TPercent;
        public
          property EvaluatorCount : integer read GetEvaluatorCount;
          ...
          property CompInpEff     : TPercent read GetCompInpEff;
        public
          procedure AddEvaluator(Evaluator : TOutputEvaluator);
          function  Evaluate(WFEffic : single; AdvWeather : boolean) : TEvaluationResult;
        public
          function  GetHint(ToTycoon : TTycoon) : string;
          function  GetSecondaryTextHeader(lang : string) : string; virtual;
        public
          procedure LoadFromBackup(Reader : IBackupReader);
          procedure StoreToBackup(Writer : IBackupWriter);
        public
...
implementation
  ...
  uses
    Population, MathUtils, SysUtils, WorkCenterBlock, SimHints, EvaluatedBlock, Languages,
    ClassStorage, Standards, Logs;
  ...
  function TOutputEvaluator.BiasOpenLimit(Block : TBlock) : TBiasRatio;
    begin
      if fMetaEvaluator.PrdWthrEnv <> noIndex
        then result := round(Block.WeatherEnvelopValue[fMetaEvaluator.PrdWthrEnv]*fMetaEvaluator.MaxBiasRatio)
        else result := fMetaEvaluator.MaxBiasRatio;
    end;
  ...
  procedure TOutputEvaluator.UpdateBiasRatio(Block : TBlock; WFEffic : single);
    ...
    begin
      ...
      if Extra > 0
        then Deficit := 0
        else Deficit := realmin(Output.Demand, theDt*Output.MetaOutput.MaxFluid.Q*UgLevel);
      // IF THERE IS DEFICIT AND DEMAND AT THE SAME TIME THEM SOMETHING IS VERY WRONG!!!!!
      if Deficit > 0
        then
          begin
            ...
            if (Block.Facility.Trouble and facNeedsWorkForce = 0) and not NeedsMoreSupply
              then
                begin
                  if {CanIncProduction and} (fBiasRatio < fMetaEvaluator.MaxBiasRatio)
                    then
                      begin
                        // Calculate the delta increment
                        Delta := min(fMetaEvaluator.fBiasInc, max(1, round(Deficit*fMetaEvaluator.MaxBiasRatio/(UgLevel*theDt*Output.MetaOutput.MaxFluid.Q))));
                ...
                if CanIncProduction
                  then
                    begin
                      ...
                      fExtraHistory := (fExtraHistory shl 1) or $1;
                    end;
          end
        else
          begin
                        ...
                        if CanDecProduction
                          then
                            begin
                              ...
                              Delta := min(fMetaEvaluator.fBiasDec, max(1, round(Extra*fMetaEvaluator.MaxBiasRatio/(UgLevel*theDt*Output.MetaOutput.MaxFluid.Q))));
                              // Adjust the BiasRatio
                              {
                              if fBiasRatio - Delta*theDt > 0
                                then fBiasRatio := fBiasRatio - round(Delta*theDt)
                                else fBiasRatio := round(Delta*theDt) div 2;
                              }
                              fBiasRatio := max(0, fBiasRatio - round(Delta*theDt));
                              // Clear the history
                              // fExtraHistory := 1;
                            end;
                      end
      ...
      // Cannot open beyond a limit, the season in this case sets the limit...
      fBiasRatio := min(fBiasRatio, BiasOpenLimit(Block));

      // Check if the technology exists
      if (fBiasRatio > 0) and (Block.Facility.MetaFacility.Technology <> nil) and not Block.Facility.Company.HasInvention[Block.Facility.MetaFacility.Technology.NumId]
        then fBiasRatio := 0;
    end;
  ...
  {function TOutputEvaluator.MaxToPullFromImput(Block : TBlock; Input : TInput) : TFluidValue;
    ...
    begin
      Output := Block.Outputs[fMetaEvaluator.MetaOutput.Index];
      Info   := fMetaEvaluator.Info[Input];
      MaxCap := Output.MetaOutput.MaxFluid.Q*Block.dt; // >> ## unused code..
      Extra  := POutputData(Output.FluidData).Extra.Q;
  ...
  procedure TOutputEvaluator.GetInventionsEffect(Block : TBlock; var QEffect : TInventionEfficiencyEffect; var KEffect : integer; DsgnK : TPercent);
    begin
      TEvaluatedBlock(Block).GetInventionsEffect(QEffect, KEffect, DsgnK);
    end;

  procedure TOutputEvaluator.RecalculateInventionsEffect(Block : TBlock);
    begin
      TEvaluatedBlock(Block).GetInventionsEffect(fInvQEffect, fInvKEffect, MetaEvaluator.DesignMaxK);
    end;
    ...
    begin
      with Block.Outputs[fMetaEvaluator.MetaOutput.Index] do
        begin
          ...
          FluidData.K := MetaEvaluator.DesignMaxK;
  ...
  function TOutputEvaluator.CanIncProduction : boolean;
    var
      bits : byte;
    begin
      bits   := 8 - MetaEvaluator.TurnCount;
      result := fExtraHistory shl bits = 0;
    end;
  ...
  function TOutputEvaluator.CanDecProduction : boolean;
    var
      mask : TExtraHistory;
    begin
      mask   := TExtraHistory(high(fExtraHistory) shr MetaEvaluator.fTurnCount);
      result := (fExtraHistory and mask) = mask;
    end;
  ...
  procedure TOutputEvaluator.UpdateExtraHistory(Block : TBlock);
    ...
    begin
      if POutputData(Block.Outputs[fMetaEvaluator.MetaOutput.Index].FluidData).Extra.Q > 0
        then
          for i := 1 to 1{Block.dt} do
            fExtraHistory := byte(byte(fExtraHistory shl 1) or byte($01))
        else
          for i := 1 to 1{Block.dt} do
            fExtraHistory := byte(fExtraHistory shl 1);
    end;
  ...
  function TOutputEvaluator.HasClients(Output : TOutput) : boolean;
    ...
    begin
      ...
      while (i < Count) and (conn = 0) do
        begin
          Input  := Output.Connections[i];
          ExtCnx := Output.ExtraConnectionInfo[i];
          if (Input <> nil) and (not Input.Block.Facility.CriticalTrouble) and (ExtCnx <> nil) and ExtCnx.Connected
            then conn := conn + 1
            else inc(i);
        end;
      result := conn > 0;
  ...
  function TOutputEvaluator.DoProduce(Block : TBlock; WFEffic : single) : single;
    ...
    begin
      try
        ...
        // Inventions
        //GetInventionsEffect(Block, InvQEffect, InvK, fMetaEvaluator.DesignMaxK);

        // Integrators
        IntEffect := GetIntegratorsEffect(Block);
        // Company Inputs
        GetCompanyInputsEffect(Block, CompInpQ, CompInpK);
        ...
        if ImpKa > 0
          then
            begin
              Ka := Ka/ImpKa;
              K  := (1 - fMetaEvaluator.AdtnKFrac)*Kb + fMetaEvaluator.AdtnKFrac*Ka;
            end
          else K := Kb;
        // Production quality
        Production.K := max(1, min(100, round(CompInpK*Compsup*(0.3*K + 0.7*fInvKEffect)))); // Production.K := min(100, round(CompInpK*((InvKEffect*(1 - fMetaEvaluator.DesignMaxK/100)) + round((fMetaEvaluator.DesignMaxK/100)*() + fMetaEvaluator.AdtnKFrac*Ka))));
        // Production capacity
        if ImpAdtnl > 0
          then AdtnlEffic := OpenRatio*AdtnlEffic/ImpAdtnl
          else AdtnlEffic := 0;

        result   := fMetaEvaluator.DesignEffic*fInvQEffect;//*CompInpQ;
        ActEffic := CompInpQ*CompSup*IntEffect*((1 - fMetaEvaluator.AdtnQFrac)*BasicCap + AdtnlEffic*fMetaEvaluator.fAdtnQFrac);
        fOperationEffic := round(100*result*WFEffic);

        // Evaluate the amount of product to produce
        TotalToProd := EfficToLevels(result)*fMetaEvaluator.MetaOutput.MaxFluid.Q*dt*UpgLevel;
        ...
        // Spend budget >>>!!!!
        Block.BlockGenMoney(-OpenRatio*dt*UpgLevel*fMetaEvaluator.Budget*(1 - fMetaEvaluator.fAdtnQFrac*AdtnlEffic), TMetaEvaluatedBlock(Block.MetaBlock).MantainanceAccount); // >>! Block.BlockGenMoney(-ActEffic*fMetaEvaluator.Budget*(fBudgetPercent/100)*(1 - fMetaEvaluator.fAdtnQFrac*AdtnlEffic), TMetaEvaluatedBlock(Block.MetaBlock).MantainanceAccount);
        fCompInpEff := CompInpQ;
      except
        result := 1;
      end;
    end;
  ...
  function TOutputEvaluator.EfficToLevels(effic : single) : single;
    begin
      if effic < 1
        then result := 1
        else result := 3*effic - 2;
    end;
  ...
  procedure TOutputEvaluator.GetCompanyInputsEffect(Block : TBlock; var QEffect : single; var KEffect : single);
    var
      i         : integer;
      ...
      d1,d2,d3  : double;
    begin
      if Block.Facility.MetaFacility.ReqCmpSupplies
        then
          begin
            sK        := 0;
            QEffect   := 1;
            ...
            for i := 0 to pred(MetaEvaluator.fCompInputCount) do
              begin
                ...
                InputData := Block.CompanyInputs[Info.Index];
                if InputData.Max > 0
                  then
                    begin
                      d1 := InputData.Max;
                      d2 := InputData.Q;
                      d3 := d2/d1;
                      CompSuppl := realmin(CompSuppl, d3); // >> Quality
                    end;
                KEffect := KEffect + Info.K*CompSuppl;
                sK      := sK      + Info.K;
              end;
            if CompSuppl > 0
              then QEffect := sqrt(realmax(0, realmin(1, CompSuppl + 0.05)))
              else QEffect := 0;
            KEffect := realmin(1, realmax(0.6, KEffect - sK));
          end
        else
          begin
            QEffect := 1;
  ...
  function TOutputEvaluator.WeatherEffect(Block : TBlock) : single;
    begin
      if fMetaEvaluator.PrdWthrEnv <> noIndex
        then result := Block.WeatherEnvelopValue[fMetaEvaluator.PrdWthrEnv]
        else result := 1;
    end;
  ...
  procedure TOutputEvaluator.PlanInputForNextPeriod(Block : TBlock; Effic : single);
    var
      ...
      UpgLevel  : single;
    begin
      UpgLevel  := realmax(0, Block.UpgradeLevel);
      if fBiasRatio > 0
        then
                ...
                then
                  begin
                    ReqFluid :=
                      ...
                      UpgLevel;
                    Input.ActualMaxFluid.Q := realmin(Input.MetaInput.MaxFluid.Q*UpgLevel, Input.ActualMaxFluid.Q + ReqFluid);
  ...
  procedure TOutputEvaluator.PlanCompanyInputs(Block : TBlock; Share : single);
    const
      CompRatio = 100; //CompRatio = 500;
    var
      i              : integer;
      CompComplexity : single;
      UpgLevel       : single;
    begin
      if Block.Facility.MetaFacility.ReqCmpSupplies
        then
          begin
            UpgLevel := realmax(1, Block.UpgradeLevel);
            if Block.Facility.CompanyDir <> nil
              then CompComplexity := Block.Facility.CompanyDir.LastComplexFacs/CompRatio //Block.Facility.CompanyDir.LastCount/CompRatio
              else CompComplexity := 1;
            for i := 0 to pred(MetaEvaluator.fCompInputCount) do
              with Block.CompanyInputs[MetaEvaluator.fCompanyInputs[i].Index]^ do
                Max := Max + CompComplexity*Share*UpgLevel*(fBiasRatio/MetaEvaluator.MaxBiasRatio)*MetaEvaluator.fCompanyInputs[i].Max;
          end
        else
          for i := 0 to pred(MetaEvaluator.fCompInputCount) do
            with Block.CompanyInputs[MetaEvaluator.fCompanyInputs[i].Index]^ do
  ...
  function TOutputEvaluator.FormatOutput(Block : TBlock; kind : TStatusKind; ShowName : boolean; ToTycoon : TTycoon ) : string;
    var
      Output : TOutput;
      dt     : TTimeDelta;
    begin
      dt := Block.dt;
      if dt = 0
        then dt := 1; // >> It might never happen!!!
      case kind of
        sttMain :
          begin
            if ShowName
              then result := SimHints.GetHintText( mtidOutputEvalFull.Values[ToTycoon.Language], [MetaEvaluator.MetaOutput.MetaFluid.Name_MLS.Values[ToTycoon.Language], fOperationRatio] )
              else result := SimHints.GetHintText( mtidOutputEval.Values[ToTycoon.Language], [fOperationRatio] )
          end;
        sttSecondary :
          begin
            Output := Block.Outputs[fMetaEvaluator.MetaOutput.Index];
            if ShowName
              then result :=
                     SimHints.GetHintText(
                       mtidOutputSec.Values[ToTycoon.Language],
                       [
                       Output.MetaOutput.MetaFluid.FormatValue(Output.LastFluid/dt, ToTycoon.Language),
                       Output.MetaOutput.MetaFluid.Name_MLS.Values[ToTycoon.Language],
                       Output.FluidData.K,
                       Efficiency
                       ] );
          end;
  ...
  function TMetaEvaluatorPool.Instantiate(aBlock : TBlock) : TEvaluatorPool;
    var
      i : integer;
    begin
      result := EvaluatorPoolClass.Create(aBlock);
  ...
  function TEvaluatorPool.GetCompInpEff : TPercent;
    var
      i   : integer;
      sum : single;
      imp : single;
    begin
      ...
      for i := 0 to pred(fEvaluators.Count) do
        with Evaluators[i] do
          begin
            sum := sum + MetaEvaluator.fImportance*fCompInpEff;
            imp := imp + MetaEvaluator.fImportance;
          end;
      result := min(100, round(100*sum/imp));
    end;
...

{
  ...
  procedure TEvaluatorPool.AddEvaluator(Evaluator : TOutputEvaluator);
    begin
      fEvaluators.Insert(Evaluator);
      if Block.OutputsByName[Evaluator.MetaEvaluator.MetaOutput.Name] = nil
        then Logs.Log('Survival', DateTimeToStr(Now) + ' Registering Unknown Output (' + Evaluator.MetaEvaluator.MetaOutput.Name + ').');
    end;


  function TEvaluatorPool.Evaluate(WFEffic : single; AdvWeather : boolean) : TEvaluationResult;
    var
      EvlCnt        : integer;
      i             : integer;
          ...
          then
            begin
              ...
              for i := 0 to pred(EvlCnt) do
                with Evaluators[i] do
                  EvaluateTable[i] := not AdvWeather and ShouldProduce(fBlock) and CheckSupplies(fBlock);
        ...
        then
          begin
            ...
            while (i < count) and (Evaluators[i].fIllInputIndex = noIllInput) do
              inc(i);
            if i < count
              then result := GetHintText(mtidEvalBlockNeedsBasicInput.Values[ToTycoon.Language], [fBlock.Inputs[Evaluators[i].fIllInputIndex].MetaInput.MetaFluid.Name_MLS.Values[ToTycoon.Language]])
              else result := GetHintText(mtidEvalBlockNeedsBasicInput.Values[ToTycoon.Language], [mtidSupplies.Values[ToTycoon.Language]]); // >> MLS ALARM
          end
        else
          begin
            ...
            if i < count
              then
                if Evaluators[i].fIllInputIndex <> noIllInput
                  then result := GetHintText(mtidEvalBlockNeedsMoreSupplies.Values[ToTycoon.Language], [fBlock.Inputs[Evaluators[i].fIllInputIndex].MetaInput.MetaFluid.Name_MLS.Values[ToTycoon.Language], Evaluators[i].MetaEvaluator.MetaOutput.MetaFluid.Name_MLS.Values[ToTycoon.Language]])
                  else result := GetHintText(mtidEvalBlockNeedsMoreSupplies.Values[ToTycoon.Language], [mtidSupplies.Values[ToTycoon.Language], Evaluators[i].MetaEvaluator.MetaOutput.MetaFluid.Name_MLS.Values[ToTycoon.Language]])
              else result := '';
            if GetCompInpEff < 70 //
              then result := result + ' ' + GetHintText(mtidEvalBlockNeedsMoreCompSupplies.Values[ToTycoon.Language], [GetCompInpEff])
              else
                if i >= count
                  then result := GetHintText(mtidVisitWebSite.Values[ToTycoon.Language], [0]);
          end;
    end;

  function TEvaluatorPool.GetSecondaryTextHeader(lang : string) : string;
    begin
      result := mtidEvalBlockProducing.Values[lang]; // 'Producing: ';
    end;
