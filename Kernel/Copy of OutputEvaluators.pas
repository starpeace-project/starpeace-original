...
interface
  ...
  const
    DefaultAdtnQFrac  = 0.05; // 5%
    DefaultAdtnKFrac  = 0.05; // 5%
    ...
    TOutputEvaluator =
      class
        ...
        public
          ...
          //function  NeedsInput(Input : TInput) : boolean;
          ...
          //procedure PayWorkForce(Block : TBlock; share : single);
          function  CheckSupplies(Block : TBlock) : boolean;
        public
          procedure CalcuteOperationRatio(Block : TBlock);
          function  FormatOutput(Block : TBlock; kind : TStatusKind; ShowName : boolean) : string; virtual;
        protected
          ...
          //function  GetExtraTendency : byte;
          function  HasClients(Output : TOutput) : boolean;
          //function  HasEnoughSupport(Block : TBlock) : boolean;
        public
        ...
        public
          property  BudgetPercent  : TPercent   read fBudgetPercent  write fBudgetPercent;
...
implementation
  ...
  function TOutputEvaluator.DoProduce(Block : TBlock; WFEffic : single) : single;
    ...
    begin
      try
        ...
        BasicCap    := fBiasRatio/fMetaEvaluator.fMaxBiasRatio;
        AdtnlCap    := 0;
        // HasBscInputs := false;
        Kb       := 0;
        Ka       := 0;
        ...
        for i := 0 to pred(InputCount) do
          begin
            ...
            case MetaInputInfo.InputKind of
              ikBasic :
                begin
                  MaxInputCap := Input.MetaInput.MaxFluid.Q*MetaInputInfo.Share*Block.dt;
                  if MaxInputCap > 0
                    then
                      begin
                        InputCap := Input.FluidData.Q/MaxInputCap;
                        Kb       := Kb + MetaInputInfo.Importance*Input.FluidData.K;
                        ImpKb    := ImpKb + MetaInputInfo.Importance;
                        BasicCap := realmin(BasicCap, InputCap);
                      end
                end;
              ikAditional :
                begin
                  ...
                  if MaxInputCap > 0
                    then
                      begin
                        ...
                        Ka       := Ka + InputCap*MetaInputInfo.Importance*Input.FluidData.K;
                        ImpKa    := ImpKa + InputCap*MetaInputInfo.Importance;
                        AdtnlCap := AdtnlCap + MetaInputInfo.Importance*InputCap;
                      end;
                end;
            end;
          end;
        ...
        // Calculate the real share that represents matter generation
        //GenShare := realmin(1, fMetaEvaluator.GenPercent/100);
        // Company Inputs
        ...
        // Production capacity
        if ImpAdtnl > 0
          then AdtnlCap := AdtnlCap/ImpAdtnl
          else AdtnlCap := 0;
        // Generated matter
        //GenCap := (fBiasRatio/fMetaEvaluator.MaxBiasRatio)*sqrt(sqrt(fBudgetPercent/100));
        // Calculate total capacity
        //ActEffic := CompSup*IntEffect*((1 - fMetaEvaluator.AdtnQFrac)*(GenShare*GenCap + (1 - GenShare)*BasicCap) + AdtnlCap*fMetaEvaluator.fAdtnQFrac)*fMetaEvaluator.DesignEffic;
        result   := fMetaEvaluator.DesignEffic*InvQEffect*CompInpQ*IntEffect;
        ActEffic := result*CompSup*((1 - fMetaEvaluator.AdtnQFrac)*BasicCap + AdtnlCap*fMetaEvaluator.fAdtnQFrac);
        fOperationEffic := round(100*result*WFEffic);
        Production.Q := realmin(1, ActEffic)*WFEffic*TotalToProd;
        // Calculate operation ratio
        fOperationRatio := min(100, SmartRound(100*Production.Q/(Output.MetaOutput.MaxFluid.Q*Block.dt)));
        // Spend budget
        Block.BlockGenMoney(-ActEffic*fMetaEvaluator.Budget*(fBudgetPercent/100)*(1 - fMetaEvaluator.fAdtnQFrac*AdtnlCap), TMetaEvaluatedBlock(Block.MetaBlock).MantainanceAccount);
      except
        result := 1;
      end;
    end;
  ...
  procedure TOutputEvaluator.LoadFromBackup(Reader : IBackupReader);
    begin
      fBudgetPercent  := Reader.ReadByte('Budget', 0);
      fBiasRatio      := round(fMetaEvaluator.MaxBiasRatio*Reader.ReadByte('BiasRatio', 100)/100);
      fSupplyRatio    := Reader.ReadByte('SupplyRatio', 100);
      fOperationRatio := Reader.ReadByte('OperationRatio', 100);
    end;

  procedure TOutputEvaluator.StoreToBackup(Writer : IBackupWriter);
    begin
      Writer.WriteByte('Budget', fBudgetPercent);
      Writer.WriteByte('BiasRatio', round(100*fBiasRatio/fMetaEvaluator.MaxBiasRatio));
      Writer.WriteByte('SupplyRatio', fSupplyRatio);
      Writer.WriteByte('OperationRatio', fOperationRatio);
    end;

  procedure TOutputEvaluator.CalcuteOperationRatio(Block : TBlock);
    var
      Output     : TOutput;
      OutputData : POutputData;
    begin
      Output := Block.Outputs[fMetaEvaluator.MetaOutput.Index];
      OutputData := POutputData(Output.FluidData);
      fOperationRatio := min(100, SmartRound(100*OutputData.Q/(Output.MetaOutput.MaxFluid.Q*Block.dt)));
    end;
            ...
            begin
              ...
              if not (mfWorkForce in Input.MetaInput.MetaFluid.Options)
                then
                  begin
                    if Effic < MinEffic
                      then ReqFluid := Input.MetaInput.MaxFluid.Q*InputInfo.Share
                      else
                        ReqFluid :=
                          Input.MetaInput.MaxFluid.Q*
                          (fBiasRatio/fMetaEvaluator.MaxBiasRatio)*
                          InputInfo.Share/Effic;
  ...
  function TEvaluatorPool.Evaluate(WFEffic : single) : TEvaluationResult;
    ...
    function CreateEvaluateTable : integer;
      var
        i : integer;
      begin
        result := 0;
        for i := 0 to pred(EvlCnt) do
          with Evaluators[i] do
            if ShouldProduce(fBlock) and CheckSupplies(fBlock)
              then
                begin
    ...
    procedure SignInputsRelationShip;
      var
        EvlIdx    : integer;
        InfoIdx   : integer;
        InputIdx  : integer;
        Evaluator : TOutputEvaluator;
      begin
        for EvlIdx := 0 to pred(EvlCnt) do
          if EvaluateTable[EvlIdx]
            then
              begin
                Evaluator := Evaluators[EvlIdx];
                for InfoIdx := 0 to pred(Evaluator.MetaEvaluator.fMetaInfoList.Count) do
                  begin
                    InputIdx := Evaluator.MetaEvaluator.MetaInputInfo[InfoIdx].MetaInput.Index;
                    ShareOutTable[EvlIdx, InputIdx] := 1;
                  end;
              end;
      end;

    procedure ShareInputs(index : integer);
      var
        EvaluatorShares : TEvaluatorShares;
        sum             : single;
        aux             : single;
        i               : integer;
      begin
        sum := 0;
        for i := 0 to pred(EvlCnt) do
          if EvaluateTable[i] and (ShareOutTable[i, index] = 1)
            then
              begin
                aux := Evaluators[i].ShareToPullFromImput(fBlock.Inputs[index]);
                EvaluatorShares[i] := aux;
                sum := sum + aux;
              end
            else EvaluatorShares[i] := 0;
        for i := 0 to pred(EvlCnt) do
          if sum > 0
            then ShareOutTable[i, index] := InputValues[index]*EvaluatorShares[i]/sum
            else ShareOutTable[i, index] := 0;
      end;

    procedure AssignInputs(index : integer);
      var
        i : integer;
      begin
        for i := 0 to pred(InputCount) do
          fBlock.Inputs[i].FluidData.Q := ShareOutTable[index, i];
      end;
        ...
        if not fBlock.Facility.CriticalTrouble
          then
            begin
              ...
              // Create the evaluator table and get how many of them will be evaluated.
              CanEvlCnt := CreateEvaluateTable;
              // If more than one evaluator is going to be evaluated then...
              if CanEvlCnt > 1
                then
                  begin
                    // Clear the share out table
                    FillChar(ShareOutTable, sizeof(ShareOutTable), 0);
                    // Get the net inputs to be shared.
                    for i := 0 to pred(InputCount) do
                      InputValues[i] := fBlock.Inputs[i].FluidData.Q;
                    // Sign the relationship between inputs and evaluators.
                    SignInputsRelationShip;
                    // And then share the inputs.
                    for i := 0 to pred(InputCount) do
                      ShareInputs(i);
                  end;
              for i := 0 to pred(EvlCnt) do
                  ...
                  if EvaluateTable[i]
                    then
                      begin
                        ...
                        //CurEvaluator.PayWorkForce(fBlock, 1/EvaluatorCount);
                      end
                    else
                      begin
                        CurEvaluator.BeforeProduce(fBlock);
              ...
              // Make next period plan null
              for i := 0 to pred(InputCount) do // >> BARBARO!!
                if mfTradeable in fBlock.Inputs[i].MetaInput.MetaFluid.Options
                  then fBlock.Inputs[i].ActualMaxFluid.Q := 0;
