...
interface
  ...
  type
    ...
    TMetaFacilityOption  = (mfcInTown, mfcGenerateName, mfcShowCompanyInText, mfcShowProfitInText, mfcStopDisabled, mfcIgnoreZoning, mfcForbiddenRename);
    ...
    TTycoon =
      class( TMoneyDealer )
        ...
        private
          ...
          procedure SetIsDemo(value : boolean);
        published
          ...
          property IsDemo       : boolean             read GetIsDemo      write SetIsDemo;
          property RoadBlocks   : integer             read fRoadBlocks    write fRoadBlocks;
        published
          property EffectId        : word   read fEffectId       write fEffectId;
          property EffectCreator   : string read fEffectCreator  write fEffectCreator;
...
implementation
  ...
  procedure TFacility.SetStopped( Value : boolean );
    begin
      ...
      if CheckOpAuthenticity and not (mfcStopDisabled in MetaFacility.Options)
        then
          begin
            Lock;
            try
  ...
  procedure TFacility.EndOfPeriod( PeriodType : TPeriodType; PeriodCount : integer );
    ...
    procedure ChargeTaxes;
      var
        ...
        Tax   : TTax;
      begin
        if (Town <> nil) and (Company <> nil) and (Company.Owner <> nil) and not Company.Owner.IsDemo
          then
            begin
  ...
  procedure TFacility.RDOSetInputSortMode(FluidId : widestring; mode : integer);
    var
      Input : TPullInput;
    begin
      Logs.Log(tidLog_Survival, DateTimeToStr(Now) + ' Changing Sort Mode.. ');
      try
        Lock;
        try
          Input := TPullInput(CurrBlock.InputsByName[FluidId]);
                            ...
                            for ko := 0 to pred(self.CurrBlock.OutputCount) do
                              for ki := 0 to pred(CurrBlock.InputCount) do
                                if (CurrBlock.Inputs[ki].MetaInput.MetaFluid = self.CurrBlock.Outputs[ko].MetaOutput.MetaFluid) and
                                  ...
                                  then flag := flag or (CurrBlock.Inputs[ki].ConnectTo( self.CurrBlock.Outputs[ko] ) = cnxValid)
                          end;
                    end;
          if flag
            then
        ...
        case Modification of
          ...
          bmEvolve :
            begin
              ...
              try
                Logs.Log( 'Survival', DateTimeToStr(Now) + ' BLOCK EVOLVE: looking for suppliers.' );
                TInhabitedTown(Town).World.SearchForSuppliers( fCurrBlock ); // >> EL TOTI
                Logs.Log( 'Survival', DateTimeToStr(Now) + ' BLOCK EVOLVE: looking for suppliers OK.' );
              except
                Logs.Log( 'Survival', DateTimeToStr(Now) + ' BLOCK EVOLVE: Exception looking for suppliers.' );
              end;
              CheckConnections( chkBoth );
              ...
              if TMetaCompanyInput(MetaBlock.CompanyInputs[i]).Editable
                then
                  begin
                    ...
                    WriteFloat('cInputMax' + IntToStr(i), round(TMetaCompanyInput(MetaBlock.CompanyInputs[i]).Fluid.ConvertToUnits((1 + (UpgradeLevel - 1)/10)*TMetaCompanyInput(MetaBlock.CompanyInputs[i]).Max)));
                  end
                else WriteFloat('cInputMax' + IntToStr(i), round(TMetaCompanyInput(MetaBlock.CompanyInputs[i]).Fluid.ConvertToUnits(UpgradeLevel*TMetaCompanyInput(MetaBlock.CompanyInputs[i]).Max)));
        ...
        then
          try
            if (index >= 0) and (index < CompanyInputCount)
              then
                begin
                  CompanyInputs[index].Max := realmax(1, UpgradeLevel)*(min(perc, 100)/100)*TMetaCompanyInput(MetaBlock.CompanyInputs[index]).Max;
      ...
      if (fUpgradeHours = 0) and (fUpgradeLevel < MetaBlock.MaxUpgrade)
        then
          begin
            ...
            Facility.Company.GenMoney(-upgCost, accIdx_Construction);
            ModelServerCache.BackgroundInvalidateCache(Facility); //ModelServerCache.UpdateObjectCache(Facility, noKind, noInfo)
          end;
    end;
  ...
  *)
  ...
  constructor TSlice.Create;
    begin
      inherited;
      ExtraConnectionInfo.Distance := -1;
    end;
...

{
  ...
  constructor TTycoon.Create( anId : TTycoonId );
    begin
      ...
      if TheGlobalConfigHandler.GetConfigParm('Tutorial', 'disabled') = 'enabled'
  ...
  function TTycoon.GetIsDemo : boolean;
    begin
      Lock;
      try
        result := Cookie['demo'] <> '';
  ...
  procedure TTycoon.SetIsDemo(value : boolean);
    begin
      Lock;
      try
        if value
  ...
  function TTycoon.RDOSendMoney( ToTycoon, Reason, AmountStr : widestring ) : OleVariant;
    ...
    begin
      ...
      try
        ...
        if (Amount > 0) and (Dest <> nil) and (not IsRole or Dest.IsRole) and Dest.CanGetLoan(Amount)
          then
            begin
                ...
                then
                  begin
                    if Loan.Bank.Owner <> nil
                      then Loan.Bank.Owner.GenMoney(Loan.Amount*(1 + Loan.Interest/100), accIdx_Bank_LoanIncome);
  ...
  procedure TTycoon.EndOfPeriod( PeriodType : TPeriodType; PeriodCount : integer );
    ...
    begin
      ...
      Lock;
      try
        if (PeriodType = perHour) and (fTutorial <> nil) and IsOnline and HasValidCompany and fTutorial.Active and fTutorial.TimeToSimulate(PeriodCount)
          then
            begin
  ...
  procedure TTycoon.ResetTutorial;
    begin
      Lock;
      try
        if (fTutorial <> nil) and (fNobPoints < 10)
          then
            begin
              fTutorial.Free;
              fTaskContext := nil;
  ...
  function TTycoon.GetLoanLimit : TMoney;
    begin
      if (Level <> nil)
        then result := (Level.Tier + 1)*500*1000*1000
