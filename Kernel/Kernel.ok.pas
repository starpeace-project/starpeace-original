...
interface
  ...
  uses
    ...
    MapStringToObject, Rankings, Inventions, Seasons, Events, Languages;
  ...
  type
    ...
    TMetaFacilityOption  = (mfcInTown, mfcGenerateName, mfcShowCompanyInText, mfcShowProfitInText, mfcStopDisabled, mfcIgnoreZoning);
    ...
    TMetaBlock =
      class( TMetaInstance )
        ...
        published
          ...
          property Transcends     : boolean             read fTranscends     write fTranscends;
        private
...
implementation
  ...
  procedure TMetaFacility.RequiresInvention(Invention : TInvention);
    ...
    begin
      ...
      for i := 0 to pred(LangList.Count) do
        fRequires_MLS.Values[LangList[i]] := SimHints.GetHintText(mtidTechRequired.Values[LangList[i]], [Invention.Name, Invention.Resp]);
    end;
  ...
  procedure TMetaFacility.EvaluateTexts;
    ...
    begin
      inherited;
      for i := 0 to pred(LangList.Count) do
        Desc_MLS.Values[LangList[i]] := TypicalStage.MetaBlock.Desc_MLS.Values[LangList[i]] + Desc_MLS.Values[LangList[i]];
    end;
  ...
  constructor TMetaGate.Create( aName          : string;
    ...
    begin
      ...
      if ClassIs(TPushInput.ClassName, fGateClass) and (sizeof(TPushInputData) <> aFluidDataSize)
        then Logs.Log( 'ClassInfo', DateTimeToStr(Now) + ' - Invalid record size ' + Name );
  ...
  procedure TFacility.SetName( aName : string );
    begin
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Renaming Facility.' );
      Lock;
      try
        if fName <> aName
          then
  ...
  procedure TFacility.SetStopped( Value : boolean );
    begin
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Stopping Facility.' );
      if not (mfcStopDisabled in MetaFacility.Options)
        then
          begin
            Lock;
            try
...

{
  procedure TFacility.SetDeleted( Value : boolean );
    begin
      try
        ...
        Logs.Log( 'Survival', DateTimeToStr(Now) + ' Error in SetDelete of ' + Name);
      end;
    end;
}
        ...
        case PeriodType of
          ...
          perMonth :
            begin
              ...
              if (fToBeDemolished <> 1) and fRequiresLandTech and not WaterAllowed
                then Town.ModelFactory.RequestDeletion( self );
            end;
        end;
      finally
  ...
  procedure TFacility.Loaded;
    begin
      try
        ...
        Logs.Log( 'Survival', DateTimeToStr(Now) + ' Error in TFacility.Loaded ' + Name);
      end;
    end;
  ...
  function TFacility.ConnectTo( Facility : TFacility ) : string;
    begin
      try
        ...
        Logs.Log( 'Survival', DateTimeToStr(Now) + ' Error in ConnectTo facility ' + Name + ' to ' + Facility.Name);
        result := '';
      end;
    end;
  ...
  procedure TFacility.DisconnectFrom( Facility : TFacility );
    begin
      try
        ...
        Logs.Log( 'Survival', DateTimeToStr(Now) + ' Error in Disconnecting facility ' + Name + ' from ' + Facility.Name);
      end;
    end;
  ...
  procedure TFacility.RDOConnectInput( FluidId, Suppliers : widestring );
    begin
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' - Fac(' + IntToStr(XPos) + ',' + IntToStr(YPos) + ') ' + 'Input connected: ' + FluidId + ' to ' + Suppliers );
      ModifyConnection( FluidId, Suppliers, true, true );
    end;

  procedure TFacility.RDOConnectOutput( FluidId, Clients : widestring );
    begin
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' - Fac(' + IntToStr(XPos) + ',' + IntToStr(YPos) + ') ' + 'Output connected: ' + FluidId + ' to ' + Clients );
      ModifyConnection( FluidId, Clients, true, false );
    end;

  procedure TFacility.RDODisconnectInput( FluidId, Suppliers : widestring );
    begin
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' - Fac(' + IntToStr(XPos) + ',' + IntToStr(YPos) + ') ' + 'Input disconnect: ' + FluidId + ' from ' + Suppliers );
      ModifyConnection( FluidId, Suppliers, false, true );
    end;

  procedure TFacility.RDODisconnectOutput( FluidId, Clients : widestring );
    begin
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' - Fac(' + IntToStr(XPos) + ',' + IntToStr(YPos) + ') ' + 'Output disconnect: ' + FluidId + ' from ' + Clients );
  ...
  procedure TFacility.RDOSetOutputPrice( FluidId : widestring; Price : integer );
    var
      Output : TOutput;
    begin
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' - Fac(' + IntToStr(XPos) + ',' + IntToStr(YPos) + ') ' + 'Output price set: ' + FluidId + ' to ' + IntToStr(Price) );
      try
        Lock;
        try
  ...
  procedure TFacility.RDOSetInputOverPrice( FluidId : widestring; SupplierIdx, OverPrice : integer );
    ...
    begin
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' - Fac(' + IntToStr(XPos) + ',' + IntToStr(YPos) + ') ' + 'Input overprice set: ' + FluidId + ' to ' + IntToStr(OverPrice) );
      try
        Lock;
        try
        ...
        on E : Exception do
          Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error in SetOverPrice ' + E.Message );
      end;
    end;                   
  ...
  procedure TFacility.RDOSetInputMaxPrice( FluidId : widestring; MaxPrice : integer );
    var                                                     
      Input : TPullInput;
    begin
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' - Fac(' + IntToStr(XPos) + ',' + IntToStr(YPos) + ') ' + 'Input max price set: ' + FluidId + ' to ' + IntToStr(MaxPrice) );
      try
        Lock;
        try
          Input := TPullInput(CurrBlock.InputsByName[FluidId]);
          if (Input <> nil) and ObjectIs('TPullInput', Input)
            then Input.MaxPrice := MaxPrice;
        finally
          Unlock;
        end;
        UpdateCache;
  ...
  procedure TFacility.RDOSetInputMinK( FluidId : widestring; MinK : integer );
    var
      Input : TPullInput;
    begin
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' - Fac(' + IntToStr(XPos) + ',' + IntToStr(YPos) + ') ' + 'Input min K set: ' + FluidId + ' to ' + IntToStr(MinK) );
      try
        Lock;
        try
          Input := TPullInput(CurrBlock.InputsByName[FluidId]);
          if Input <> nil
            then Input.MinK := MinK;
        finally
          Unlock;
        end;
        UpdateCache;
  ...
  function TFacility.RDOGetConnectionReport : OleVariant;
    var
      ...
      Gate   : TGate;
    begin
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' - Fac(' + IntToStr(XPos) + ',' + IntToStr(YPos) + ') ' + 'Get connection report' );
      try
        Lock;
        try
          with CurrBlock do      
        ...
        Logs.Log( tidLog_Survival, DateTimeToStr(Now) + 'Get connection report error.' );
      end;
      result := report;
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + 'Get connection report OK.' );
    end;
  ...
  procedure TFacility.RDOConnectToTycoon( TycoonId, FacTypes : integer; SetAsDefault : wordbool );
    var
      ...
      ki, ko  : integer;
    begin
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' - Fac(' + IntToStr(XPos) + ',' + IntToStr(YPos) + ') ' + 'Connect to Tycoon: ' + IntToStr(TycoonId) );
      try
        Lock;
        try
          ...
          for i := 0 to pred(Tycoon.Companies.Count) do
            for j := 0 to pred(TCompany(Tycoon.Companies[i]).fFacilities.Count) do
              with TFacility(TCompany(Tycoon.Companies[i]).fFacilities[j]) do
                begin
                  if (FacTypes and ftpWarehouses <> 0) and (MetaFacility.Kind.Role in [rolDistributer, rolCompExport, rolCompInport]) or
  ...
  procedure TFacility.RDODisconnectFromTycoon( TycoonId, FacTypes : integer; RemoveAsDefault : wordbool );
    var
      ...
      ki, ko  : integer;
    begin
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' - Fac(' + IntToStr(XPos) + ',' + IntToStr(YPos) + ') ' + 'Disconnect from Tycoon: ' + IntToStr(TycoonId) );
      try
        Lock;
        try
          Tycoon := TTycoon(pointer(TycoonId));
  ...
  procedure TFacility.RDOConnectToTradeCenter;
    var
      ...
      ki, ko      : integer;
    begin
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' - Fac(' + IntToStr(XPos) + ',' + IntToStr(YPos) + ') ' + 'Connect to Trade Center' );
      try
        Lock;
        try
  ...
  procedure TFacility.RDODisconnectFromTradeCenter;
    var
      ...
      ki, ko      : integer;
    begin
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' - Fac(' + IntToStr(XPos) + ',' + IntToStr(YPos) + ') ' + 'Disconnect from Trade Center' );
      try
        Lock;
        try
          TradeCenter := TInhabitedTown(Town).TradeCenter.CurrBlock;
  ...
  procedure TFacility.RDOStartUpgrade;
    var
      Block : TBlock;
    begin
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Facility Start Upgrade..' );
      try
        Block := CurrBlock;
        if (Block.Facility = self) and (Block.MetaBlock <> nil) and (Block.UpgradeLevel < Block.MetaBlock.MaxUpgrade) and (Company <> nil) and (Company.Owner <> nil) and (Company.Owner.FacCount < Company.Owner.Level.FacLimit)
          then Block.StartUpgrading;
        Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Facility Start Upgrade OK!' );
      except
        Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error in Facility Start Upgrade..' );
      end;
    end;
  ...
  procedure TFacility.RDOStopUpgrade;
    var
      Block : TBlock;
    begin
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Facility Stop Upgrade..' );
      try
        Block := CurrBlock;
        if (Block.Facility = self) and (Block.UpgradeHours > 0)
          then Block.StopUpgrading;
        Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Facility Stop Upgrade OK!' );
      except
        Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error in Facility Stop Upgrade..' );
      end;
    end;
  ...
  procedure TFacility.RDODowngrade;
    var
      Block : TBlock;
    begin
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Facility Downgrade..' );
      try
        Block := CurrBlock;
        if (Block.Facility = self) and (Block.UpgradeLevel > 1)
          then Block.Downgrade;
        Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Facility Downgrade OK!' );
      except
        Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error in Facility Downgrade..' );
      end;
    end;
        ...
        case Modification of
          ...
          bmEvolve :
            begin
              ...
              try
                Logs.Log( 'Survival', 'BLOCK EVOLVE: looking for suppliers.' );
                TInhabitedTown(Town).World.SearchForSuppliers( fCurrBlock );
                Logs.Log( 'Survival', 'BLOCK EVOLVE: looking for suppliers OK.' );
              except
                Logs.Log( 'Survival', 'BLOCK EVOLVE: Exception looking for suppliers.' );
              end;
              CheckConnections( chkBoth );
              Cache(false);
              try
...

{
  ...
  procedure TFacility.Cache(isNew : boolean);
    begin
      if not Deleted
        then
          begin
            ...
            try
              if isNew
                then CacheObject(self, noKind, recLinks)
                else CacheObject(self, noKind, noInfo)
            finally
              Unlock;
            end;
      end;
  ...
  procedure TFacility.CopySettingsFrom( Facility : TFacility );
    begin
      if (fCurrBlock <> nil) and fCurrBlock.AcceptCloning
        then fCurrBlock.CopySettingsFrom( Facility.CurrBlock );
    end;
  ...
  procedure TFacility.LoadFromBackup( Reader : IBackupReader );
    begin
      ...
      try
        ...
        fToBeDemolished := 1;
        Logs.Log( 'Survival', DateTimeToStr(Now) + 'Error loading facility ' + Name + '(' + IntToStr(fxPos) + ',' + IntToStr(fyPos) + ')');
        raise;
      end;
    end;
  ...
  procedure TFacility.StoreToBackup( Writer : IBackupWriter );
    begin
      try
        ...
        Logs.Log( 'Backup', DateTimeToStr(Now) + 'Error backing up facility ' + Name + '(' + IntToStr(xPos) + ',' + IntToStr(yPos) + ')');
        raise;
      end;
    end;
  ...
  function TFacility.HasTechnology : boolean;
    begin
      result := (MetaFacility.Technology = nil) or ((Company <> nil) and Company.HasInvention[MetaFacility.Technology.NumId]);
    end;

  function TFacility.GetMainLink : string;
    begin
      if (Company <> nil) and (Company.Owner <> nil)
        then result := GetGlobalPath(tidCachePath_Tycoons + Company.Owner.Name + '.five\Companies\' + Company.Name + '\Facilities\' + MetaFacility.Kind.SuperType + '\')
        else result := '';
    end;
  ...
  function TBlock.Evaluate : TEvaluationResult;
    ...
    begin
      ...
      if (Facility.Company <> nil) and (Facility.Company.Owner <> nil)
        then Facility.Company.Owner.CurrFacPrestige := Facility.Company.Owner.CurrFacPrestige + Facility.UpgradeLevel*MetaBlock.Prestige;
    ...
    begin
      ...
      for i := 0 to pred(InputCount) do
        try
          ...
          on E : Exception do
            try
              Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error in Collect (' + IntToStr(xPos) + ',' + IntToStr(yPos) + ') ' + ' Input: ' + IntToStr(i) + ' Fac: ' + Facility.Name + ' ' + E.Message);
              //Input.AutoRepair(self, i);
            except
              //RebuildInput(i);
              Logs.Log(tidLog_Survival, DateTimeToStr(Now) + ' Error in Input.AutoRepair');
            end;
        end;
    end;
    ...
    begin
      ...
      for i := 0 to pred(InputCount) do
        try
          ...
          on E : Exception do
            try
              Logs.log( tidlog_survival, DateTimeToStr(now) + ' error in collectinputextra (' + inttostr(xpos) + ',' + inttostr(ypos) + ') ' + ' input: ' + inttostr(i) + ' Fac: ' + Facility.Name + ' ' + E.Message);
              //Input.AutoRepair(self, i);
            except
              //RebuildInput(i);
              Logs.log( tidlog_survival, DateTimeToStr(now) + ' Error in Input.AutoRepair');
    ...
    begin
      ...
      for i := 0 to pred(OutputCount) do
        try
          ...
          on E : Exception do
            try
              Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error in SpreadOutputs (' + IntToStr(xPos) + ',' + IntToStr(yPos) + ') ' + ' Output: ' + IntToStr(i) + ' ObjId: ' + IntToStr(integer(Facility)) + ' ' + E.Message);
              //Output.AutoRepair(self, i);
            except
              //RebuildOutput(i);
              Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error in Output.AutoRepair');
            end;
        end;
    end;
    ...
    begin
      ...
      for i := 0 to pred(OutputCount) do
        try
          ...
          on E : Exception do
            try
              Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error in SpreadOutputExtra (' + IntToStr(xPos) + ',' + IntToStr(yPos) + ') ' + ' Output: ' + IntToStr(i) + ' ObjId: ' + IntToStr(integer(Facility)) + ' ' + E.Message);
              //Output.AutoRepair(self, i);
            except
              //RebuildOutput(i);
              Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error in Output.AutoRepair');
    ...
    begin
      for i := 0 to pred(InputCount) do
        try
          ...
          on E : Exception do
            try
              Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error in ResetInputs (' + IntToStr(xPos) + ',' + IntToStr(yPos) + ') ' + ' Input: ' + IntToStr(i) + ' ObjId: ' + IntToStr(integer(Facility)) + ' ' + E.Message);
            except
            end;
        end;
    end;
  ...
  procedure TBlock.CheckCircuits;
    ...
    begin
      try
        ...
        on E : Exception do
          Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error in CheckCircuits (' + IntToStr(xPos) + ',' + IntToStr(yPos) + ') ' + E.Message);
      end;
    end;
          ...
          for i := 0 to pred(OutputCount) do
            try
              ...
              on E : Exception do
                begin
                  try
                    Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error in CheckConnections Output ' + IntToStr(i) + ' (' + IntToStr(xPos) + ', ' + IntToStr(yPos) + ') ' + E.Message);
                    //Output.AutoRepair(self, i);
                  except
                    //RebuildOutput(i);
                    Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error in Output.AutoRepair');
          ...
          for i := 0 to pred(InputCount) do
            try
              ...
              on E : Exception do
                begin
                  try
                    Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error in CheckConnections Input ' + IntToStr(i) + ' (' + IntToStr(xPos) + ', ' + IntToStr(yPos) + ') ' + E.Message);
                    //Input.AutoRepair(self, i);
                  except
                    //RebuildInput(i);
                    Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error in Input.AutoRepair');
        ...
        begin
          ...
          for i := 0 to pred(InputCount) do
            if mgoptCacheable in Inputs[i].MetaInput.Options
              then
                begin
                  //WriteString('Input' + IntToStr(cnt), Inputs[i].MetaInput.MetaFluid.Name_MLS.Values[langDefault]); // >> MLS2
                  StoreMultiStringToCache('Input' + IntToStr(cnt) + '.', Inputs[i].MetaInput.MetaFluid.Name_MLS, Cache );
                  WriteString('InputPath' + IntToStr(cnt), GetObjectPath(Inputs[i], noKind, noInfo));
                  try
                    CacheObject( Inputs[i], noKind, noInfo );
                  except
                    // >>
            ...
            if mgoptCacheable in Outputs[i].MetaOutput.Options
              then
                begin
                  ...
                  StoreMultiStringToCache( 'Output' + IntToStr(cnt) + '.', Outputs[i].MetaOutput.MetaFluid.Name_MLS, Cache );
                  WriteString('OutputPath' + IntToStr(cnt), GetObjectPath(Outputs[i], noKind, noInfo));
                  try
                    CacheObject( Outputs[i], noKind, noInfo );
                  except
                    // >>
          ...
          for i := 0 to pred(MetaBlock.CompanyInputs.Count) do
            begin
              ...
              WriteInteger('cInputDem' + IntToStr(i), ceil(TMetaCompanyInput(MetaBlock.CompanyInputs[i]).Fluid.ConvertToUnits(CI.Max)));
              WriteInteger('cInputSup' + IntToStr(i), ceil(TMetaCompanyInput(MetaBlock.CompanyInputs[i]).Fluid.ConvertToUnits(CI.Q)));
              WriteInteger('cInputK' + IntToStr(i), CI.K);
              if (CI.Max > 0) and (CI.Q > 0)
                then WriteInteger('cInputRatio' + IntToStr(i), min(100, round(100*smartround(CI.Q)/smartround(CI.Max))))
                else WriteInteger('cInputRatio' + IntToStr(i), 0);
              if TMetaCompanyInput(MetaBlock.CompanyInputs[i]).Editable
                then WriteString('cEditable' + iStr, 'yes');
              WriteFloat('cInputMax' + IntToStr(i), round(TMetaCompanyInput(MetaBlock.CompanyInputs[i]).Fluid.ConvertToUnits(UpgradeLevel*TMetaCompanyInput(MetaBlock.CompanyInputs[i]).Max)));
              StoreMultiStringToCache( 'cUnits' + IntToStr(i) + '.', TMetaCompanyInput(MetaBlock.CompanyInputs[i]).Fluid.UnitName_MLS, Cache );
            end;
          ...
          WriteInteger( 'MaxUpgrade', MetaBlock.MaxUpgrade );
        end;
    end;
  ...
  procedure TBlock.RemoveFromCache;
    ...
    begin
      for i := 0 to pred(InputCount) do
        if mgoptCacheable in Inputs[i].MetaInput.Options
          then UnCacheObject( Inputs[i], noKind, noInfo );
      for i := 0 to pred(OutputCount) do
        if mgoptCacheable in Outputs[i].MetaOutput.Options
          then UnCacheObject( Outputs[i], noKind, noInfo );
    end;
  ...
  procedure TBlock.CopySettingsFrom( Block : TBlock );
    ...
    begin
      if ObjectIs(ClassName, Block)
        ...
        else Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Trying to clone two different kind of blocks.' );
    end;
  ...
  procedure TBlock.BlockLoaded;
        ...
        for i := pred(Gate.fConnections.Count) downto 0 do
          if not MetaInstances.ObjectIs( SuperClass.ClassName, Gate.fConnections[i] )
            then
              begin
                try
                  Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Fixing Model Inconsistency ' + Gate.fConnections[i].ClassName + ' at ' + Gate.ClassName );
                except
                  Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Fixing Model Inconsistency!' );
                end;
      ...
      begin
        ...
        try
          ...
          for i := 0 to pred(OutputCount) do
            begin
              Output := Outputs[i];
              if (Output <> nil) and (Output.MetaOutput = nil)
                then
                  begin
    ...
    begin
      ...
      for i := 0 to pred(OutputCount) do
        Outputs[i].Loaded;
      AutoConnect( true );
      fUpgradeLevel := max(1, fUpgradeLevel);
    end;
  ...
  procedure TBlock.Deleted;
    ...
    begin
      try
        ...
        Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error in block deleted.. ');
      end;
      inherited;
    end;
  ...
  procedure TBlock.RDOSetCompanyInputDemand(index, perc : integer);
    begin
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' - Fac(' + IntToStr(XPos) + ',' + IntToStr(YPos) + ') ' + 'SetCompanyInputDemand' );
      try
        if (index >= 0) and (index < CompanyInputCount)
          then
            begin
              CompanyInputs[index].Max := realmin(1, UpgradeLevel*(perc/100))*TMetaCompanyInput(MetaBlock.CompanyInputs[index]).Max;
              CacheObject(Facility, noKind, noInfo);
            end;
      except
        Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error in SetCompanyInputDemand' );
      end;
    end;
  ...
  procedure TBlock.SetTradeLevel(aTradeLevel : TTradeLevel);
    var
      i : integer;
    begin
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' - Fac(' + IntToStr(XPos) + ',' + IntToStr(YPos) + ') ' + 'SetTradeLevel' );
      try
        ...
        Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error in SetTradeLevel..' );
      end;
    end;

  procedure TBlock.RDOSetTradeLevel(aTradeLevel : integer);
    begin
      SetTradeLevel(TTradeLevel(aTradeLevel));
    end;
  ...
  procedure TBlock.AdvanceUpgrading;
    ...
    begin
      try
        if not Facility.CriticalTrouble and (fUpgradeHours > 0) and (Facility.Company <> nil) and not Facility.Company.Owner.ReachedBuildLimit
          then
            begin
              ...
              upgCost := delta*Facility.MetaFacility.UpgradeHourCost;
        ...
        Logs.Log( 'Survival', DateTimeToStr(Now) + ' Error in AdvanceUpgrading..');
      end;
    end;

  procedure TBlock.StartUpgrading;
    var
      upgCost : TMoney;
    begin
      if (fUpgradeHours = 0) and (fUpgradeLevel < MetaBlock.MaxUpgrade)
        then
          begin
            fUpgradeHours := 1;
            upgCost := Facility.MetaFacility.UpgradeHourCost;
            Facility.fCost := Facility.fCost + upgCost;
            Facility.Company.GenMoney(-upgCost, accIdx_Construction);
            ModelServerCache.UpdateObjectCache(Facility, noKind, noInfo);
          end;
    ...
    begin
      try
        if (fUpgradeHours > 0) and (fUpgradeHours <= Facility.MetaFacility.UpgradeTime)
          then
            begin
              upgCost := fUpgradeHours*Facility.MetaFacility.UpgradeHourCost;
              Facility.fCost := Facility.fCost - upgCost;
        ...
        then
          begin
            ...
            if Tycoon <> nil
              then
                begin
                  upgCost := realmax(0, Facility.MetaFacility.UpgradeCost);
                  Facility.fCost := Facility.fCost - upgCost;
                  Tycoon.CountFacility(Facility, false);
                  dec(fUpgradeLevel);
                  Tycoon.CountFacility(Facility, true);
            ...
            then level := cprSameOwner
              ...
              begin
                ...
                if priority <> cprNewbieA
                  ...
                  else level := cprNewbieA;
              end
        else level := cprNormal;
      result := PriceRange*level;
      {
      if (Block.Facility <> nil) and
         (Block.Facility.MetaFacility <> nil) and
         (Block.Facility.MetaFacility.Kind <> nil) and
         (Block.Facility.MetaFacility.Kind.Role = rolDistributer) and
         (Connection.Block.Facility <> nil) and
         (Connection.Block.Facility.MetaFacility <> nil) and
         (Connection.Block.Facility.MetaFacility.Kind <> nil) and
         (Connection.Block.Facility.MetaFacility.Kind.Role = rolDistributer)
        then
          begin
            result := result + PriceRange*(cprMonopoly + 1);
          end;
      }
    end;
        ...
        if (Block.Facility.MetaFacility <> nil) and
          ...
          then
            begin
              ...
              while fConnections.Count > limit do
                begin
  ...
  function TGate.ConnectionAllowedByPolicy( Gate : TGate ) : boolean;
    const
      ...
      //    SO,    Pup,   Ally,  Any      SO,    Pup,   Ally,  Any      SO,    Pup,   Ally,  Any
        ( ((false, false, true,  true),  (false, false, false, true),  (false, false, false, false)),  // Ally
          ((false, false, true,  true),  (false, false, false, true),  (false, false, false, false)),  // Neutral
      ...
      if (Block.Facility.Company <> nil) and (Block.Facility.Company.Owner <> nil) and
        ...
        then
          begin
            ...
            if Owner = GateOwner
              then result := true
              else result := CnxPolicyTable[Owner.Policy[GateOwner], GateOwner.Policy[Owner], Block.TradeLevel]
          end
        else result := true;
    end;
  ...
  procedure TInput.InsertConnection( Connection : TGate );
    begin
      if MetaInstances.ObjectIs( TOutput.ClassName, Connection )
        ...
        else Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' ERROR Attempting to connect ' + ClassName + ' to ' + Connection.ClassName );
    end;
  ...
  procedure TInput.RDOSetInputFluidPerc(Perc : integer);
    begin
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' - Fac(' + IntToStr(Block.Facility.XPos) + ',' + IntToStr(Block.Facility.YPos) + ') ' + 'Setting Input fluid perc: ' + IntToStr(Perc) );
      ...
      for i := pred(fConnections.Count) downto 0 do
        begin
          ...
          if ObjectIs('TInput', Input)
            ...
            else Logs.Log( 'Survival', DateTimeToStr(Now) + ' Error in SetPricePerc, unknow object as TInput.' );
        end;
    end;
      ...
      if index <> NoIndex
        then
          try
            ...
            result := nil;
            Logs.Log( 'Survival', DateTimeToStr(Now) + ' Error in GetExtraConnectionInfo' );
          end
        else result := nil;
    end;
                ...
                if diff > LazyCnxThreshold
                  then
                    begin
                      ...
                      if (Input <> nil) and not (Input.Block.Role in [rolDistributer, rolCompExport, rolCompInport]) and (Comp <> nil) and (Comp.Owner <> Owner)
          ...
          begin
            ...
            for i := pred(ConnectionCount) downto MetaOutput.MetaFluid.CnxLimit do
              begin
                Input := TInput(Connections[i]);
                Comp := Input.Block.Facility.Company;
                if (Input <> nil) and (Input.Block.Role = rolBuyer) and
                   (Comp <> nil) and (Comp.Owner <> Owner) and
                   (Comp.Owner <> nil) and (Comp.Owner.Level.Tier > 1)
                  then DisconnectFrom(Input);                                    
              end;
          end;
        ...
        begin
          ...
          with TSlice(fSlices[i]) do
            if not taken and not Input.Block.Facility.CriticalTrouble and ExtraConnectionInfo.Connected and TPullInput(Input).TransferAllowed( self )
              then
                begin
                  if (Block.Role = rolDistributer) and (Input.Block.Role = rolDistributer)
                    then cap := (Input.Capacity - Input.FluidData.Q)/1000 // >>!
                    else cap := Input.Capacity - Input.FluidData.Q;       // >>!
                  if val + Value > cap
                    then v := cap - val
                    else v := Value;
                  Value := Value - v;
      ...
      if Idx = NoIndex
        then
          if ConnectTo( Input ) = cnxValid
            then
              begin
                Logs.Log( 'Survival', DateTimeToStr(Now) + ' Error in GetSliceFor, Unknown Input found x:' + IntToStr(Input.Block.xPos) + ' y:' + IntToStr(Input.Block.yPos));
    ...
    begin
      ...
      for i := 0 to pred(fConnections.Count) do
        begin
          ...
          Writer.WriteSingle( aux, TSlice(fSlices[i]).ExtraConnectionInfo.YearValue );
          aux := 'YearInfo' + IntToStr(i) + '.Cost';
          Writer.WriteCurrency( aux, TSlice(fSlices[i]).ExtraConnectionInfo.YearCost );
      ...
      if fSlices.Count <> fConnections.Count
        then
          begin
            ...
            Logs.Log(tidLog_Survival, DateTimeToStr(Now) + ' Connections and Slices differ in facility: (' + IntToStr(Block.xPos) + ',' + IntToStr(Block.yPos) + ')');
          end;
    end;
        ...
        then
          try
            ...
            if Account <> nil
              ...
              else Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Account not found: ' + IntToStr(Reason) );
              //raise Exception( 'Invalid account Id: ' + IntToStr(Reason) );
          except
            Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' error in GenMoney: ' + IntToStr(Reason) );
          end
        else Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' GenMoney out of bounds: ' + IntToStr(Reason) );
    end;
...

{
      ...
      if Owner <> nil
        then
          begin
            ...
            if Facility.MetaFacility.TypicalStage.MetaBlock.Transcends
  ...
  procedure TCompany.DeclareInvention(Invention : TInvention);
    ...
    begin
        ...
        if not fInventionSet.Included(Invention.NumId)
          then
            begin
              ...
              // Add the invention
              fInventionSet.Include(Invention.NumId);
              fInventions.Insert(TInventionRecord.Create(Invention, LicFee + Invention.Price));
              // Send notification
              if (Owner <> nil)
                then
                  if Invention.EnablesTech
                    then Owner.SendNotification( ntkGenericEvent, '', Format( mtidMsgMainResearchCompleted.Values[lang], [Invention.Name] ), gevnId_RefreshBuildPage )
                    else Owner.SendNotification( ntkChatMessage, '', Format( mtidMsgResearchCompleted.Values[lang], [Invention.Name] ), 0 );
      ...
      if added
        then ModelServerCache.CacheObject(self, noKind, noInfo);
    end;
      ...
      if Rec <> nil
        then
          try
            ...
            try
              ModelServerCache.CacheObject( self, noKind, noInfo );
              if buidAffected > 0
              ...
              Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error in TCompany.RetireInvention Notifications name: ' + Name );
            end;
            result := true;
          except
            Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error in TCompany.RetireInvention name: ' + Name );
            result := false;
          end
        else result := false;
    end;
  ...
  function TTycoon.GetAreaTax : TMoney;
    begin
      if (Level <> nil) and not IsRole
        then result := Level.Tier*fArea*LandSquareCost
  ...
  procedure TTycoon.Sleep;
    ...
    begin
      ...
      if fTimesAwaken = 0
        then
          begin
            Cookie[tidCookie_LastTimeOnline] := DateToStr( Now );
            DecodeDate( Timer.GetVirtualTime, lastYear, useless, useless );
            //Cookie[tidCookie_LastVirtTimeOnline] := IntToStr( lastYear );
          end;
    end;
        ...
        for i := pred(fEvents.Count) downto 0 do
          begin
            try
              ...
              del := false;
              fEvents.AtExtract( i );
              Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error in TTycoon.RecordEvent..' );
            end;
            if del
    ...
    begin
      if Roles.IndexOf( Role ) = NoIndex
        then                         
          begin
            UncacheObject( self, noKind, noInfo );
            UncacheObject( Role, noKind, noInfo );
            Roles.Insert( Role );
            Role.SuperRole := self;
            Role.Password  := Password;
            CacheObject( self, noKind, noInfo );
            CacheObject( Role, noKind, noInfo );
          end;
    end;
  ...
  function TTycoon.RDOAskLoan( AmountStr : widestring ) : OleVariant;
    var
      Amount : TMoney;
    begin
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' AskLoan: ' + Name + ', $' + AmountStr );
      Lock;
      try
  ...
  function TTycoon.RDOSendMoney( ToTycoon, Reason, AmountStr : widestring ) : OleVariant;
    var
      ...
      i      : integer;
    begin
      Logs.Log( 'Money', DateTimeToStr(Now) + ' Sending money: ' + Name + ' to ' + ToTycoon + ', $' + AmountStr );
      try
        Amount := StrToCurr( AmountStr );
        if (Amount > 0) and not IsRole
          then
            begin
              Dest := WorldLocator.GetTycoonByName( ToTycoon );
              if (Dest <> nil) and (Dest.CountItemsInCurriculum( crrKind_Transcended ) = 0)
                then
                  begin
                    Loan := 0;
                    Loans.Lock;
                    try
                      for i := 0 to pred(Loans.Count) do
                        Loan := Loan + TLoan(Loans[i]).Amount;
                    finally
                      Loans.Unlock;
                    end;
                    Lock;
                    try
                      if Amount > fBudget - Loan - InitialBudget
                        then Amount := realmax( 0, fBudget - Loan - InitialBudget );
                    ...
                    try
                      ...
                      CacheObject( Dest, noKind, noInfo );
                    finally
                      Dest.Unlock
                    end;
                    result := NOERROR;
  ...
  procedure TTycoon.RDOAddAutoConnection( FluidId, Suppliers : widestring );
    begin
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Adding initial suppliers: ' + Name + ', ' + FluidId + ', ' + Suppliers );
      try
        ModifyAutoConnection( FluidId, Suppliers, true );
      except
        Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error Adding initial suppliers..');
      end;
    end;

  procedure TTycoon.RDODelAutoConnection( FluidId, Suppliers : widestring );
    begin
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Deleting initial suppliers: ' + Name + ', ' + FluidId + ', ' + Suppliers );
      try
        ModifyAutoConnection( FluidId, Suppliers, false );
      except
        Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error deleting initial suppliers..');
      end;
    end;
  ...
  procedure TTycoon.RDOHireTradeCenter( FluidId : widestring );
    var
      AutoConnection : TAutoConnection;
    begin
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Initial suppliers, include Trade Center: ' + Name + ', ' + FluidId );
      try
        AutoConnection := FindAutoConnection( FluidId );
        if AutoConnection <> nil
          then
        ...
        Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error hiring TradeCenter..');
      end;
    end;
  ...
  procedure TTycoon.RDODontHireTradeCenter( FluidId : widestring );
    var
      AutoConnection : TAutoConnection;
    begin
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Initial suppliers, excluding Trade Center: ' + Name + ', ' + FluidId );
      try
        AutoConnection := FindAutoConnection( FluidId );
        if AutoConnection <> nil
          then
        ...
        Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error excluding TradeCenter..');
      end;
    end;
  ...
  procedure TTycoon.RDOHireOnlyFromWarehouse( FluidId : widestring );
    var
      AutoConnection : TAutoConnection;
    begin
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Initial suppliers, hire only warehouses: ' + Name + ', ' + FluidId );
      try
        AutoConnection := FindAutoConnection( FluidId );
        if AutoConnection <> nil
          then
        ...
        Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error in hire only warehouses..');
      end;
    end;
  ...
  procedure TTycoon.RDODontHireOnlyFromWarehouse( FluidId : widestring );
    var
      AutoConnection : TAutoConnection;
    begin
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Initial suppliers, hire all: ' + Name + ', ' + FluidId );
      try
        AutoConnection := FindAutoConnection( FluidId );
        if AutoConnection <> nil
          then
        ...
        Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Initial suppliers, hire all OK! ' );
      except
        Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error in RDODontHireOnlyFromWarehouse');
      end;
    end;
  ...
  procedure TTycoon.RDOSetPolicyStatus( ToTycoon : widestring; Status : integer );
    var
      ...
      Msg    : TRegMultiString;
    begin
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Setting policy status: ' + Name + ', ' + ToTycoon + ', ' + IntToStr(Status) );
      try
        ...
        Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Setting policy status OK!' );
      except
        Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error in Setting policy status' );
      end;
    end;

  procedure TTycoon.RDOSetAdvanceToNextLevel( yes : integer );
    begin
      Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Setting AdvanceToNextLevel: ' + Name + ', ' + IntToStr(yes) );
      try
        ...
        WorldLocator.SendEvent(
          TEvent.Create(
            ...
            '', '' ));
        Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Setting AdvanceToNextLevel OK!' );
      except
        Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error in AdvanceToNextLevel' );
      end;
    end;

  procedure TTycoon.RDODelCurItem(index : integer);
    begin
      Logs.Log(tidLog_Survival, DateTimeToStr(Now) + ' Deleting Curriculum Item: ' + Name);
      try
        fCurriculum.Lock;
        try
          if index < fCurriculum.Count
            then fCurriculum.AtDelete(index);
        finally
          fCurriculum.Unlock;
        end;
        Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Deleting Curriculum Item OK!' );
      except
        Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error in Delete Curriculum Item' );
      end;
    end;
    ...
    procedure Compensate;
      ...
      begin
        if Level <> nil
          then
            begin
              resVal := Accounts.AccountArray[accIdx_ResearchCenter_Research].Value;
                  ...
                  begin
                    ...
                    for iInv := 0 to pred(Company.Inventions.Count) do
                      with TInventionRecord(Company.Inventions[iInv]) do
                        Subsidy := Level.PercOfResearchSubs*TotalCost
                  end;
              finally
                Companies.Unlock;
              end;
    ...
    procedure CheckForNextLevel;
      ...
      begin
        if AdvanceToNextLevel                
          then
            begin
              if (fLevel.NextLevel <> nil) and
                 (fLevel.Tier < TheGlobalConfigHandler.GetInteger( CFGID_LevelLimit )) and
                 fLevel.NextLevel.AdvanceTycoon( self, fLevelReqStatus )
                then
                  begin
          ...
          else fLevelReqStatus := nil;
        //UpdateObjectCache( self, noKind, noInfo );
      end;

    procedure PayExtraTaxes;
      const
        PercPerValue   = 5000000;
        MinPerc        = 50;
      var
        MA     : TMetaAccount;
      ...
      begin
        ...
        try
          ...
          Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error in PayExtraTaxes' );
        end;
      end;

    procedure PublishCurriculum;
      var
        ...
        try
          for i := 0 to pred(fCurriculum.Count) do
            try
              ...
              if not Item.Published and Item.IsGlobal
          ...
          begin
            ...
            try
              ...
              try
                ...
                Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error unregistering supplier ' + Facility.Name + ' to tycoon ' + fName );
              except
              end;
            end;
            dec(i);
  ...
  procedure TTycoon.TycoonDeleted( Tycoon : TTycoon );
    ...
    begin
      try
        ...
        Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error excluding Tycoon Deleted..');
      end;
    end;
  ...
  procedure TTycoon.LoadFromBackup( Reader : IBackupReader );
    ...
    begin
      ...
      // Tutorial
      Reader.ReadObject( 'TaskContext', Obj, nil );
      if Obj <> nil
        then Obj.Free;
      Reader.ReadObject( 'Tutorial', Obj, nil );
      if Obj <> nil
        then Obj.Free;
      //if fTutorial = nil
                      ...
                      for k := 0 to pred(Facilities.Count) do
                        with TFacility(Facilities[k]) do
                          try
                            ...
                            on E : Exception do
                              Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Error in UpdateParameter.. ' + E.Message);
                          end;
                    finally
                      Facilities.Unlock;
                    end;
  ...
  procedure TTycoon.UpdateRankings;
    ...
    begin
      ...
      try
        sum   := 0;
        count := 0;
        for i := 1 to pred(fRankings.Count) do
  ...
  procedure TTycoon.RDOActivateTutorial(Value : LongBool);
    begin
      //Logs.Log( tidLog_Survival, DateTimeToStr(Now) + ' Activating tutorial: ' + Name );
      //if fTutorial <> nil
  ...
  procedure TTycoon.CountFacility(Facility : TFacility; created : boolean);
    begin
      if created
        then
          begin
            inc( fFacilityCount, Facility.UpgradeLevel );
            inc( fArea, Facility.MetaFacility.xSize*Facility.MetaFacility.ySize );
          end
        else
          begin
            dec( fFacilityCount, Facility.UpgradeLevel );
