...
implementation
  ...
  function TWorldProxy.GetCache(Id, kind, info : integer) : string;
    var
      ObjId : string;
      exp   : TDateTime;
    begin
              ...
              begin
                ...
                try
                  ...
                  if result = resOK
                    then
                      begin
                        exp := CreateExpireDate(ExpireTTL);
                        fHistory.AddRecord(ObjId, exp);
                      end;
                finally
                  fHistory.Unlock
                end;
  ...
  function TWorldProxy.RenewCache(Agent, ObjId : string) : string;
    var
      exp : TDateTime;
    begin
      try
              ...
              if ClientConn.Connect(ConnectionTime)
                then
                  begin
                    ...
                    Proxy.BindTo(MSObjectCacherName);
