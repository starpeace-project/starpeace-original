...
interface
  ...
  type
    ...
    TMailMessage =
      class
        public
          constructor Create(aFrom, aTo, aSubject : string);
...
implementation
  ...
  function TInterfaceServerData.RegisterEvents(ClientAddress : widestring; ClientPort : integer) : OleVariant;
    begin
      try
        ...
        result := false;
        {IFDEF Logs}
        LogFile.LogThis(DateTimeToStr(Now) + ': Error registering event of Addr: ' + ClientAddress + ' Port: ' + IntToStr(ClientPort));
        {ENDIF}
      end;
    end;
  ...
  function TInterfaceServerData.LogOff : OleVariant;
    begin
      try
        ...
        {IFDEF Logs}
        LogFile.LogThis(DateTimeToStr(Now) + ': Error while logging off Interface Server.');
        {ENDIF}
      end;
    end;
  ...
  function TWorldData.SetDAConnection(Address : widestring; Port : integer) : OleVariant;
    begin
      try
        ...
        result := false;
        {IFDEF Logs}
        LogFile.LogThis(DateTimeToStr(Now) + ': Error setting direct access connection with the ModelServer.');
        {ENDIF}
      end;
    end;
  ...
  function TMailServer.DoSetForwardRule(const AccPath, FwdAddr : string; KeepMsg : boolean) : boolean;
    ...
    begin
      try
        ...
        result := false;
        {IFDEF Logs}
        LogFile.LogThis(DateTimeToStr(Now) + ': Error setting the forward rule AccPath=' + AccPath + 'and FwdAddr=' + FwdAddr);
        {ENDIF}
      end;
    end;
  ...
  function TMailServer.RegisterWorld(WorldName : widestring) : OleVariant;
    ...
    begin
      try
        ...
        result := integer(nil);
        {IFDEF Logs}
        LogFile.LogThis(DateTimeToStr(Now) + ': Error registering world ' + WorldName);
        {ENDIF}
      end;
    end;
  ...
  function TMailServer.LogServerOn(WorldName : widestring) : OleVariant;
    ...
    begin
      try
        ...
        result := integer(nil);
        {IFDEF Logs}
        LogFile.LogThis(DateTimeToStr(Now) + ': Error logging on Interface Server to ' + WorldName);
        {ENDIF}
      end;
    end;
  ...
  function TMailServer.LogServerOff(Id : integer) : OleVariant;
    begin
      try
        ...
        result := false;
        {IFDEF Logs}
        LogFile.LogThis(DateTimeToStr(Now) + ': Error logging off Interface Server.');
        {ENDIF}
      end;
    end;
  ...
  function TMailServer.NewMailAccount(ServerId : integer; Account, Alias, FwdAddr : widestring; KeepMsg : WordBool) : OleVariant;
    ...
    begin
      try
        ...
        {IFDEF Logs}
        LogFile.LogThis(DateTimeToStr(Now) + ': Error creating mail account ' + Account);
        {ENDIF}
      end;
    end;
  ...
  function TMailServer.DeleteAccount(ServerId : integer; Account : widestring) : OleVariant;
    ...
    begin
      try
        ...
        {IFDEF Logs}
        LogFile.LogThis(DateTimeToStr(Now) + ': Error deleting mail account ' + Account);
        {ENDIF}
      end;
    end;
  ...
  function TMailServer.CheckNewMail(ServerId : integer; Account : widestring) : OleVariant;
    ...
    begin
      try
        ...
        result := -1;
        {IFDEF Logs}
        LogFile.LogThis(DateTimeToStr(Now) + ': Error checking for new mail of account ' + Account);
        {ENDIF}
      end;
    end;
  ...
  function TMailServer.NewMail(aFrom, aTo, aSubject : widestring) : OleVariant;
    ...
    begin
      try
        Msg := TMailMessage.Create(aFrom, aTo, aSubject);
        fMessages.Insert(Msg);
        result := integer(Msg);
      except
        result := 0;
        {IFDEF Logs}
        LogFile.LogThis(DateTimeToStr(Now) + ': Error creating new mail message From=' + aFrom + ' To=' + aTo);
        {ENDIF}
      end;
    end;
  ...
  function TMailServer.SendMailTo(Msg : TMailMessage; WorldName : string; List : TStringList) : boolean;
    ...
    begin
      ...
      try
        ...
        result := posted;
        {IFDEF Logs}
        LogFile.LogThis(DateTimeToStr(Now) + ': Error sending mail message to ' + ^M^J + List.Text);
        {ENDIF}
      end;
    end;
  ...
  procedure TMailServer.ReportMail(const World, Account, From, Subject, MsgId : string);
    ...
    begin
      try
              ...
              try
                for i := 0 to pred(WData.fIntServers.Count) do
                  try
                    ...
                    {IFDEF Logs}
                    LogFile.LogThis(DateTimeToStr(Now) + ': Error, Interface Server Proxy fail reporting new mail.');
                    {ENDIF}
                  end;
              finally
        ...
        {IFDEF Logs}
        LogFile.LogThis(DateTimeToStr(Now) + ': Error reporting mail to account ' + Account);
        {ENDIF}
      end;
    end;
  ...
  procedure TMailServer.PostMailIn(Path, Folder : string; Msg : TMailMessage; Execute : boolean);
    begin
      try
        ...
        {IFDEF Logs}
        LogFile.LogThis(DateTimeToStr(Now) + ': Error posting mail in folder ' + Folder);
        {ENDIF}
      end;
    end;
  ...
  function TMailServer.Post(WorldName : widestring; Id : integer) : OleVariant;
    ...
    begin
      try
        ...
        result := false;
        {IFDEF Logs}
        LogFile.LogThis(DateTimeToStr(Now) + ': Error posting message in ' + WorldName);
        {ENDIF}
      end;
    end;
  ...
  function TMailServer.Save(WorldName : widestring; Id : integer) : OleVariant;
    ...
    begin
      try
        ...
        result := false;
        {IFDEF Logs}
        LogFile.LogThis(DateTimeToStr(Now) + ': Error saving message in ' + WorldName);
        {ENDIF}
      end;
    end;
  ...
  procedure TMailServer.CloseMessage(Id : integer);
    begin
      try
        ...
        {IFDEF Logs}
        LogFile.LogThis(DateTimeToStr(Now) + ': Error deleting message.');
        {ENDIF}
      end;
    end;
  ...
  function TMailServer.OpenMessage(WorldName, Account, Folder, MessageId : widestring) : OleVariant;
    ...
    begin
      ...
      try
        ...
        result := 0;
        {IFDEF Logs}
        LogFile.LogThis(DateTimeToStr(Now) + ': Error opening message ' + MessageId);
        {ENDIF}
      end;
    end;
  ...
  procedure TMailServer.DeleteMessage(WorldName, Account, Folder, MessageId : widestring);
    ...
    begin
      ...
      try
        ...
        {IFDEF Logs}
        LogFile.LogThis(DateTimeToStr(Now) + ': Error deleting message ' + MessageId);
        {ENDIF}
      end;
    end;
  ...
  constructor TMailMessage.Create(aFrom, aTo, aSubject : string);
    begin
      inherited Create;
