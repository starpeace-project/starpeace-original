...
implementation
  ...
  uses
    Windows, SysUtils, TypInfo, RDOInterfaces, ErrorCodes, Logs;
      ...
      try
        ...
        on E : Exception do
          begin
            ...
            Logs.Log('Survival', DateTimeToStr(Now) + 'Error at: TRDOObjectServer.GetProperty (126) ' + E.Message);
          end;
      end
    end;
    ...
    begin
      ...
      try
                    ...
                    try
                      ...
                      on E : Exception do
                        begin
                          ...
                          Logs.Log('Survival', DateTimeToStr(Now) + 'Error at: TRDOObjectServer.SetProperty (174) ' + E.Message);
                        end;
                    end
                  else
                    ErrorCode := errUnexistentProperty
        ...
        on E : Exception do
          begin
            ErrorCode := errIllegalObject;
            Logs.Log('Survival', DateTimeToStr(Now) + 'Error at: TRDOObjectServer.SetProperty (186) ' + E.Message);
          end;
      end
    end;
    ...
    @DoCall:          mov  eax, theObject
                  ...
                  on E : Exception do
                    begin
                      ...
                      Logs.Log('Survival', DateTimeToStr(Now) + ' Error at: TRDOObjectServer.CallMethod "' + MethodName + '" (319) ' + E.Message);
                    end;
                end
              else
            ...
            on E : exception do
              begin
                QueryStatus := 524;
                ErrorCode := errIllegalObject;
                Logs.Log('Survival', DateTimeToStr(Now) + ' Error at: TRDOObjectServer.CallMethod "' + MethodName + '" (326) ' + E.Message);
              end;
          end;
    end;
