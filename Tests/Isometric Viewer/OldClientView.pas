...
implementation
  ...
  constructor TTestMasterUrlHandler.Create;
    begin
      ...
      fRoadSegs.LoadFromFile(getWorldURL + '\roads.' + getWorldName + '.dat');
      fRailroadSegs := TStringList.Create;
      fRailroadSegs.LoadFromFile(getWorldURL + '\railroads.' + getWorldName + '.dat');
    end;
      ...
      if (RawSegments <> nil) and (RawSegments.Count <> 0)
        then
          with SegmentReport do
            begin
              getmem(Segments, RawSegments.Count*sizeof(Segments[0]));
              SegmentCount := 0;
              for SegmentIdx := 0 to pred(RawSegments.Count) do
                begin
                  segment := RawSegments[SegmentIdx];
