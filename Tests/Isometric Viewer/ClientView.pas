...
interface
  ...
  uses
    ...
    VoyagerServerInterfaces, Matrix, GMKernel;
...
implementation
  ...
  constructor TTestMasterUrlHandler.Create;
    ...
    begin
      ...
      try
        ...
        rawsegs.LoadFromFile(getWorldURL + '\railroads.' + getWorldName + '.dat');
        if rawsegs.count > 0
          then
            with fRailroadSegs do
              begin
                getmem(Segments, rawsegs.Count*sizeof(Segments[0]));
                SegmentCount := rawsegs.Count;
                for i := 0 to pred(rawsegs.Count) do
    ...
    begin
      ...
      case CircuitId of
        cirRoads:
          begin
            Segments := fRoadSegs;
            FileName := getWorldURL + '\roads.' + getWorldName + '.dat';
          end;
        cirRailroads:
          begin
            Segments := fRailroadSegs;
            FileName := getWorldURL + '\railroads.' + getWorldName + '.dat';
          end;
      end;
      ...
      case CircuitId of
        cirRoads:
          fRoadSegs := Segments;
        cirRailroads:
          fRailroadSegs := Segments;
      end;
      RawSegments := TStringList.Create;
      try
        for i := 0 to pred(Segments.SegmentCount) do
          with Segments.Segments[i] do
            RawSegments.Add(IntToStr(x1) + ' ' + IntToStr(y1) + ' ' + IntToStr(x2) + ' ' + IntToStr(y2));
        RawSegments.SaveToFile(FileName);
      finally
        RawSegments.Free;
      end;
    end;
  ...
  function TTestMasterUrlHandler.PickEvent : TStringList;
    begin
      Result := nil;
    end;
  ...
  function TTestMasterUrlHandler.getUserMasterName : string;
    begin
      Result := '';
    end;

  function TTestMasterUrlHandler.getUserPassword : string;
    begin
      Result := '';
    end;
  ...
  function TTestMasterUrlHandler.ConnectToGameMaster( ClientId : TCustomerId; ClientInfo : widestring; GameMasters : widestring ) : OleVariant;
    begin
      Result := NO_ERROR;
    end;

  function TTestMasterUrlHandler.SendGMMessage( ClientId : TCustomerId; GMId : TGameMasterId; Msg : WideString ) : OleVariant;
    begin
      Result := NO_ERROR;
    end;

  procedure TTestMasterUrlHandler.DisconnectUser( ClientId : TCustomerId; GMId : TGameMasterId );
    begin
    end;
  ...
  procedure TTestMasterUrlHandler.CreateBuildings;
    ...
    begin
      ...
      try
        ...
        buildings.LoadFromFile(getWorldURL + '\buildings.' + getWorldName + '.dat');
        for k := 0 to pred(buildings.Count) do
          begin
            building := buildings[k];
            i := StrToInt(copy(building, 1, pred(Pos(' ', building))));
            delete(building, 1, Pos(' ', building));
            j := StrToInt(copy(building, 1, pred(Pos(' ', building))));
            delete(building, 1, Pos(' ', building));
            vclass := StrToInt(copy(building, 1, pred(Pos(' ', building))));
            delete(building, 1, Pos(' ', building));
            bclass := fBuildClasses.Get(vclass);
            if (bclass <> nil) and (bclass.ImagePath <> '')
              then fBuildings[i, j] := bclass.id;
          end;
      finally
        buildings.Free;
      end;
