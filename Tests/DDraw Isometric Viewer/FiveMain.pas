...
implementation
  ...
  uses
    ...
    DDrawMgr, BufferManager, LogFile, SelectVideoMode, IsoProfile;
  ...
  procedure TFiveMainForm.FormCreate(Sender: TObject);
    ...
    begin
      {$IFDEF GDI}
      DirectDrawMgr.SetCooperativeLevel(clNormal);
      {$ELSE}
      DirectDrawMgr.SetCooperativeLevel(clFullScreen);
      {$ENDIF}
      ...
      {$IFDEF GDI}
      BufferMgr := TBufferManager.Create(bschGDI, VideoMode^);
      {$ELSE}
      {$IFDEF OFFSCREEN}
      BufferMgr := TBufferManager.Create(bschOffScreen, VideoMode^);
      {$ELSE}
      BufferMgr := TBufferManager.Create(bschFlipChain, VideoMode^);
      {$ENDIF}
      {$ENDIF}
      if BufferMgr.IsUsingDDraw
        then
          begin
            fFSWindowManager := TFullScreenWindowManager.Create(Handle, DirectDrawMgr.DirectDraw, BufferMgr.FrontBuffer, BufferMgr.BackBuffer);
            BufferMgr.SetFSWindowManager(fFSWindowManager);
          end;
      if Debugging
  ...
  procedure TFiveMainForm.FormShow(Sender: TObject);
    ...
    begin
      ...
      if BufferMgr.IsUsingDDraw
        then SetMainPanelDefaultRegion;
