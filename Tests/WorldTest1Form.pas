...
uses
  ...
  ComCtrls, Population, ToolWin, ExtCtrls, World, Menus, Buttons, StdCtrls, Surfaces;
...
implementation
  ...
  uses
    ClassStorage, PopulatedBlock, WorkCenterBlock, Trade,
  ...
  procedure TWorldTestForm.FormCreate(Sender: TObject);
    ...
    begin
      ...
      Construction.RegisterMetaFluids;
      Kernel.RegisterSurfaces;
      Population.RegisterSurfaces;
      TCluster.Create( 'Microxoft' ).Register( 'Clusters' );
      ...
      RegisterTownHall( 'Microxoft', 'TownHall', 12, 3, 3, 20 );
      RegisterTradeCenter( 'Microxoft', 'TradeCenter', 14, 2, 2 );

      GridSize := 25;
      fWorld := TInhabitedWorld.Create( 'Lola', 200, 200 );
      ...
      for i := 0 to trunc(sqrt(TownCount)) - 1 do
        for j := 0 to trunc(sqrt(TownCount)) - 1 do
          begin
            ...
            x := (fWorld.xSize div (trunc(sqrt(TownCount))))*succ(i) - (fWorld.xSize div (trunc(sqrt(TownCount)))) div 2 + (1 - 2*random(2))*random(0);
            y := (fWorld.ySize div (trunc(sqrt(TownCount))))*succ(j) - (fWorld.ySize div (trunc(sqrt(TownCount)))) div 2 + (1 - 2*random(2))*random(0);
      ...
      Tycoon := TTycoon.Create;
      Company := TCompany.Create( 1 );
      Company.Owner := Tycoon;
      ...
      for i := 0 to pred(TheClassStorage.ClassCount['Facilities']) do
        if mfcInTown in (TheClassStorage.ClassByIdx['Facilities', i] as TMetaFacility).Options
          then
            begin
      ...
      try
        ...
        for i := 0 to pred(List.Count div 4) do
          begin
            ...
            x         := StrToInt(List[4*i + 2]);
            y         := StrToInt(List[4*i + 3]);
