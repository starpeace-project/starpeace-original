    ...
    TOverlayMainForm =
      class(TForm)
        ...
        private
          ...
          procedure UpdateOverlayClipper;
        public
          { Public declarations }
      end;

  var
    OverlayMainForm: TOverlayMainForm;
...
implementation
  ...
  const
    cPixelFormats : array [0..pred(cMaxPixelFormats)] of TDDPixelFormat =
      (
        ...
        (dwSize : sizeof(TDDPixelFormat); dwFlags : DDPF_RGB; dwFourCC : 0; dwRGBBitCount : 24; dwRBitMask : $FF0000; dwGBitMask : $00FF00; dwBBitMask : $0000FF; dwRGBAlphaBitMask : 0), // 24-bit RGB 8:8:8
        (dwSize : sizeof(TDDPixelFormat); dwFlags : DDPF_RGB; dwFourCC : 0; dwRGBBitCount : 24; dwRBitMask : $FF0000; dwGBitMask : $00FF00; dwBBitMask : $0000FF; dwRGBAlphaBitMask : 0), // 32-bit RGB 8:8:8
        (dwSize : sizeof(TDDPixelFormat); dwFlags : DDPF_RGB; dwFourCC : 0; dwRGBBitCount : 24; dwRBitMask : $FF0000; dwGBitMask : $00FF00; dwBBitMask : $0000FF; dwRGBAlphaBitMask : 0) // 22-bit RGB 8:8:8
      );

  function CreateFlippingOverlay(const DirectDraw : IDirectDraw4; Control : TWinControl; out BackBuffer : IDirectDrawSurface4) : IDirectDrawSurface4;
    var
      hRet    : HRESULT;
        ...
        then
          if (DirectDraw.GetCaps(@devcaps, @HELCaps) = DD_OK) and ((devcaps.dwCaps and DDCAPS_OVERLAY) <> 0)
	        ...
	        ddsd.dwHeight := Control.ClientHeight;
                ...
                ddsd.ddpfPixelFormat := cPixelFormats[cFormatIdx];
  ...
  function CreateOverlay(const DirectDraw : IDirectDraw4; Control : TWinControl) : IDirectDrawSurface4;
    var
      hRet    : HRESULT;
      ddsd    : TDDSurfaceDesc2;
      devcaps : TDDCaps;
  ...
  function ResizeOverlay(const DirectDraw : IDirectDraw4; Control : TWinControl; const PrimarySurface : IDirectDrawSurface4; var Overlay : IDirectDrawSurface4) : boolean;
    begin
      ...
      Overlay := CreateOverlay(DirectDraw, Control);
      Result := Overlay <> nil;
    end;
  ...
  procedure TOverlayMainForm.FormShow(Sender: TObject);
    var
      ddsd    : TDDSurfaceDesc2;
      //ddscaps : TDDSCAPS2;
      hRet    : HRESULT;
  ...
  procedure TOverlayMainForm.FormPaint(Sender: TObject);

    function DoBlit(const Src, Dest : IDirectDrawSurface4; x, y : integer; Clip : boolean) : boolean;
      var
        BltEfx   : TDDBltFX;
        DestRect : TRect;
        SrcRect  : TRect;
        ...
        if Clip
          then
            begin
              ...
              IntersectRect(SrcRect, SrcRect, ClientRect);
            end;
        Result := Dest.Blt(@DestRect, Src, @SrcRect, DDBLT_WAIT, @BltEfx) = DD_OK;
      end;

    function DoFastBlit(const Src, Dest : IDirectDrawSurface4; x, y : integer; Clip : boolean) : boolean;
      var
        DestRect : TRect;
        SrcRect  : TRect;
      begin
        ...
        Result := Dest.BltFast(x, y, Src, @SrcRect, DDBLTFAST_WAIT) = DD_OK;
      end;

    var
      ScPt : TPoint;
    begin
      if (fBitmap <> nil) and ((fOverlay <> nil) or (fClipper <> nil))
        then
          begin
              ...
              then
                begin
                  if not fOverlayVisible
                    then fOverlayVisible := ShowOverlay(Self, fOverlay, fPrimarySurface);
                  FillSurface(fOverlay, RGB(0, 0, 0));
                  DoBlit(fBitmapSurface, fOverlay, 0, 0, true);
                end
              else DoBlit(fBitmapSurface, fPrimarySurface, ScPt.x, ScPt.y, false);
          end;
    end;

  procedure TOverlayMainForm.LoadClick(Sender: TObject);
        ...
        then
          begin
            ...
            try
              ...
              if (fOverlay = nil) and (fClipper = nil)
                then
                  begin
                    fOverlay := CreateOverlay(fDirectDraw, Self);
                    fClipper := CreateClipper(fDirectDraw, Self);
                    if fClipper <> nil
                      then
                        if fOverlay <> nil
                          then
                            begin
                              UpdateOverlayClipper; 
                              hRet := fOverlay.SetClipper(fClipper);
                              if hRet <> DD_OK
  ...
  procedure TOverlayMainForm.FormResize(Sender: TObject);
    begin
      if fOverlay <> nil
        then UpdateOverlayClipper;
    end;

  procedure TOverlayMainForm.WMMove(var Msg : TWMMove);
    begin
      inherited;
      if fOverlay <> nil
        then
          begin
            MoveOverlay(Self, fOverlay);
            UpdateOverlayClipper;
          end;
    end;

  procedure TOverlayMainForm.UpdateOverlayClipper;
    type
    ...
    var
      RgnData    : PRgnData;
      RegionData : PRegionData absolute RgnData;
      R          : TRect;
    begin
      getmem(RgnData, sizeof(TRegionData));
      if fClipper <> nil
        then
          begin
            R := Rect(0, 0, pred(fBitmap.Width), pred(fBitmap.Height));
            IntersectRect(R, R, ClientRect);
            RegionData.Header.dwSize := sizeof(RegionData.Header);
            RegionData.Header.iType := RDH_RECTANGLES;
            RegionData.Header.nCount := 1;
            RegionData.Header.nRgnSize := sizeof(RegionData.Rect);
            RegionData.Header.rcBound := R;
            RegionData.Rect := R;
            if fClipper.SetClipList(RgnData, 0) <> DD_OK
              then raise Exception.Create('SetClipList FAILED');
          end;
    end;

  procedure TOverlayMainForm.FormCreate(Sender: TObject);
    begin
      SetBounds(0, 0, 700, 500);
    end;

initialization
  SetLogFile('C:\Tmp\Overlay.log');
end.
