...
interface
  ...
  type
    ...
    TBackupObject =
      class(TInterfacedObject)
        public
          constructor Create(aStream : TStream; OwnsStream : boolean; Section : TCriticalSection; aPath : string);
    ...
    TBackupWriter =
      class(TBackupObject, IBackupWriter)
        public
          ...
          procedure WriteInt64(Name : string; Value : int64);         virtual; stdcall; abstract;
    ...
    TVerboseBackupWriter =
      class(TBackupWriter)
        ...
        public
          ...
          procedure WriteInt64(Name : string; Value : int64);         override;
    ...
    TBackupReader =
      class(TBackupObject, IBackupReader)
        public
          constructor Create(aStream : TStream; OwnsStream : boolean; aPath : string);
          destructor  Destroy; override;
        protected
          procedure   FixEntry(var Entry : TIndexEntry; anObj : TObject);
          procedure   DoneEntry(var Entry : TIndexEntry);
        private
        ...
        public
          ...
          function  ReadInt64(Name : string; DefVal : int64) : int64;                   virtual; stdcall; abstract;
    ...
    TVerboseBackupReader =
      class(TBackupReader)
        ...
        public
          ...
          function  ReadInt64(Name : string; DefVal : int64) : int64;                  override;
  ...
  function CreateBackup(Path : string; Verbose : boolean; Section : TCriticalSection; Header : boolean = true) : TBackupWriter;
  function OpenBackup(Path : string; aNotProc : TBackupReaderNotify) : TBackupReader;
  function OpenBackupFromPath(Path : string; aNotProc : TBackupReaderNotify; Verbose : boolean) : TBackupReader;
...
implementation
  ...
  constructor TBackupObject.Create(aStream : TStream; OwnsStream : boolean; Section : TCriticalSection; aPath : string);
    begin
      inherited Create;
      fStream := aStream;
  ...
  function TBackupObject.GetPath : string;
    begin
      result := fPath;
    end;
  ...
  procedure TVerboseBackupWriter.WriteInt64(Name : string; Value : int64);
    begin
      WriteLine(Name + EqualSign + IntToStr(Value));
    end;
  ...
  constructor TBackupReader.Create(aStream : TStream; OwnsStream : boolean; aPath : string);
    begin
      inherited Create(aStream, OwnsStream, nil, aPath);
      //fFixups := TObjectIndex.Create(bckDefaultIndexSize);
      fFixups := TObjectHash.Create;
      fSize   := aStream.Size;
    end;

  destructor TBackupReader.Destroy;
    begin
      fFixups.Iterate(DoneEntry);
      fFixups.Free;
      inherited;
    end;
  ...
  procedure TBackupReader.DoneEntry(var Entry : TIndexEntry);
    begin
      if not Entry.Flg
        then FixEntry(Entry, nil);
    end;
  ...
  procedure TBackupReader.DoReadObject(agInfo : TAgentInfo; var anObj : TObject; anObjId : integer; aClassName : string; Fixups : boolean);
    ...
    procedure UpdateFixups;
      var
        IdxEntry : PIndexEntry;
        AuxPtr   : PObject;
      begin
        IdxEntry := fFixups.Entries[anObjId];
        if IdxEntry = nil
          then
            ...
            begin
              ...
              while not IdxEntry.Flg and (AuxPtr <> nil) do
                begin
  ...
  function TBackupReader.GetFixups : TObject;
    begin
      result := fFixups;
    end;

  procedure TBackupReader.SetFixups(Obj : TObject);
    begin
      fFixups := TObjectHash(Obj);
    end;
  ...
  constructor TVerboseBackupReader.Create(aStream : TStream; aPath : string);
    begin
      inherited Create(aStream, false, aPath);
      try
        if Assigned(fNotify)
  ...
  function TVerboseBackupReader.ReadInt64(Name : string; DefVal : int64) : int64;
    var
      aux : string;
    begin
      aux := ReadProp(Name, '');
  ...
  constructor TBinaryBackupWriter.Create(aStream : TStream; OwnsStream : boolean; Section : TCriticalSection; aPath : string);
    begin
      inherited Create(aStream, OwnsStream, Section, aPath);
    end;
  ...
  procedure TBinaryBackupWriter.WriteInt64(Name : string; Value : int64);
    begin
      fStream.Write(Value, sizeof(Value));
    end;
  ...
  constructor TBinaryBackupReader.Create(aStream : TStream; OwnsStream : boolean; aPath : string);
    begin
      inherited Create(aStream, OwnsStream, aPath);
    end;
  ...
  function TBinaryBackupReader.ReadInt64(Name : string; DefVal : int64) : int64;
    begin
      fStream.Read(result, sizeof(result));
    end;
  ...
  function CreateBackup(Path : string; Verbose : boolean; Section : TCriticalSection; Header : boolean) : TBackupWriter;
    var
      Stream : TStream;
      aux    : string;
    begin
      ...
      if Verbose
        then
          begin
            ...
            result := TVerboseBackupWriter.Create(Stream, true, Section, Path);
          end
        else
          begin
            if Header
            ...
            result := TBinaryBackupWriter.Create(Stream, true, Section, Path);
          end;
    end;
      ...
      try
        ...
        if aux = 'VER'^M^J
          then result := TVerboseBackupReader.Create(Stream, Path)
            ...
            if aux = 'BIN'^M^J
              then result := TBinaryBackupReader.Create(Stream, true, Path)
  ...
  function OpenBackupFromPath(Path : string; aNotProc : TBackupReaderNotify; Verbose : boolean) : TBackupReader;
    ...
    begin
      ...
      try
        if Verbose
          then result := TVerboseBackupReader.Create(Stream, Path)
          else result := TBinaryBackupReader.Create(Stream, true, Path);
        result.Notify := aNotProc;
      except
        Stream.Free;
        raise;
