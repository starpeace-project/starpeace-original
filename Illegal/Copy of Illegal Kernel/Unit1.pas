...
type
  TForm1 = class(TForm)
    ...
    procedure eObjXChange(Sender: TObject);
    procedure eObjYChange(Sender: TObject);
...
procedure TForm1.StartButtonClick(Sender: TObject);
  ...
  begin
    ...
    Criminals.Text := IllSystem.RDOGetCriminalNames;
    CriminalToShow := TStringList.Create;
    for n := 1 to 8 do
      begin
        CriminalToShow.Text := IllSystem.RDOFindCriminal(widestring(Criminals.Values[IntToStr(n - 1)]));
...
procedure TForm1.btnLeaderImageClick(Sender: TObject);
  begin
    OpenPictureDialog1.InitialDir := 'E:\work\five\source\illegal\LeadersPicture\';
...
procedure TForm1.btnLeaderOkClick(Sender: TObject);
  ...
  begin
    ...
    leaderList.Text := IllSystem.RDOFindLeader(widestring(edLeaderName.Text));
    if leaderList.Values['Name'] <> ''
    then
      ...
      begin
        IllSystem.RDOCreateLeader(widestring(edLeaderName.Text), widestring(OpenPictureDialog1.FileName));
        cbLeaders.Items.Add(edLeaderName.Text);
...
procedure TForm1.btnCreateLeaderClick(Sender: TObject);
  begin
    edLeaderName.Text := '';
    Image1.Picture.LoadFromFile('E:\work\five\source\illegal\LeadersPicture\PictureBlank.bmp');
...
procedure TForm1.btnLeaderChosenClick(Sender: TObject);
  ...
  begin
    ...
    btnLeaderChosen.Visible := False;
    btnCreateNewTeam.Visible := True;
    btnDismissTeam.Visible := True;
    LeaderList := TStringList.Create;
    LeaderList.text := IllSystem.RDOFindLeader(widestring(cbLeaders.Text));
  ...
  begin
    ...
    then
      begin
        ...
        then
          image2.Picture.LoadFromFile('e:\work\five\source\illegal\LeadersPicture\NoPicture.bmp')
        else
...
procedure TForm1.btnChooseLeaderClick(Sender: TObject);
  begin
    ...
    btnCreateNewTeam.Visible := False;
  end;
  ...
  begin
    ...
    then
      begin
        ...
        criminal.Text := IllSystem.RDOFindCriminal(widestring(CriminalLabels[SenderIndex].Caption));
...
procedure TForm1.btnTeamClick(Sender: TObject);
  var
    j       : integer;
  begin
    if IllSystem.RDOCreateTeam(widestring(cbLeaders.Text), widestring(eTeamName.Text)) = 'no'
    then
...
procedure TForm1.btnHireClick(Sender: TObject);
  ...
  begin
    ...
    for i := 1 to 8 do
      ...
      then
        Crim.Text := IllSystem.RDOFindCriminal(widestring(CriminalLabels[i].Caption));
    hired := 'no';
    i := 0;
    while hired = 'no' do
      begin
        i := i+1;
          ...
          then
            begin
              ...
              IllSystem.RDOHireCriminal(widestring(cbLeaders.Text), widestring(cbTeams.Text), widestring(Crim.Values['Name']), (TrackBar1.Position));
              ...
              while newOne = 'no' do
                begin
                  NewCrim.Text := IllSystem.RDOFindCriminal(widestring(Criminals.Values[IntToStr(o)]));
                    ...
                    begin
                      ...
                      then
                        ...
                        begin
                          ...
                          CriminalImages[k].Picture.LoadFromFile('E:\work\five\source\illegal\CriminalsPicture\NoCriminals.bmp');
    ...
    then
      begin
        for i := 1 to (TeamRoles.Count - 1) do
          if StrLComp(PChar(TeamRolesCombos[SenderIndex].Text), PChar(TMetaRole(TeamRoles.Items[i]).Name), StrLen(PChar(TMetaRole(TeamRoles.Items[i]).Name))) = 0
          then
            IllSystem.RDOChangeRole(widestring(cbLeaders.Text), widestring(cbTeams.Text), widestring(TeamLabels[SenderIndex].Caption), widestring(TMetaRole(TeamRoles.Items[i]).Name));
      end
    else
      IllSystem.RDOChangeRole(widestring(cbLeaders.Text), widestring(cbTeams.Text), widestring(TeamLabels[SenderIndex].Caption), widestring(''));
...
procedure TForm1.cbTeamsChange(Sender: TObject);
  ...
  begin
    ...
    TeamToShow.Text := IllSystem.RDOFindTeam(widestring(cbLeaders.Text), widestring(cbTeams.Text));
    ...
    then
      begin
        ...
        lblObjective.Caption := MisStoredOptions[0].Objective + '(' +  eObjX.Text + ', ' + eObjy.Text + ')';
...
procedure TForm1.ShowCriminal(CrimName : string; Position : integer);
  ...
  begin
    ...
    crimList.Text := IllSystem.RDOFindCriminal(widestring(CrimName));
    ...
    if crimList.Values['State'] = 'InTeam: Training'
    then
      begin
        StatusImages[Position].Picture.LoadFromFile('E:\work\five\source\illegal\UtilityPictures\Training.bmp');
              ...
              begin
                ...
                for i := 0 to (SkillModifiers.Count - 1) do
                  if crimList.Values['TrainingClass'] = TAttributeModifier(SkillModifiers[i]).Name
                    then
                      TeamSkillNameLabels[m, Position-1].Hint := TAttributeModifier(SkillModifiers[i]).Name + ', Difficulty: ' + IntToStr(TAttributeModifier(SkillModifiers[i]).Difficulty) + ', Av. Gain: ' + IntToStr(TAttributeModifier(SkillModifiers[i]).value) + ' - Finishing Time: ' + IntToStr(TAttributeModifier(SkillModifiers[i]).Time + Time) + ', Cost /h: ' + IntToStr(Round(TAttributeModifier(SkillModifiers[i]).Cost / TAttributeModifier(SkillModifiers[i]).Time));
              end;
          end;
      end;
    if crimList.Values['State'] = 'InTeam: StandBy'
    then
      begin
        TeamRolesCombos[Position].Enabled := True;
        StatusImages[Position].Picture := nil;
      end;
    if crimList.Values['State'] = 'InTeam: InMission'
    then
      begin
        StatusImages[Position].Picture.LoadFromFile('E:\work\five\source\illegal\UtilityPictures\Mission.bmp');
      end;
    if crimList.Values['State'] = 'InTeam: InMission: Wounded'
    then
      begin
        TeamRolesCombos[Position].Enabled := True;
        StatusImages[Position].Picture.LoadFromFile('E:\work\five\source\illegal\UtilityPictures\MissionWounded.bmp');
      end;
    if crimList.Values['State'] = 'InTeam: Wounded'
    then
      begin
        StatusImages[Position].Picture.LoadFromFile('E:\work\five\source\illegal\UtilityPictures\Wounded.bmp');
      end;
    if crimList.Values['State'] = 'InTeam: InMission: Dead'
    then
      begin
        StatusImages[Position].Picture.LoadFromFile('E:\work\five\source\illegal\UtilityPictures\MissionDead.bmp');
      end;
    if crimList.Values['State'] = 'InTeam: Dead'
    then
      begin
        TeamRolesCombos[Position].Enabled := False;
        TeamRolesCombos[Position].ItemIndex := -1;
        StatusImages[Position].Picture.LoadFromFile('E:\work\five\source\illegal\UtilityPictures\Dead.bmp');
      end;
    if crimList.Values['State'] = 'InTeam: InJail'
    then
      begin
        TeamRolesCombos[Position].Enabled := False;
        TeamRolesCombos[Position].ItemIndex := -1;
        StatusImages[Position].Picture.LoadFromFile('E:\work\five\source\illegal\UtilityPictures\Jail.bmp');
      end;
    if crimList.Values['State'] = 'InTeam: InJail : Wounded'
    then
      begin
        TeamRolesCombos[Position].Enabled := False;
        TeamRolesCombos[Position].ItemIndex := -1;
        StatusImages[Position].Picture.LoadFromFile('E:\work\five\source\illegal\UtilityPictures\JailWounded.bmp');
      end;
    if crimList.Values['State'] = 'InJail'
    then
      begin
        StatusImages[Position].Picture.LoadFromFile('E:\work\five\source\illegal\UtilityPictures\Jail.bmp');
      end;
    if crimList.Values['State'] = 'InJail : Wounded'
    then
      begin
        StatusImages[Position].Picture.LoadFromFile('E:\work\five\source\illegal\UtilityPictures\JailWounded.bmp');
      end;
...
procedure TForm1.Choice1Click(Sender: TObject);
  begin
    ...
    CalculateExpenses;
    IllSystem.RDOChangeCriminalState(widestring(CriminalInTeamName), widestring('OnTheMarket'));
...
procedure GetCriminalOutOfTeam;
  ...
  begin
    for i := 1 to 8 do
      ...
      then
        CleanCriminal(i);
    IllSystem.RDOFireCriminal(widestring(Form1.cbLeaders.Text), widestring(Form1.cbTeams.Text), widestring(CriminalInTeamName));
  ...
  begin
    ...
    then
      begin
        ...
        crimList := TStringList.Create;
        crimList.Text := IllSystem.RDOFindCriminal(widestring(CriminalInTeamName));
  ...
  begin
    ...
    then
      begin
        ...
        crimList := TStringList.Create;
        crimList.Text := IllSystem.RDOFindCriminal(widestring(CriminalName));
...
procedure TForm1.btnDismissTeamClick(Sender: TObject);
  ...
  begin
    btnDismissTeam.Enabled := False;
    IllSystem.RDODismissTeam(widestring(cbLeaders.Text), widestring(cbTeams.Text));
    for i := 1 to 8 do
      if TeamLabels[i].Caption <> ''
      then
...
procedure TForm1.ChangeTeamClick(Sender: TObject);
  var
    i          : integer;
  begin
    IllSystem.RDOChangeTeam(widestring(cbLeaders.Text), widestring(cbTeams.Text), widestring(TMenuItem(Sender).Caption), widestring(CriminalInTeamName));
    for i := 1 to 8 do
      If CriminalInTeamName = TeamLabels[i].Caption
      then
        CleanCriminal(i);
...
procedure TForm1.StopTraining(Sender: TObject);
  var
    ...
    j : integer;
  begin
    IllSystem.RDOStopCriminalTraining(widestring(CriminalInTeamName));
...
procedure TForm1.Training(Sender: TObject);
  var
    ...
    skill : string;
  begin
    IllSystem.RDOCriminalTraining(widestring(CriminalInTeamName), widestring(TAttributeModifier(SkillModifiers[StrToInt(TMenuItem(Sender).Hint)]).Name));
    lblTotalTraining.Caption := FloatToStr(Round(CalculateTotalTrainingCost(cbTeams.Text)));
    if StrToInt(lblTotalTraining.Caption) < 1
      then
        GroupBox6.Visible := False
    ...
    for i := 1 to 8 do
      begin
        ...
        then
          begin
            StatusImages[i].Picture.LoadFromFile('E:\work\five\source\illegal\UtilityPictures\Training.bmp');
            for k := 0 to (Skills.Count- 1) do
            begin
...
procedure TForm1.CustomPopUpMenu(CriminalState : string; Index : integer);
  ...
  begin
    ...
    for p := 0 to (TeamsInMenu.Count - 1) do
      begin
        Team.Text := IllSystem.RDOFindTeam(widestring(cbLeaders.Text), widestring(TMenuItem(TeamsInMenu[p]).Caption));
    ...
    then
      begin
        ...
        HistoryList.Text := IllSystem.RDORecoveryHistoryItem(widestring(''), widestring(''), widestring(TeamLabels[Index].Caption));
    ...
    if CriminalState = 'InTeam: StandBy'
    then
      begin
        for j := 1 to 4 do
    ...
    if CriminalState = 'InTeam: Training'
    then
      begin
    ...
    if CriminalState = 'InTeam: InMission'
    then
      begin
        PopUpMenuItems[1].Enabled := False;
    ...
    then
      begin
        ...
        CrimPenRecord.Text := IllSystem.RDORecoveryCriminalPendingRecord(widestring(TeamLabels[Index].Caption));
    ...
    then
      begin
        ...
        CrimPolRecord.Text := IllSystem.RDORecoveryCriminalPoliceRecord(widestring(TeamLabels[Index].Caption));
    ...
    if CriminalState <> 'NoCriminal'
    then
      if CriminalState = 'InJail'
      then
        begin
          JailDetails := TStringList.Create;
          JailDetails.Text := IllSystem.RDORecoveryCriminalJailDetails(widestring(TeamLabels[Index].Caption));
            ...
            begin
              ...
              then
                ...
                begin
                  PopUpMenuItems[7].Visible := True;
                  PopUpMenuItems[7].Enabled := True;
                  PopUpMenuItems[7].Caption := 'Trial Day N. ' + IntToStr(StrToInt(JailDetails.Values['TrialDay']) + StrToInt(JailDetails.Values['TrialLength']) - Time) + ' of ' + JailDetails.Values['TrialLength'] + ' - Lawyers'' Hours: ' + JailDetails.Values['LawyersHours'];
                end
            end
          else
            begin
...
procedure TForm1.btnHeadquarterClick(Sender: TObject);
  begin
    IllSystem.RDOAssignHeadquarter(widestring(cbLeaders.Text), widestring(cbTeams.Text), StrToInt(eHeadX.Text), StrToInt(eHeadY.Text));
...
function CalculateTotalSalary(TeamToCalcName : string): single;
  var
    total : single;
  begin
    total := StrToFloat(IllSystem.RDOCalculateSalary(widestring(Form1.cbLeaders.Text), widestring(TeamToCalcName)));
    Result := total;
  end;
...
function CalculateAvarPerc(TeamToCalcName : string): single;
  var
    total : single;
  begin
    total := StrToFloat(IllSystem.RDOCalculatePercentage(widestring(Form1.cbLeaders.Text), widestring(TeamToCalcName)));
    Result := total;
  end;
...
function CalculateTotalTrainingCost(TeamToCalcName : string): single;
  var
    total : single;
  begin
    total := StrToFloat(IllSystem.RDOCalculateTrainingCost(widestring(Form1.cbLeaders.Text), widestring(TeamToCalcName)));
    Result := total;
  end;

procedure CalculateExpenses;
  begin
    form1.lblExpenses.Caption := FloatToStr(Round(StrToFloat(IllSystem.RDOCalculateLeaderExpenses(widestring(Form1.cbLeaders.Text)))));
  end;
...
procedure FillTeamHistoryItems(TeamName : string);
  ...
  begin
    HistoryList := TStringList.Create;
    HistoryList.Text := IllSystem.RDORecoveryHistoryItem(widestring(Form1.cbLeaders.Text), widestring(TeamName), widestring(''));
...
procedure FillLeaderHistoryItems(LeaderName : string);
  ...
  begin
    HistoryList := TStringList.Create;
    HistoryList.Text := IllSystem.RDORecoveryHistoryItem(widestring(LeaderName), widestring(''), widestring(''));
  ...
  begin
    for j := 1 to 8 do
      ...
      then
        begin
          crimList := TStringList.Create;
          crimList.Text := IllSystem.RDOFindCriminal(widestring(CriminalLabels[j].Caption));
        end;
...
procedure TForm1.SalaryChange(Sender: TObject);
  ...
  begin
    ...
    IllSystem.RDOChangeCriminalSalary(widestring(cbLeaders.Text), widestring(TeamLabels[SenderIndex].Caption), SalaryBars[SenderIndex].Position);
    crimList := TStringList.Create;
    crimList.Text := IllSystem.RDOFindCriminal(widestring(TeamLabels[SenderIndex].Caption));
  ...
  begin
    ...
    then
      begin
        ...
        Info.Text := IllSystem.RDORecoveryMissionInfos(widestring(cbMissions.Text));
        ...
        for i := 0 to (MaxOptions - 1) do
          begin
            ...
            OptionInfos.Text := IllSystem.RDORecoveryMissionOption(widestring(cbMissions.Text), i);
            if i = 0
            then
      ...
      begin
        ...
        for m := 1 to TotalRows do
          begin
            ...
            then
              begin
                for u := 0 to (TeamRoles.Count - 1) do
        ...
        for r := 1 to 8 do
          begin
            ...
            then
              ...
              begin
                IllSystem.RDOChangeRole(widestring(Form1.cbLeaders.Text), widestring(Form1.cbTeams.Text), widestring(TeamLabels[r].Caption), widestring(''));
...
procedure TForm1.btnAcceptMissionClick(Sender: TObject);
  ...
  begin
      ...
      begin
        ...
        then
          begin
            ...
            StatusImages[i].Picture.LoadFromFile('E:\work\five\source\illegal\UtilityPictures\Mission.bmp');
          end;
      end;
    ...
    IllSystem.RDOAssignMission(widestring(edLeader.Text), widestring(cbTeams.Text), widestring(cbMissions.Text), widestring(RolesToPass.Text), widestring(Parameters.Text), StrToInt(eObjX.Text), StrToInt(eObjY.Text), lblMissionCost.Caption);
    FillTeamHistoryItems(cbTeams.Text);
...
procedure TForm1.eObjXChange(Sender: TObject);
  begin
    if (StrToInt(eObjX.Text) > 0)
    then
      btnAssignObj.Enabled := True
    else
      btnAssignObj.Enabled := False;
  end;

procedure TForm1.eObjYChange(Sender: TObject);
  begin
    if (StrToInt(eObjY.Text) > 0)
    then
      btnAssignObj.Enabled := True
    else
      btnAssignObj.Enabled := False;
...
procedure TForm1.btnStartMissionClick(Sender: TObject);
  begin
    IllSystem.RDOStartMission(widestring(edLeader.Text), widestring(cbTeams.Text), widestring(cbMissions.Text));
    GroupBox9.Top := 72;
    GroupBox9.Left := 688;
  ...
  begin
    ...
    for i := 1 to 8 do
      begin
        ...
        then
          begin
            CrimList.Text := IllSystem.RDOFindCriminal(widestring(TeamLabels[i].Caption));
            if CrimList.Values['State'] = 'InTeam: InMission'
            then
              begin
                TeamRolesCombos[i].Enabled := True;
    ...
    IllSystem.RDODeassignMission(widestring(edLeader.Text), widestring(cbTeams.Text), widestring('no'), widestring(cbMissions.Text));
    GroupBox7.Visible := True;
    GroupBox9.Visible := False;
...
procedure TForm1.ShowMissionBox(TeamList : TStringList);
  ...
  begin
    ...
    Report.text := IllSystem.RDOGetMissionReport(widestring(edLeader.Text), widestring(cbTeams.Text));
    mReport.Clear;
...
procedure TForm1.btnCancelMissionClick(Sender: TObject);
  ...
  begin
    ...
    IllSystem.RDODeassignMission(widestring(edLeader.Text), widestring(cbTeams.Text), widestring('yes'), widestring(cbMissions.Text));
    CleanMissionSheet;
    CleanMissionGrid;
    cbMissions.ItemIndex := -1;
    cbMissions.Enabled := True;
...
procedure TForm1.GiveTheBeat(Sender: TObject);
  ...
  begin
    ...
    then
      begin
        IllSystem.PropagateTheBeat; // >>
        LeaderList := TStringList.Create;
        LeaderList.Text := IllSystem.RDOFindLeader(widestring(edLeader.Text));
        lblMoney.Caption := FloatToStr(Round(StrToFloat(LeaderList.Values['Money'])));
          ...
          begin
            ...
            then
              begin
                ...
                TeamList.Text := IllSystem.RDOFindTeam(widestring(edLeader.Text), widestring(cbTeams.Text));
            ...
            for k := 1 to 8 do
              ...
              then
                begin
                  crimList := TStringList.Create;
                  crimList.Text := IllSystem.RDOFindCriminal(widestring(TeamLabels[k].Caption));
                  if crimList.Values['Change'] = 'yes'
                  then
                    begin
                      ShowCriminal(TeamLabels[k].Caption, k);
                      IllSystem.RDOCriminalShowed(TeamLabels[k].Caption);
                    end;
                end;
            lblTotalTraining.Caption := FloatToStr(Round(CalculateTotalTrainingCost(cbTeams.Text)));
            if StrToInt(lblTotalTraining.Caption) < 1
