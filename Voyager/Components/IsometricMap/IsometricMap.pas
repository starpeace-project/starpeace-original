  ...
  procedure TIsometricMap.SynchronizeSelection;
    ...
    begin
      ...
      if (i <> 0) and (j <> 0) and fExposed
        then
          begin
            x := trunc(2*nu*(rows - i + j)/du) - fOrigin.x;
            y := trunc(nu*((rows - i) + (cols - j))/du) - fOrigin.y;
            NotifySelect(x, y);
          end;
    end;
      ...
      if fExposed
        then
          begin
            ...
            j := fSelection.x;
            if (i <> 0) and (j <> 0)
              then Origin := Point(trunc(2*nu*(rows - i + j)/du) - size.x div 2, trunc(nu*((rows - i) + (cols - j))/du) - size.y div 2);
          end;
    end;
      ...
      if assigned(fOnSelect)
        then
          begin
            inc(x, fOrigin.x);
            inc(y, fOrigin.y);
            i := trunc((2*nu*(2*rows + cols) - du*(x + 2*y))/(4*nu));
            j := trunc((2*nu*cols + du*(x - 2*y))/(4*nu));
    ...
    begin
      ...
      for y := oy to y2 do
        begin
          ...
          for x := ix1 to ix2 do
            begin
              SetPixel(hCanvas, x - fOrigin.x, y - fOrigin.y, fMapper.GetColor(trunc(ix/(4*nu)), trunc(jx/(4*nu))));
              ix := ix - du;
              jx := jx + du;
            end;
          iy := iy - 2*du;
