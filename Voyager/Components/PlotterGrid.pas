...
implementation
  ...
  procedure TPlotter.Chart( info : string; EndDate : integer; Money, Kilos : boolean );
    var
      PtData : array[0..PointCount - 1] of Cardinal;
      Max    : double;
      Min    : double;
      count  : Cardinal;
      pos    : integer;
      val    : double; //val    : integer;
      i, j   : Cardinal;
      R1, R2 : TRect;
    begin
      try
        ...
        // Do the dew
        pos   := 1;
        count := StrToInt64(GetNextStringUpTo( info, pos, ',' ));
        min   := StrToInt64(GetNextStringUpTo( info, pos, ',' ));
        max   := StrToInt64(GetNextStringUpTo( info, pos, ',' ));
        for i := 0 to pred(count) do
          begin
            PtData[i] := StrToInt(GetNextStringUpTo( info, pos, ',' ));
            val := ((max - min)*PtData[i]) / 255 + min;
            if Money
              then values[i].Caption := FormatMoney( val )
              else values[i].Caption := IntToStr(round(val));
            if Kilos
        ...
        if (min < 0) and (max > 0)
          then
            begin
              val := -255*min / (max - min); //val := -255*min div (max - min);
              with Lines.Picture.Bitmap.Canvas do
                begin
                  ...
                  Brush.Style := bsClear;
                  Pen.Width   := 1;
                  val := Grid.Height - (Grid.Height*val) / 255;
                  MoveTo( 0, round(val) );
                  LineTo( Grid.Width, round(val) );
                end;
            end;
        Lines.Transparent := true;
      except
