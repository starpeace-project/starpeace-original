...
interface
  ...
  uses
    Classes, SoundMixer, SoundTypes, TimerTypes, ShutDown, VCLUtils;
    ...
    begin
      try
        for i := pred(fTargets.Count) downto 0 do
          begin
            RemoveTarget(PTargetInfo(fTargets[i]).Target);
{
            TargetInfo := PTargetInfo(fTargets[i]);
            if (TargetInfo <> nil) and (TargetInfo.Target<>nil)
              then
                begin
                  with TargetInfo.Target do
                    fSoundMixer.StopPlayingSound(GetSoundName, GetSoundKind);
