...
interface
  ...
  uses
    Circuits, VoyagerServerInterfaces, GameTypes, MapTypes, Land, VCLUtils;
...
implementation
  ...
  function RoadBlockId(topolid : TRoadBlockId; landid : idLand; onconcrete, onrailroad, isdummy : boolean) : TRoad;
    ...
    begin
      if topolid <> rbNone
        then
          begin
            ...
            if (LandClassOf(landId) = lncZoneD) and not onconcrete
              then
                begin
      ...
      case Result of
        42, 50, 51, 53, 64, 65, 68, 80, 81, 84, 89, 102, 109, 35, 34, 37, 46, 54, 59, 56, 110: Result := roadNone;
        39 : Result := roadNone;  //Result := $31; // ,
        38 : Result := roadNone; //Result := $31;
        86 : Result := $16;//roadNone; //Result := 
      end
    end;
