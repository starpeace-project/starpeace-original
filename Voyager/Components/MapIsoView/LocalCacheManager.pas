...
interface
  ...
  uses
    ...
    Vehicles, LocalCacheTypes, IniClasses, Config;
    ...
    TLocalCacheManager =
      class(TInterfacedObject, ILocalCacheManager)
        ...
        private // ILocalCacheManager
          procedure DestroyAll;  //.rag
          procedure SetClientView(const ClientView : IClientView);
          procedure SetConfigHolder(const ConfigHolder : IConfigHolder);
          ...
          function  GetPedestrianClassCount : integer;
...
implementation
  ...
  destructor TLocalCacheManager.Destroy;
    begin
      DestroyAll;  //.
      // pointer(fAdviseSink) := nil;   // Cross referenced  //.rag
      inherited;
    end;

  procedure TLocalCacheManager.DestroyAll;  //.rag 
    begin
      if fCacheLoaded
        then ReleaseCache;
      fBuildingClasses := nil;
      fClientView      := nil;
      fAdviseSink      := nil;
    end;
  ...
  procedure TLocalCacheManager.SetConfigHolder(const ConfigHolder : IConfigHolder);
    begin
      fConfigHolder := ConfigHolder;
    end;
  ...
  procedure TLocalCacheManager.SetAdviseSink(const Sink : ILocalCacheAdviseSink);
    begin
      fAdviseSink := Sink;
      // fAdviseSink._Release; // cross referenced  //.rag
    end;
  ...
  function TLocalCacheManager.GetPedestrianClassCount : integer;
    begin
      Result := fPedestrianClassCount;
    end;
                ...
                then
                  begin
                    ...
                    if fConfigHolder<>nil
                      then DefSite := fConfigHolder.ReadString(true, '', 'DefaultSite', '')
                      else DefSite := '';
                    if DefSite=''
                      then DefSite := fClientView.getWorldAbsURL;
                    if Synchronize(DefSite + cServerDir + 'BuildingImages/'+ aux.ImagePath, fBasePath + 'BuildingImages\' + aux.ImagePath, nil) = SYNC_NOERROR
                      then Result := LoadGameImage(fBasePath + 'BuildingImages\' + aux.ImagePath);
                  end;
                if Result <> nil
                  then aux.Height := Result.Height;
              {$ELSE}
              if Result = nil
  ...
  function TLocalCacheManager.GetPedestrianImage(const zoom : TZoomRes; id : idPedestrian; angle : TAngle) : TGameImage;
    begin
      ...
      if fPedestrianClasses[id].ImagePaths[angle] <> ''
        then
          begin
            Result := LoadGameImage(fBasePath + 'PedestrianImages\' + fPedestrianClasses[id].ImagePaths[angle]);
            if Result<>nil
              then
                with fPedestrianClasses[id].ImageInfo[angle] do
                  begin
                    width := Result.Width;
              ...
              then
                begin
                  ...
                  try
                    ...
                    fRoadBlockClasses[id].Freq  := ini.ReadInteger('General', 'Freq', 1);
            ...
            if info.Attr and faDirectory <> faDirectory
              then
                begin
                  ini := TIniClass.Open(path + info.Name);
                  try
                    inc(fPedestrianClassCount);
                    id := ini.ReadInteger('General', 'Id', high(id));
                    assert((id >= 0) and (id < 256));
                    fPedestrianClasses[id].id := id;
                    fPedestrianClasses[id].valid := true;
                    fPedestrianClasses[id].DelayTick := ini.ReadInteger('General', 'DelayTick', 32);
                    for pa := low(pa) to high(pa) do
                      begin
