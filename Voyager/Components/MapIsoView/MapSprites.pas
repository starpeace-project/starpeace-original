...
interface
  ...
  uses
    Windows, Classes, GameTypes, ShutDown, TimerTypes, LanderTypes, MapTypes, VCLUtils;
...
implementation
  ...
  constructor TMapSpriteManager.Create(const Map : IWorldMap; const Converter : ICoordinateConverter; const Manager : ILocalCacheManager; MinAnimInterval : integer; OwnsSprites : boolean);
    begin
      ...
      fManager := Manager;
      fFocuses := TInterfaceList.Create;
            ...
            if assigned(view)
              then
                begin
                  ...
                  BlockX := Sprite.GetOldBlockX(view);
                  BlockY := Sprite.GetOldBlockY(view);
                  inc(oldx, BlockX);
                  inc(oldy, BlockY);
                  ...
                  if not IsRectEmpty(R)
                    then (focus as IAnimationCache).AddAnimationRect(R);
                end;
          end;
    end;
  ...
  procedure TMapSpriteManager.Disable;
    ...
    begin
      ...
      fEnabled := false;
      for i := pred(GetSpriteCount) downto 0 do  //.rag Lo puse alreves la lista
        try
          begin
            Sprite := GetSprite(i);
            if Sprite <> nil
  ...
  procedure TMapSpriteManager.OnShutDown;
    ...
    begin
      fEnabled := false;
      for i := pred(GetSpriteCount) downto 0 do  //.rag se recore la lista alrevez
        begin
          CurSprite := GetSprite(i);
          if CurSprite <> nil
            then  
              begin
                UnregisterSprite(CurSprite); //.rag
                CurSprite := nil //.rag
              end;
        end;
    end;
  ...
  procedure TMapSpriteManager.AnimationTick;
    ...
    begin
      ...
      try
        for i := pred(GetSpriteCount) downto 0 do  //.rag esta lista la puse alrevez porque hay tipos que se eliminan
          begin
            CurSprite := GetSprite(i);
            try
              if CurSprite <> nil
