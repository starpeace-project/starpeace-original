...
interface
  ...
  uses
    ...
    ColorTableMgr, SyncObjs, VCLUtils;
  ...
  type
    ...
    TPedestrianImages    = array[idPedestrian, TAngle] of TGameImage;
  ...
  ICacheImager =       // rag
    interface(IImager) 
      ...
      procedure ReloadLandImages(ImageSuit : integer; flag : boolean); 
      // Parche
    ...
    TCacheImager =
      class(TInterfacedObject, IImager)
        ...
        private // IImager
          ...
          fPedestrianImages    : TPedestrianImages;
        ...
        private
          ...
          function  GetViewCount: integer;  //.rag
          function  Instance: TObject;      //.rag (Parche)
      end;
    ...
    TImageCache =
      class
        ...
        private
          ...
          fImagers : array[TRotation, TZoomRes] of ICacheImager;
...
implementation
  ...
  uses
    Shutdown, SysUtils, GDI, SpriteUtils, Land, Concrete, Roads,
    Railroads;  //AxlDebug,  //.rag
  ...
  type
    TBasicImager =
      class(TCacheImager, IShutDownTarget, ICacheImager)
        private
  ...
  type
    TTransformImager =
      class(TCacheImager, ICacheImager)
        private
          constructor Create(const view : IGameView; BasicImager : ICacheImager);
          constructor CreateExplicit(zoom : TZoomRes; rotation : TRotation; BasicImager : ICacheImager);
          procedure   Initialize(ImageSuit : integer); override;
          destructor  Destroy; override;
        private // IImager
          function  GetObjectImage(id : integer; angle : TAngle) : TGameImage; override;
  ...
  destructor TCacheImager.Destroy;
    ...
    begin
          ...
          try
            ...
            for i := low(fPedestrianImages) to high(fPedestrianImages) do
              for ang := low(ang) to high(ang) do
                ReleaseImgRef(fPedestrianImages[i, ang]);
      ...
      try
        fViews.Free;  // <<>> finalize them // A veces explota....
      except
      end;
  ...
  procedure TCacheImager.DetachView(const which : IGameView);  // >>
    begin
      fViews.Remove(which); //.rag
      {if fViews.Remove(pointer(which)) <> -1
        then which._Release;}//.rag
    end;
  ...
  function TCacheImager.GetViewCount: integer;
    begin
      if fViews<>nil
        then result := fViews.Count
        else result := 0;
    end;

function TCacheImager.Instance: TObject;
  begin
    result := self;
  end;
...
// TBasicImager
  ...
  procedure TBasicImager.Initialize(ImageSuit : integer);
    ...
    begin
      ...
      (*if Asserting
        then fThread.fDebugName := 'ImagerDownloader';*)  //.rag
      fImageSuit := ImageSuit;
      fViews.free;  //.rag
      fViews := TInterfaceList.Create;  //.rag , Cuantas veces piensas que se crea esta lista.
      for i := low(fLandImages) to high(fLandImages) do
      ...
      for i := low(fPedestrianImages) to high(fPedestrianImages) do
        for j := low(j) to high(j) do
          fPedestrianImages[i, j] := fManager.GetPedestrianImage(fZoom, i, j);
      ...
      case mask of
        ...
        idConcreteMask :
          begin
            ...
            {$ifopt D+}
               if Result=nil
                 then OutputDebugString(pchar('ConcreteImages not found '+inttostr(id)));
            {$endif}
          end;
        idRoadBlockMask :
          begin
            ...
            {$ifopt D+}
               if Result=nil
                 then OutputDebugString(pchar('RoadBlockImages not found '+inttostr(id)));
            {$endif}
          end;
        idRailingMask:
        ...
        idPedestrianMask:
          Result := fPedestrianImages[id, angle];
        else
  ...
  procedure TBasicImager.OnShutDown;
    begin
      ...
      FreeAndNil(fThread);
      FreeAndNil(fDownloadRequests);
      FreeAndNil(fDownloadRequestsCS);
      FreeAndNil(fNewDownloadRequest);
  ...
  constructor TTransformImager.Create(const view : IGameView; BasicImager : ICacheImager);
    begin
      inherited Create(view);
      fBasicImager := (BasicImager.Instance as TBasicImager);
      Initialize(view.GetImageSuit);
    end;

  constructor TTransformImager.CreateExplicit(zoom : TZoomRes; rotation : TRotation; BasicImager : ICacheImager);
    begin
      inherited CreateExplicit(zoom, rotation);
      fBasicImager := (BasicImager.Instance as TBasicImager);
      Initialize(0);
  ...
  function TTransformImager.GetObjectImage(id : integer; angle : TAngle) : TGameImage;
    ...
    begin
      ...
      case mask of
        ...
        idPedestrianMask:
          begin
            if fPedestrianImages[id, angle] = nil
              then
                begin
                  SrcImg := fBasicImager.GetObjectImage(mask or id, angle);
                  if (SrcImg <> nil) and (fZoom <> cBasicZoomRes)
                  ...
                  fPedestrianImages[id, angle] := TransfImg;
                end;
            Result := fPedestrianImages[id, angle];
          end
        else
      ...
      if fImagers[r, z] = nil
        then
          if (r = cBasicRotation) and (z = cBasicZoomRes)
            ...
            else fImagers[r, z] := TTransformImager.Create(view, fImagers[cBasicRotation, cBasicZoomRes]);
      ...
      if fImagers[rotation, zoom] = nil
        then
          if (rotation = cBasicRotation) and (zoom = cBasicZoomRes)
            ...
            else fImagers[rotation, zoom] := TTransformImager.CreateExplicit(zoom, rotation, fImagers[cBasicRotation, cBasicZoomRes]);
      Result := fImagers[rotation, zoom];
    end;

  function TImageCache.ThrowAwayExplicitImager(zoom : TZoomRes; rotation : TRotation) : IImager;
    begin
      if (fImagers[rotation, zoom] <> nil) and (fImagers[rotation, zoom].GetViewCount = 0) and ((rotation <> cBasicRotation) or (zoom <> cBasicZoomRes))
        then
          begin
            // fImagers[rotation, zoom].Free; //.rag
  ...
  procedure TImageCache.SetImageSuit( ImageSuit : integer );
    ...
    begin
      for r := low(r) to high(r) do
        for z := low(z) to high(z) do
          if (fImagers[r, z] <> nil) and (fImagers[r, z].GetViewCount > 0) //.rag  (.GetViewCount=.fViews.Count)
  ...
  procedure TImageCache.CheckDeletableImager;
    ...
    begin
      for r := low(r) to high(r) do
        for z := low(z) to high(z) do
          if (fImagers[r, z] <> nil) and (fImagers[r, z].GetViewCount = 0) and ((r <> cBasicRotation) or (z <> cBasicZoomRes))//.rag  (.GetViewCount=.fViews.Count)
            then
              begin
                // fImagers[r, z].Free; //.rag
                fImagers[r, z] := nil;
              end;
    end;
