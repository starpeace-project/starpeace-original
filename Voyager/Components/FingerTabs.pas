    ...
    TFingerTabs =
      class(TCustomControl)
        ...
        private
          ...
          procedure DrawTabs;
          procedure RenderToBmp(recalc : boolean);
          procedure RedrawAll;
          function  OffSetRects(dy : integer) : integer;
        protected
        ...
        public
          ...
          procedure MyOnFingerChange(Sender: TObject);
        private
...
implementation
  ...
  constructor TFingerTabs.Create(AOwner : TComponent);
    begin
      ...
      TStringList(fTabNames).OnChange :=  MyOnFingerChange;
  ...
  procedure TFingerTabs.CalcRects;
    ...
    begin
      if fCurrentFinger = noFinger
        then crTop := fTopOffset + fFingerRadius
        else
          if fCurrentFinger = 0
            then crTop := 0
            else crTop := fTopOffset + fTextMargin;
      for i := 0 to pred(fTabNames.Count) do
        begin
          R := MessureLineHeight(fTabNames[i]);
          crHgh := R.Bottom;
          // Let's align the Rect
          if fFixedSize
            then
              begin
  ...
  procedure TFingerTabs.RenderToBmp(recalc : boolean);
    begin
      //GradientUtils.Gradient(fBitmap.Canvas, fLeftColor, fRightColor, true, ClientRect);
      if recalc
        then RebuildRects;
      ...
      if not fUpdating
        then
          begin
            ...
            if TtHght > Height
              then fScrollSize := TtHght - Height
              else
                begin
                  fScrollSize := 0;
                  fTopOffset  := 0;
  ...
  function TFingerTabs.OffSetRects(dy : integer) : integer;
    var
      i : integer;
    begin
      if fTabNames.Count > 0
        then
          begin
            for i := 0 to pred(fTabNames.Count) do
              Windows.OffsetRect(fRects[i], 0, dy);
            result := fRects[0].Top;
          end
        else result := 0;
    end;
  ...
  procedure TFingerTabs.SetBounds(ALeft, ATop, AWidth, AHeight : Integer);
    begin
      ...
      if (Parent <> nil) and (Width > 0) and (Height > 0)
        then RenderToBmp(fRects = nil);
    end;
  ...
  procedure TFingerTabs.MouseMove(Shift: TShiftState; X, Y: Integer);
    ...
    begin
      ...
      if fScrollSize > 0
        then
          if (Height - Y < 2*fStdHeight) and (fTopOffset + fScrollSize - fStdHeight > 0)
            then
              begin
                dy := 4;
                fTopOffset := OffSetRects(-dy);
                RenderToBmp(false);
                Refresh;
              end
            else
              if (Y < 2*fStdHeight) and (fTopOffset <= 4)
                then
                  begin
                    dy := 4;
                    fTopOffset := OffSetRects(dy);
  ...
  procedure TFingerTabs.SetCurrentFinger(index : integer);
    begin
      if (fCurrentFinger <> index) and (index >= 0) and (index < fTabNames.Count)
        then
          begin
  ...
  procedure TFingerTabs.MyOnFingerChange(Sender: TObject);
    begin
      RedrawAll;
    end;
