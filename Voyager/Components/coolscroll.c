...
//
void CoolSB_ZeroMemory(void *ptr, DWORD bytes)
{
	BYTE *bptr = (BYTE *)ptr;
...
static UINT DrawScrollButton(SCROLLBUT *sbut, HDC hdc, const RECT *pctrl, UINT flags)
{
	...
	case SBBT_OWNERDRAW:
		...
		IntersectClipRect(hdc, rect.left, rect.top, rect.right, rect.bottom);
                //hwnd = GetParent(hwnd);
		SendMessage(hwnd, WM_NOTIFY, nmcd.hdr.idFrom, (LPARAM)&nmcd);
...
LRESULT CALLBACK CoolSBWndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	static int count;
        int nVScrollPos, nPrevVScrollPos, nVScrollPage, nVScrollMax;
        int nHScrollPos, nPrevHScrollPos, nHScrollPage, nHScrollMax;
        SCROLLINFO si;
        POINT pt1;
        int ColumnWidths, ColumnWidth, Columns;
...
/*        case WM_PAINT:
                if (swnd->IsListView)
                {
                  Columns = SendMessage(hwnd, LVM_GETITEMCOUNT, 0, 0);
                  SendMessage(hwnd, LVM_SETCOLUMNWIDTH, Columns-1 , MAKELPARAM ((int) -2, 0) );
                }
                break;*/
        case WM_PAINT:
	...
	case WM_HSCROLL:
		...
		//OutputDebugString(ach);
                //nHScrollPos = CoolSB_GetScrollPos(hwnd, SB_HORZ);
               ...
               if ((LOWORD(wParam)==SB_THUMBTRACK) || (LOWORD(wParam)==SB_THUMBPOSITION))
                {
                  nHScrollPos = si.nTrackPos;
                   ...
                   {
                     nHScrollPos = GetScrollPos( hwnd, SB_HORZ);//si.nPos;
                   }

                nHScrollPage = si.nPage;
                nHScrollMax = si.nMax;

		switch(LOWORD(wParam))
		{
		case SB_LINEUP:
                        nPrevHScrollPos = si.nPos;
                        nHScrollPos = GetScrollPos( hwnd, SB_HORZ);//si.nPos;
                        nHScrollPos = min(nPrevHScrollPos, nHScrollPos);
                	nHScrollPos -= 1 ;					break;
		case SB_LINEDOWN:
                        nHScrollPos = GetScrollPos( hwnd, SB_HORZ);//si.nPos;
                        nHScrollPos ++;					break;
		...
		case SB_THUMBTRACK, SB_THUMBPOSITION: nHScrollPos = HIWORD(wParam);	break;
		}

		if ((nHScrollPos < 0))// && ( LOWORD(wParam)!= SB_ENDSCROLL ))
			nHScrollPos = 0;
		...
		if(nHScrollPos > nHScrollMax - nHScrollPage + 1)
			nHScrollPos = nHScrollMax - nHScrollPage + 1;
                ...
                {
                  //SendMessage(hwnd, LVM_GETITEMPOSITION, nHScrollPos, (long)&pt1);
                  nPrevHScrollPos = GetScrollPos( hwnd, SB_HORZ);
/*                  ColumnWidths = 0;
                  Columns = nHScrollPos;
                  while ( Columns )
                  {
                    Columns--;
                    ColumnWidth = SendMessage(hwnd, LVM_GETCOLUMNWIDTH, Columns ,0);
                    ColumnWidths += ColumnWidth;
                  }*/
                     //LVItemRect.left = LVIR_BOUNDS;
                     //SendMessage(hwnd, LVM_GETITEMRECT, 0, (LPARAM)&LVItemRect);
                  //SendMessage(hwnd, LVM_SCROLL, 0, -(nVScrollPos+1)*(LVItemRect.bottom-LVItemRect.top));
                  //nPrevHScrollPos = GetScrollPos( hwnd, SB_HORZ);
//                  SendMessage(hwnd, LVM_SCROLL, nHScrollPos , 0); //LVM_GETITEMRECT
                  if ((LOWORD(wParam)!=SB_THUMBTRACK) && (LOWORD(wParam)!=SB_THUMBPOSITION) )
                     SendMessage(hwnd, LVM_SCROLL, -(nPrevHScrollPos-nHScrollPos)/*1*/ , 0); //LVM_GETITEMRECT
                  else
                  {
                     SendMessage(hwnd, LVM_SCROLL, -(nPrevHScrollPos-nHScrollPos) , 0); //LVM_GETITEMRECT
                  }
                  //SendMessage(hwnd, LVM_SCROLL, (nHScrollPos-nPrevHScrollPos)*((LVItemRect.right-LVItemRect.left)), 0); //LVM_GETITEMRECT
		  //CoolSB_SetScrollPos(hwnd, SB_HORZ, (nHScrollPos)/* *((LVItemRect.right-LVItemRect.left)/4)*/, TRUE);
                  CoolSB_SetScrollPos( hwnd, SB_HORZ, nHScrollPos/* *nHScrollMax)/(nHScrollMax-nHScrollPage)*/, TRUE );
//                  si.cbSize = sizeof(SCROLLINFO);
//                  si.fMask  = SIF_ALL;
...
/*		  CoolSB_SetScrollPos(hwnd, SB_HORZ, nHScrollPos, TRUE);
        ...
        case WM_VSCROLL://ogm
                //nVScrollPos = CoolSB_GetScrollPos(hwnd, SB_VERT);
                si.cbSize = sizeof(SCROLLINFO);
                si.fMask  = SIF_ALL;
                ...
                if ((LOWORD(wParam)==SB_THUMBTRACK) || (LOWORD(wParam)==SB_THUMBPOSITION))
                  {
                    nVScrollPos = si.nTrackPos;
                     ...
                     {
                       nVScrollPos = GetScrollPos( hwnd, SB_VERT);//si.nPos;
                     }
                //nPrevVScrollPos = si.nPos;
                nVScrollPage = si.nPage;
                nVScrollMax = si.nMax;
		switch(LOWORD(wParam))
		{
		...
		case SB_THUMBTRACK, SB_THUMBPOSITION: nVScrollPos = HIWORD(wParam);	break;
		}

		//wsprintf(ach, "WM_VSCROLL %d\n", count++);
		//OutputDebugString(ach);

		if ((nVScrollPos < 0)) //&& ( LOWORD(wParam)!= SB_ENDSCROLL ))
			nVScrollPos = 0;

		//if ((nVScrollPos > nVScrollMax - nVScrollPage + 1) && ( LOWORD(wParam)!= SB_ENDSCROLL ))
		//	nVScrollPos = nVScrollMax - nVScrollPage + 1;
                ...
                if ( LOWORD(wParam)!= SB_ENDSCROLL )
                {
                  ...
                  {
                      nPrevVScrollPos = GetScrollPos( hwnd, SB_VERT);
                      if ((LOWORD(wParam)!=SB_THUMBTRACK) && (LOWORD(wParam)!=SB_THUMBPOSITION) )
                         SendMessage(hwnd, LVM_SCROLL, 0, -(nPrevVScrollPos-nVScrollPos)/*1*/ ); //LVM_GETITEMRECT
                      else
                      {
                         SendMessage(hwnd, LVM_SCROLL, 0, -(nPrevVScrollPos-nVScrollPos)); //LVM_GETITEMRECT
                      }
                      //SendMessage(hwnd, LVM_SCROLL, (nHScrollPos-nPrevHScrollPos)*((LVItemRect.right-LVItemRect.left)), 0); //LVM_GETITEMRECT
                      //CoolSB_SetScrollPos(hwnd, SB_HORZ, (nHScrollPos)/* *((LVItemRect.right-LVItemRect.left)/4)*/, TRUE);
                      CoolSB_SetScrollPos( hwnd, SB_VERT, nVScrollPos/* *nHScrollMax)/(nHScrollMax-nHScrollPage)*/, TRUE );
                  }
                  else
                    {
                      ...
                      //SendMessage(hwnd, LVM_SCROLL, 0, -(nVScrollPos+1)*(LVItemRect.bottom-LVItemRect.top));
                      nPrevVScrollPos = GetScrollPos( hwnd, SB_VERT);
                      SendMessage(hwnd, LVM_SCROLL, 0, (nVScrollPos-nPrevVScrollPos)*(LVItemRect.bottom-LVItemRect.top)); //LVM_GETITEMRECT
                      //SetScrollPos( hwnd, SB_VERT, nVScrollPos, TRUE );*/
                    };
                }
                else
...
//                  SendMessage(hwnd, LVM_SCROLL, 0, (nVScrollPos-nPrevVScrollPos)*(LVItemRect.bottom-LVItemRect.top)); //LVM_GETITEMRECT
                ...
                //if (LOWORD(wParam)== SB_THUMBTRACK)
                  // SendMessage(hwnd, LVM_SCROLL, 0, nVScrollPos*50);
  		//return 0;
                break;
	case WM_NCDESTROY:
...
/*                  // if the notification is a Custom Draw notification
	...
	case WM_SIZE:
		SetupScrollbars(hwnd);
		return 0;
        case LVM_INSERTITEM: case LVM_DELETEALLITEMS:
        case LVM_DELETECOLUMN: case LVM_DELETEITEM:
