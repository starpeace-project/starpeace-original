...
implementation
                      ...
                      if ((fInstances[idx].fEfxs.efxs[k].x <> low(integer)) and (fInstances[idx].fEfxs.efxs[k].y <> low(integer))) or CalcEffectCoords(idx, k)
                        then
                          begin
                            ...
                            item.Overlays.objects[item.Overlays.Count].id := idEffectMask or fInstances[idx].fEfxs.efxs[k].id;
                            item.Overlays.objects[item.Overlays.Count].xshift := round((fInstances[idx].fEfxs.efxs[k].x - effectcls.XHook)*FocusObj.fZoomFactor);
                            item.Overlays.objects[item.Overlays.Count].yshift := round((fInstances[idx].fEfxs.efxs[k].y - effectcls.YHook)*FocusObj.fZoomFactor);
                            item.Overlays.objects[item.Overlays.Count].angle := agN;
                            item.Overlays.objects[item.Overlays.Count].frame := fInstances[idx].fEfxs.efxs[k].frame;
                            if item.Overlays.objects[item.Overlays.Count].frame = cUnAssignedFrame
                              then item.Overlays.objects[item.Overlays.Count].frame := 0;
                            item.Overlays.objects[item.Overlays.Count].Options := item.Options;
                            item.Overlays.objects[item.Overlays.Count].shadeid := item.shadeid;
                            if eoGlassed in fInstances[idx].fEfxs.efxs[k].Opts
                              then include(item.Overlays.objects[item.Overlays.Count].Options, loGlassed);
                          end;
                    end;
              end;
                                  ...
                                  if CarInstance <> nil
                                    then
                                      begin
                                        ...
                                        objects[count].xshift := CarInstance.GetBlockX(fView);
                                        objects[count].yshift := CarInstance.GetBlockY(fView);
                                      end;
                                end;
                            ...
                            if img <> nil
                              then
                                begin
                                  ...
                                  objects[count].xshift := 0;
                                  objects[count].yshift := u - img.Height;
                          ...
                          begin
                            ...
                            if img <> nil
                              then
                                begin
                                  inc(count);
                                  objects[count].id := efxid;
                                  u := 2 shl FocusObj.fView.ZoomLevel;
                                  objects[count].xshift := 2*u - img.Width div 2;
                                  objects[count].yshift := -(img.Height div 2);
                                  if CheckForConcrete(i, j) and (LandClassOf(fLands[i, j].landId) = lncZoneD) and (FocusObj <> nil)
                                    then dec(objects[count].yshift, round(cPlatformShift*FocusObj.fZoomFactor));
                                  if FocusObj.fCachingAnimations
              ...
              then
                begin
                  ...
                  if LandClassOf(fLands[which.r, which.c].landid) = lncZoneD
                    then concretesize := pred(cConcreteSize)
                    else concretesize := cConcreteSize;
                  if (buildclass.Urban or (LandClassOf(fLands[which.r, which.c].landid) = lncZoneD)) and isnewbuild and not which.fDummy
                    then
                      for i := which.r - concretesize to which.r + size + concretesize - 1 do
                        for j := which.c - concretesize to which.c + size + concretesize - 1 do
                          if (LandClassOf(fLands[i, j].landId) <> lncZoneD) or ((LandClassOf(fLands[which.r, which.c].landid) = lncZoneD) and (LandTypeOf(fLands[i, j].landId) = ldtCenter))
              ...
              then
                for r := fInstances[idx].r - concretesize to fInstances[idx].r + size + concretesize - 1 do
                  for c := fInstances[idx].c - concretesize to fInstances[idx].c + size + concretesize - 1 do
                    if (LandClassOf(fLands[r, c].landId) <> lncZoneD) or ((LandClassOf(fLands[fInstances[idx].r, fInstances[idx].c].landid) = lncZoneD) and (LandTypeOf(fLands[r, c].landId) = ldtCenter))
  ...
  function TWorldMap.CheckForConcrete(i, j : integer) : boolean;
    begin
      if ((i >= 0) and (i < fRows)) and ((j >= 0) and (j < fColumns)) and (fBlocks <> nil) and (fBlocks[i shr cBlockBits, j shr cBlockBits].fConcreteCounts <> nil)
  ...
  function TWorldMap.GetRailroad(i, j : integer) : TRailroad;
    begin
      if ((i >= 0) and (i < fRows)) and ((j >= 0) and (j < fColumns))
        then
          if (fBlocks <> nil) and (fBlocks[i shr cBlockBits, j shr cBlockBits].fRailroadBlocks <> nil)
              ...
              if worldid = fWorldId
                then
                  begin
                    ...
                    if (Renderedrailroads <> nil) and (fWorldId = worldid)
                      then
                        for i := imin to imax do
                          for j := jmin to jmax do
                            begin
                                ...
                                if (worldid = fWorldId)
                                  then
                                    begin
                                      ...
                                      // update roads
                                      for i := imin - cConcreteSize to imax + cConcreteSize do
                                        for j := jmin - cConcreteSize to jmax + cConcreteSize do
                                          begin
                                            roadblock := Roads[i, j];
          ...
          if CheckForConcrete(i, j)
            then
              begin
                ...
                concconfig[cUp, cCenter] := CheckForConcrete(i + 1, j) or CheckPluggedRoad(cUp, cCenter);
                concconfig[cDown, cCenter] := CheckForConcrete(i - 1, j ) or CheckPluggedRoad(cDown, cCenter);
                concconfig[cCenter, cLeft] := CheckForConcrete(i, j - 1) or CheckPluggedRoad(cCenter, cLeft);
                concconfig[cCenter, cRight] := CheckForConcrete(i, j + 1) or CheckPluggedRoad(cCenter, cRight);
                ...
                if LandClassOf(fLands[i, j].landId) = lncZoneD
                  then concreteid := concreteid or cPlatformFlag;
            ...
            while (r < size) and ok do
              begin
                ...
                while (c < size) and ok do
                  begin
                    ok := (Cells[i, j] >= cellUnused) and CheckWaterConditions(i, j, toucheswater) and CheckZone(i, j) and (Roads[i, j] = roadNone) and (Railroads[i, j] = railroadNone) and (not IsVoidSquare(i, j, allowedfid) or (allowedfid = bclass.FacId));
                    inc(j, jinc);
                    inc(c);
                  end;
                inc(i, iinc);
                  ...
                  if ok
                    then
                      begin
                        ok := false;
                        i := row;
                        roadmindist := pred(cConcreteSize);
      ...
      if worldid = fWorldId
        then
          begin
            ...
            try
              for i := max(imin shr cBlockBits, 0) to min(imax shr cBlockBits, fRows shr cBlockBits) do
                for j := max(jmin shr cBlockBits, 0) to min(jmax shr cBlockBits, fColumns shr cBlockBits) do
