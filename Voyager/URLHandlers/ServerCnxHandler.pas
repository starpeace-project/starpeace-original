...
interface
  ...
  uses
    VoyagerInterfaces, VoyagerServerInterfaces, VoyagerEvents, Protocol, Windows, Controls, WinSockRDOConnection,
    RDOServer, RDOInterfaces, Classes, Collection, Matrix, SyncObjs, GMKernel, Privacy, VCLUtils;
    ...
    TServerCnxHandler =
      class( TInterfacedObject, IMetaURLHandler, IURLHandler, IClientView )
        ...
        private
          ...
          procedure CloneFacility(x, y, options1, options2 : integer);
          ...
          function  FavNewItem    ( Location : widestring; Kind : integer; Name, Info : widestring ) : OleVariant;
          function  FavDelItem    ( Location : widestring ) : OleVariant;
          function  FavMoveItem   ( ItemLoc : widestring; Dest : widestring ) : OleVariant;
          function  FavRenameItem ( ItemLoc : widestring; Name : widestring ) : OleVariant;
          function  FavGetSubItems( ItemLoc : widestring ) : OleVariant;
        ...
        private
          ...
          procedure threadedSetClientData( const parms : array of const );
        private
...
{$endif}
  ...
  const
    ...
    htmlParmName_MasterUserName   = 'MasterUserName';
...
implementation
  ...
  uses
    URLParser, SysUtils, WinSockRDOConnectionsServer, RDOObjectProxy,
    Forms, ConfirmPasswordDialog, mr_StrUtils, ServerCnxEvents, StreamToStr, MapCompress,
    Config, Notification, CompStringsParser, Threads,
    ConnectingWindow,
     MessageBox,
    Literals, ClientMLS, CRC32
    {$IFDEF VER140}
      Variants,
    {$ENDIF}
    //ChatHandler
  ...
  resourcestring
    rsSystemFormat = '<s /ital><z10><c$00FF00><z13><f Times New Roman><s ital>';
        ...
        while not Terminated and not fSuccess do
          begin
            ...
            if (not Terminated) and (fServerCnxHandler<>nil)
              then
                with fServerCnxHandler do
                  fSuccess := Reconnect(fUserName, true, true);
          end;
      end;
  ...
  destructor TServerCnxHandler.Destroy;
    begin
      if fReconnectThread<>nil
        then
          try
            fReconnectThread.OnTerminate := nil;
            fReconnectThread.Terminate;
            TReconnectThread(fReconnectThread).fServerCnxHandler := nil;
          except
      ...
      FreeAndNil(fClientEventsServer);
      inherited;
    end;
              ...
              then
                begin
                  ...
                  if StrToBoolean(URLParser.GetParmValue( URL, htmlParmName_Threaded ))
                    then
                      begin
                        with ConnectingWin do
                          begin
                            Left := Screen.Width - ConnectingWin.Width;
                            Top  := Screen.Height - ConnectingWin.Height;
                            MaxTimeout := ISTimeout;
                            Display( GetLiteral('Literal319') );
                          end;
  ...
  procedure TServerCnxHandler.setMasterURLHandler( const URLHandler : IMasterURLHandler );
    begin
      ...
      if uppercase(Dest) <> uppercase(fUserName)
        then
          ...
          begin
            ...
            Info.Msg  := Msg;
            // System
            Info.Formatted := rsSystemFormat + TimeToStr(Now) + ' - ' + Info.From + ': ' + Info.Msg;
  ...
  procedure TServerCnxHandler.CloneFacility(x, y, options1, options2 : integer);
    begin
      try
        fISProxy.CloneFacility(x, y, options1, options2, fTycoonUId);
      except
      end;
    end;
  ...
  function TServerCnxHandler.FavNewItem( Location : widestring; Kind : integer; Name, Info : widestring ) : OleVariant;
    begin
      try
        result := fISProxy.RDOFavoritesNewItem( Location, Kind, Name, Info );
      except
      end;
    end;

  function TServerCnxHandler.FavDelItem( Location : widestring ) : OleVariant;
    begin
      try
        result := fISProxy.RDOFavoritesDelItem( Location );
      except
      end;
    end;

  function TServerCnxHandler.FavMoveItem( ItemLoc : widestring; Dest : widestring ) : OleVariant;
    begin
      try
        result := fISProxy.RDOFavoritesMoveItem( ItemLoc, Dest );
      except
      end;
    end;

  function TServerCnxHandler.FavRenameItem( ItemLoc : widestring; Name : widestring ) : OleVariant;
    begin
      try
        result := fISProxy.RDOFavoritesRenameItem( ItemLoc, Name );
      except
      end;
    end;

  function TServerCnxHandler.FavGetSubItems( ItemLoc : widestring ) : OleVariant;
    var
      x : string;
    begin
      try
        x := widestring(fISProxy.RDOFavoritesGetSubItems( ItemLoc ));
        result := x;
      except
      end;
    end;
  ...
  function TServerCnxHandler.getCacheAddr : string;
    ...
    begin
      ...
      p2 := p1;
      while (p2 <= length(fWorldURL)) and (fWorldURL[p2] <> '/') and (fWorldURL[p2] <> ':') do
        inc(p2);
  ...
  procedure TServerCnxHandler.StoreLastConfig;
    ...
    begin
      ...
      ConfigHolder.WriteString ( false, KeyName, 'MasterUserName',   fMasterUserName);
    end;
          ...
          then
            try
              ...
              try
                ...
                tmp          := URLParser.GetParmValue( URL, htmlParmName_MasterUserName );
                if tmp<>''
                  then fMasterUserName := tmp;
                fUserName    := URLParser.GetParmValue( URL, htmlParmName_UserName );
                fPassword    := URLParser.GetParmValue( URL, htmlParmName_Password );
                Silent       := StrToBoolean(URLParser.GetParmValue( URL, htmlParmName_Silent ));
                IgnoreErrors := StrToBoolean(URLParser.GetParmValue( URL, htmlParmName_IgnoreErrors ));
  ...
  procedure TServerCnxHandler.syncCreateEventsServer( const parms : array of const );
    ...
    begin
      ...
      Fork( threadedSetClientData, priLower, [0] );
      fMasterURLHandler.HandleEvent( evnAnswerPrivacyHandler, fPrivacyHandler );
    end;

  procedure TServerCnxHandler.threadedSetClientData( const parms : array of const );
    var
      crc  : integer;
      data : TStringList;
    begin
      try
        crc := CRC_File( paramstr(0) );
        Lock;
        try
          try
            if not VarIsEmpty(fISProxy) and not fOffline
              then
                begin
                  ...
                  try
                    data.values['CRC'] := IntToStr( crc );
                    fISProxy.SetClientData( data.text );
                    ReportCnxValid;
                  finally
                    data.free;
                  end;
  ...
  procedure TServerCnxHandler.syncChatMsg;
    ...
    begin
      if (fMasterURLHandler <> nil) and ((fPrivacyHandler = nil) or not fPrivacyHandler.UserIsIgnored( syncChatFrom ))
        then
          begin
            Info.From := syncChatFrom;
            if uppercase(syncChatFrom) = GetLiteral('Literal333')
              then Info.Msg := TimeToStr(Now) + ' - ' + syncChatFrom + ': ' + syncChatText
              else Info.Msg := syncChatText;
            if Info.From = GetLiteral( 'Literal320' )
              then
                begin
                  // System
                  nameformat := '<z10>';
                  textformat := '<c$00FF00><z13><f Times New Roman><s ital>';
                end
              else
                if system.pos('GM_', Info.From ) = 1
                  then
                    begin
                      // Game master
                      nameformat := '<i oc><i gm> <c$CCCCCC><z9><f Verdana><s /ital>' + Info.From + ': ';
                      textformat := '<c$FFFFFF><f Verdana><s ital>';
                    end
                  else
                    if system.pos('OC_', Info.From ) = 1
                      then
                        begin
                          // Oceanus
                          nameformat := '<i oc> <c$CCCCCC><z10><f Verdana><s /ital>' + Info.From + ': ';
                          textformat := '<c$FFFFFF><f Verdana><s ital>';
                        end
                      else
                        begin
                          // Normal guy
                          nameformat := nameformat + '<c$00FFFF><f Verdana>' + Info.From + ': ';
                          textformat := '<c$FFFFFF>';
                        end;
            Info.Formatted := '<s /ital>' + nameformat + textformat + Info.Msg;
            fMasterURLHandler.HandleEvent( cmdAnswerChatHandler, ChatHandler );
            if ChatHandler <> nil
              then Info.Formatted := ChatHandler.filterChatLine( Info.Formatted );
            fMasterURLHandler.HandleEvent( evnChatMsg, Info );
          end;
    end;
...
{$endif}
  ...
  procedure TServerCnxHandler.ConnectionDropped;
    ...
    begin
      ...
      //Info.From := GetLiteral('Literal320');
      Info.Formatted := '<s /ital><z10><c$00FF00><z13><f Times New Roman><s ital>' +TimeToStr(Now) + ' - ' + Info.From + ': ' + Info.Msg;
      fMasterURLHandler.HandleEvent( evnChatMsg, Info );
    ...
    begin
      ...
      try
        ...
        try
          ...
          result := Logon( LogonURL ) = urlHandled;
          if result
            then
              begin
                fOffline   := false;
              ...
              begin  // This is that attent to connect
                if (fMsgCount mod 115) = 0
                  then
                    begin
  ...
  procedure TServerCnxHandler.CacheObjectText( Id : TObjId; Text : string );
    ...
    begin
      ...
      p1   := 1;
      repeat
        p2 := mr_strutils.Pos( StatusTextSeparator, Text, p1 );
        if p2 > 0
          then
            begin
  ...
  function TServerCnxHandler.Offline : boolean;
    begin
      result := fOffline or VarIsEmpty(fISProxy);
    end;
...
function TServerCnxHandler.IsServerBusy: boolean;
  begin
    try
      fServerBusy := false;
      if fExceptCount<4
        then
          begin
            if not VarIsEmpty(fISProxy) and not fOffline
              then fServerBusy := fISProxy.ServerBusy;
            fExceptCount := 0;
          end;
    except
      inc(fExceptCount);
