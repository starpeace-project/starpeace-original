...
uses
  ...
  InternationalizerComponent, Config, Privacy, ImgList;
...
implementation
  ...
  uses
    Events, SocketComp, Threads, OptionsHandler, FileCtrl, Literals, ClientMLS, CoolSB, Display, DisplayControl;
  ...
  procedure TOptionsHandlerView.MusicVolChange(Sender: TObject);
    begin
      ...
      fConfigHolder.WriteString(false, fClientView.getUserName, 'MusicVolume', IntToStr(MusicVol.Value));
    end;
  ...
  procedure TOptionsHandlerView.PlaylistDblClick(Sender: TObject);
    begin
      if (Playlist.Selected <> nil) and EnableSounds.Checked
  ...
  procedure TOptionsHandlerView.Loaded;
    ...
    procedure InitOptionControls;
      ...
      begin
        ...
        s := fConfigHolder.ReadString(false, fClientView.getUserName, 'MusicVolume', '50');
        MusicVol.Value := StrToInt(s);
        fMasterURLHandler.HandleURL('?frame_Id=MP3Handler&frame_Action=SetVolume&MediaId=MainSoundTrack&Volume=' + s);
        fLoadConfig := false;
      end;
...
procedure TOptionsHandlerView.SetParent(which: TWinControl);
  begin
    inherited;
    if InitSkinImage and (which<>nil)
      then
        begin
          InitializeCoolSB(Playlist.Handle);
          InitializeCoolSB(IgnoreList.Handle);
          InitializeCoolSB(IgnoreList.Handle);
          InitializeCoolSB(FacIdsListBox.Handle);
          if hThemeLib <> 0
            then
              begin
                SetWindowTheme(PlayList.Handle, ' ', ' ');
                SetWindowTheme(IgnoreList.Handle, ' ', ' ');
                SetWindowTheme(FacIdsListBox.Handle, ' ', ' ');
              end;
        end;
 end;

procedure TOptionsHandlerView.btApplyClick(Sender: TObject);
  var
    i : integer;
    s : string;
    result: integer;
...
procedure TOptionsHandlerView.cbResChange(Sender: TObject);
  var
    i : integer;
    s : string;
  begin
