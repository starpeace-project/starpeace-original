...
implementation
  ...
  destructor TMP3Handler.Destroy;
    var
      i : integer;
    begin
      for i := pred(fPlayers.Count) downto 0 do
  ...
  function TMP3Handler.HandleURL( URL : TURL ) : TURLHandlingResult;
    ...
    function Play( URL : string ) : TURLHandlingResult;
      ...
      begin
        ...
        if (MediaId = '') and (MediaURL = '') and (fLastPlay<>'')
          then
            begin
              MediaId := fLastPlay;
              fLastPlay := '';
    ...
    function Stop( URL : string ) : TURLHandlingResult;
      ...
      begin
        ...
        if idx >= 0
          then
            begin
              TMP3Player(fPlayers.Objects[idx]).Stop;
              fLastPlay := '';
            end
          else
            begin
              for idx:=0 to fPlayers.count-1 do
                begin
                  if TMP3Player(fPlayers.Objects[idx]).State <> mpsStopped
                    then
                      begin
                        fLastPlay :=fPlayers.strings[idx];
                        TMP3Player(fPlayers.Objects[idx]).Stop;
                      end;
                end;
            end;
        ...
        if idx >= 0
          then TMP3Player(fPlayers.Objects[idx]).Pause
            ...
            begin
              for idx:=0 to fPlayers.count-1 do
                begin
                  if TMP3Player(fPlayers[idx]).State = mpsPlaying
                    then
                      begin
                        fLastPlay :=fPlayers.strings[idx];
                        TMP3Player(fPlayers.Objects[idx]).Pause;
                      end;
                end;
            end;
        result := urlHandled;
        ...
        if idx >= 0
          then TMP3Player(fPlayers.Objects[idx]).Play
            ...
            begin
              idx := 0;
              if fLastPlay<>''
                then idx := fPlayers.IndexOf( fLastPlay );
              if fPlayers.Count >0
                then TMP3Player(fPlayers.Objects[idx]).Play;
            end;
        result := urlHandled;
  ...
  function TMP3Handler.HandleEvent( EventId : TEventId; var info ) : TEventHandlingResult;
    begin
      result := evnHandled;
      if EventId=evnShutDown   //.rag
        then fMasterURLHandler := nil;      
    end;
  ...
  procedure TMP3Handler.setMasterURLHandler( const URLHandler : IMasterURLHandler );
    begin
          ...
          if (Info.MediaId <> '')
            then
              begin
                ...
                if fLastPlay=''
                  then fMasterURLHandler.HandleEvent( evnMediaStatusChanged, Info );
              end;
      end;
    end;
