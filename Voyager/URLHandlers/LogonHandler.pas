...
implementation
  ...
  uses
    URLParser, SysUtils, Events, Forms, VCLUtils, ServerCnxEvents;
  // TLogonHandler
  ...
  function TLogonHandler.HandleEvent( EventId : TEventId; var info ) : TEventHandlingResult;
    begin
      ...
      case EventId of
        ...
        evnShutDown:
          begin
            fMasterURLHandler := nil;  //.rag
            if fControl<>nil
              then
                begin
                  (fControl as TLogonHandlerView).FreeImages;
                end;
  ...
  procedure TLogonHandler.setMasterURLHandler( const URLHandler : IMasterURLHandler );
    var
      cache : string;
    begin
