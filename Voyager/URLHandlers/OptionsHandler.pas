...
interface
  ...
  uses
    ...
    Protocol, JukeBox, ChatHandler, Privacy, VCLUtils;
...
implementation
  ...
  uses
    ServerCnxHandler, VoyagerUIEvents, Events, ServerCnxEvents, Config, SysUtils, FileCtrl, VoyagerEvents, DisplayControl;
          ...
          case KeyCommand of
            keyRestoreMode:
              begin
                ...
                fMasterURLHandler.HandleEvent(evnRedrawWindows, i);
              end;
          end;
        evnShutDown :   //.rag
          begin
  ...
  procedure TOptionsHandler.setMasterURLHandler( const URLHandler : IMasterURLHandler );
    begin
      ...
      if not UserIsIgnored( username )
        then
          begin
            ...
            if (fControl <> nil) and (fControl.IgnoreList <> nil)
              then fControl.IgnoreList.Items.Assign( fIgnoreList );
            StorePrivData;
          end;
    end;
