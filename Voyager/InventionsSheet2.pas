...
implementation
  ...
  uses
    ...
    DelphiStreamUtils, SheetUtils, ClassStorage, ClientMLS,
    {$IFDEF VER140}
      Variants,
    {$ENDIF}
    Literals, CoolSB;
  ...
  procedure TInventionsSheetViewer.SetParent(which: TWinControl);
    begin
      ...
      if InitSkinImage and (which<>nil)
        then
          begin
            InitializeCoolSB(tvAvailInventions.Handle);
            InitializeCoolSB(tvAlrDevInventions.Handle);
            InitializeCoolSB(lvSchedInvention.Handle);
          end;
    end;
