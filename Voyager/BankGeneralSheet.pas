...
implementation
  ...
  uses
    Threads, SheetHandlerRegistry, FiveViewUtils, CacheCommon, MathUtils, mr_StrUtils,
    Protocol, SheetUtils, MessageBox, 
    {$IFDEF VER140}
      Variants,
    {$ENDIF}
    Literals;
        ...
        if (fCurrBlock <> 0) and (Update = fLastUpdate)
          then
            begin
              ...
              try
                EstLoan := MSProxy.RDOEstimateLoan(fContainer.GetClientView.getTycoonId);
                //EstLoan  := realmax(0, EstLoan); // >>
              except
                //EstLoan := 10;
              end;
              ...
              Term     := MSProxy.Term;
              //Prop.Values[tidEstLoan]  := FormatMoneyStr( '%.0n', [int(EstLoan)] );
              Prop.Values[tidEstLoan]  := EstLoan;
            ...
            if not VarIsEmpty(Proxy)
              then
                begin
                  ...
                  Amount := KillSpaces( ReplaceChar( Amount, '$', ' ' ));
