...
implementation
  ...
  constructor TMetaChemMineBlock.Create(anId            : string;
                                    aCapacities     : array of TFluidValue;
                                    aLegalServMax   : TFluidValue;
                                    aCompMax        : TFluidValue;
                                    aChemicalMax    : TFluidValue;
    ...
    begin
      ...
      MetaOutputs.Insert(
        TMetaOutput.Create(
          ...
          TPullOutput,
          TMetaFluid(TheClassStorage.ClassById[tidClassFamily_Fluids, tidFluid_OreChems]),
          5,
          [mgoptCacheable, mgoptEditable],
          sizeof(Sample.fOreChems),
          Sample.Offset(Sample.fOreChems)));

      // MetaEvaluators
      with TMetaOutputEvaluator.Create(
        aMaxBudget,
        intcond(aMaxBudget > 0, 20, 0),
        OutputByName[tidGate_OreChems],
        TOutputEvaluator) do
        begin
          FullOpenTime  := 30;
          FullCloseTime := 31;
  ...
  procedure RegisterBackup;
    begin
      BackupInterfaces.RegisterClass(TChemMineBlock);
    end;
