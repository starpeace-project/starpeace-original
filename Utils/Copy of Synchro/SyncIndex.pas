...
interface
  ...
  function  CompareFolder(indexName, Folder : string; NewFiles, OldFiles, NewDirs, SyncDirs, OldDirs : TStringList; var count : integer) : boolean;
...
implementation
  ...
  function DoCompareFolder(Dest, Prefix : string; Index : TFolderIndex; NewFiles, OldFiles, NewDirs, SyncDirs, OldDirs : TStringList) : integer;
    var
      Search  : TSearchRec;
      Entry   : TFolderIndexEntry;
      i       : integer;
            ...
            begin
              repeat
                if (Search.Name <> '.') and (Search.Name <> '..')
                  then
                    if Index.FindFile(Search.Name, Entry)
                      then
                        begin
                          Entry.fFlag := true;
  ...
  function CompareFolder(indexName, Folder : string; NewFiles, OldFiles, NewDirs, SyncDirs, OldDirs : TStringList; var count : integer) : boolean;
    ...
    procedure CompareMasterIndex(Prefix : string; MasterIndex : TMasterIndex);
      var
        ...
        NewPrf : string;
      begin
        count := count + DoCompareFolder(Folder + Prefix, Prefix, MasterIndex.FolderIndex, NewFiles, OldFiles, NewDirs, SyncDirs, OldDirs);
                ...
                if lowercase(ExtractFileExt(indexName)) = '.sync'
                  then
                    begin
                      ...
                      count := count + DoCompareFolder(Folder, '', Index, NewFiles, OldFiles, NewDirs, SyncDirs, OldDirs);
                    end
                  else
                    ...
                    begin
                      ...
                      if FileExt = '.CAB'
                        then Index.AddCab(Folder + Search.Name, Search)
                          ...
                          if (FileExt <> '.SYNC') and (FileExt <> '.BAT')
                            then
                                ...
                                then
                                  Index.Add(
                                    TFolderIndexEntry.Create(
                                      ...
                                      '',
                                      sokArchive))
                                else Entry.fDate := Search.FindData.ftLastWriteTime;
                    end
                  else
