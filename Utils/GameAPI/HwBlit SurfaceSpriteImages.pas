  ...
  type
    TSurfaceFrameImage =
      class
        ...
        public
          procedure Draw(x, y, frame, alpha : integer; scale : single; const ClipArea : TRect; const RenderTarget : IDirectDrawSurface7; const aPalette : IDirectDrawPalette);
          procedure DrawOpaque(x, y, frame : integer; scale : single; const ClipArea : TRect; const RenderTarget : IDirectDrawSurface7; const aPalette : IDirectDrawPalette);
      end;
...
implementation
  ...
  procedure TSurfaceFrameImage.NewFrames(Count : integer);
    ...
    begin
      ...
      for i := 0 to pred(Count) do
        begin
          fFrames[FrameCount + i].surface := DDrawD3DMgr.CreateCompatibleOffscreenSurface(Width, Height);
  ...
  procedure TSurfaceFrameImage.Draw(x, y, frame, alpha : integer; scale : single; const ClipArea : TRect; const RenderTarget : IDirectDrawSurface7; const aPalette : IDirectDrawPalette);
    const
      cMaxAlpha = 8;
    var
      ImageArea : TRect;
    ...
    begin
      ...
      if (frame >= 0) and (frame < fFrameCount)
        then //raise Exception.Create('Bad frame specified in TSurfaceFrameImage.Draw!!');
          begin
            with ImageArea do
              begin
                Left   := x;
            ...
            IncImgCount;
            {$ENDIF}
            if not IsRectEmpty(DestRect) and not IsRectEmpty(ImageArea)
              then
                begin
                  with fFrames[frame] do
                    hr := RenderTarget.Blt(@DestRect, surface, @ImageArea, DDBLT_DONOTWAIT or DDBLT_KEYSRC, nil);
                  {
                  if Failed(hr)
                    then
                      begin
                        ...
                        WriteDebugStr('Error!');
                        WriteDebugStr('Error!');
                        WriteDebugStr('Sprite Blit Failed!!!');
  ...
  procedure TSurfaceFrameImage.DrawOpaque(x, y, frame : integer; scale : single; const ClipArea : TRect; const RenderTarget : IDirectDrawSurface7; const aPalette : IDirectDrawPalette);
    var
      ImageArea : TRect;
      DestRect  : TRect;
      hr        : HRESULT;
          ...
          begin
            ...
            if not IsRectEmpty(DestRect) and not IsRectEmpty(ImageArea)
              then
                begin
                  with fFrames[frame] do
                    hr := RenderTarget.Blt(@DestRect, surface, @ImageArea, DDBLT_DONOTWAIT, nil);
                  {
                  if Failed(hr)
                    then
                      begin
