...
implementation
            ...
            if Result <> WSAEWOULDBLOCK then
              if Assigned( SocketErrorProc )
                then
                  ...
                  raise ESocketError.CreateFmt( sWindowsSocketError, [ SysErrorMessage( Result ), Result, Op ] )
          end
        else
          Result := 0
    end;
  ...
  procedure TCustomWinSocket.ClearSpool;
    ...
    function SendPacket( Packet : TSpooledPacket ) : boolean;
      ...
      begin
        ...
        if fTCPBufSize > BytesLeft
          then
            BytesSent := Send( fSocket, Aux^, BytesLeft, 0 )
          else
            BytesSent := Send( fSocket, Aux^, fTCPBufSize, 0 );
  ...
  procedure TCustomWinSocket.Event( Socket : TCustomWinSocket; SocketEvent : TSocketEvent );
    begin
      if Assigned( fOnSocketEvent )
        then
          fOnSocketEvent( Self, Socket, SocketEvent )
    end;
  ...
  procedure TCustomWinSocket.Error( Socket:  TCustomWinSocket; ErrorEvent : TErrorEvent; var ErrorCode : integer );
    begin
      if Assigned( fOnErrorEvent )
        then
          fOnErrorEvent( Self, Socket, ErrorEvent, ErrorCode )
    end;
