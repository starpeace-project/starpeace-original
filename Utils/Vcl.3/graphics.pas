...
type
  ...
  private
    ...
    procedure NewImage(NewHandle: HBITMAP; NewPalette: HPALETTE;
      const NewDIB: TDIBSection; OS2Format: Boolean);
    procedure ReadStream(Stream: TStream; Size: Longint);
    procedure ReadDIB(Stream: TStream; ImageSize: Longint);
...
procedure InvalidGraphic(const Str: string); near;
begin
...
begin
  ...
  with FImage do
   if FInch = 0 then
     ...
     Result := MulDiv(FHeight, ScreenLogPixels, 25400);
end;
...
begin
  ...
  with FImage do
    if FInch = 0 then
      ...
      Result := MulDiv(FWidth, ScreenLogPixels, 25400);
end;
...
begin
  ...
  try
    ...
    begin
      ...
      Colors := Pointer(Integer(Bitmap) + Bitmap^.biSize);
    ...
    try
      ...
      begin
        ...
        try
          ...
          if Bitmap^.biBitCount <= 8 then
          begin
            Colors := Pointer(Integer(BitmapInfo) + BitmapInfo^.biSize);
            ByteSwapColors(Colors^, ColorCount);
            for I := 0 to ColorCount - 1 do
              for J := Low(OldColors) to High(OldColors) do
              begin
...
begin
  ...
  with DIB.dsbm do
  try
    if DIB.dsbmih.biSize < sizeof(DIB.dsbmih) then
...
begin
  with FImage do
    ...
    else if FHandle <> FDIBHandle then
    begin
      if FDIBHandle <> 0 then
...
procedure TBitmap.ReadDIB(Stream: TStream; ImageSize: Longint);
const
...
{!!    if (Stream is TMemoryMappedStream) then
    begin
      SectionHandle := Stream.Handle;
  ...
  NewImage(BMHandle, Pal, DIB, OS2Format);
  PaletteModified := Palette <> 0;
  Changed(Self);
end;
...
begin
  ...
  begin
    ...
    if FDIBHandle <> 0 then
    begin
      InternalGetDIBSizes(FDIBHandle, HeaderSize, Size, 0);
      if FOS2Format then
      begin
    ...
    begin
      ...
      begin
        ...
        if FOS2Format then RGBQuadToTriple(Colors, ColorCount);
      end;
      if FOS2Format then
      begin
