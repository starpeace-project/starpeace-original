...
interface
  ...
  type
    TTransTableStates = ( tsMixMatrixValid, tsHiColor555TableValid, tsHiColor565TableValid, tsHiColorUnpackTableValid, tsTrueColorTableValid );
    ...
    TPaletteInfo =
      class
        protected
          ...
          fHiColor555Table       : PColorTransTableHi;
          fHiColor565Table       : PColorTransTableHi;
        ...
        public
          ...
          property HiColor555Table       : PColorTransTableHi    read fHiColor555Table;
          property HiColor565Table       : PColorTransTableHi    read fHiColor565Table;
...
implementation
  ...
  procedure TPaletteInfo.Release;
    begin
      FreePtr( fMixMatrix );
      FreePtr( fHiColor555Table );
      FreePtr( fHiColor565Table );
      ...
      if NeededState <> []
        then
          begin
            ...
            if (tsHiColor555TableValid in NeededState) and (fHiColor555Table = nil)
              then fHiColor555Table := ColorTrans.Tab8toHiColor( RgbPalette^, Count, $7c00, $03e0, $001f );
            if (tsHiColor565TableValid in NeededState) and (fHiColor565Table = nil)
              then fHiColor565Table := ColorTrans.Tab8toHiColor( RgbPalette^, Count, $f800, $07e0, $001f );
  ...
  procedure TPaletteInfo.UpdateTables;
    begin
      ...
      if (tsHiColor555TableValid in fState) and (fHiColor555Table <> nil)
        then ColorTrans.Tab8toHiColorInPlace( RgbPalette^, Count, fHiColor555Table^, 0, 0, 0 );
      if (tsHiColor565TableValid in fState) and (fHiColor565Table <> nil)
        then ColorTrans.Tab8toHiColorInPlace( RgbPalette^, Count, fHiColor565Table^, 0, 0, 0 );
