...
interface
  ...
  uses
    ...
    MemUtils, ChunkStream, AnimPlay, GDI, Dibs, DibRle,
    Flics, FliPlayback;      // We'll be using a format based on Flics
...
implementation
  ...
  procedure TRleStream.LoadFrameHeader;
    begin
      fFrameHeader := Relock( fFrameHeader, FrameHeaderSize );
    end;

  function TRleStream.CurrentFrameSize : integer;
    begin
      assert( Assigned( fFrameHeader ), 'Frame Header not loaded in RlePlay.TRleStream.CurrentFrameSize!' );
      Result := PFliFrame( fFrameHeader )^.Size;
    end;

  function TRleStream.FrameHeaderSize : integer;
    begin
  ...
  function TRleDecoder.GetFrameDelay : longint;
    begin
