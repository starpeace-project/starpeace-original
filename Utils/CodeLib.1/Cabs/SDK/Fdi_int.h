...
#define DFNOPEN(fn)  int FAR DIAMONDAPI fn(char FAR *pszFile, int oflag, int pmode)
#define DFNREAD(fn)  UINT FAR DIAMONDAPI fn(int hf, void FAR *pv, UINT cb)
#define DFNWRITE(fn) UINT FAR DIAMONDAPI fn(int hf, void FAR *pv, UINT cb)
#define DFNCLOSE(fn) int FAR DIAMONDAPI fn(int hf)
#define DFNSEEK(fn)  long FAR DIAMONDAPI fn(int hf, long dist, int seektype)
