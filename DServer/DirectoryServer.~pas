...
const
  ...
  function authUser(usr, psw : pchar) : integer; cdecl;

external 'sega_snap.dll'
  function getInitResult : integer; cdecl;
...
begin
  ...
  then
  begin
    LogString(baseLog + 'Session created');
  end;
  try
    ...
    begin
      ...
      then
      begin
        LogString('Current key set (' + userkey + ')');
      end;
      spAlias := session.RDOReadString('alias');
      if log
      then
      begin
        LogString(baseLog + 'Alias fetched: ' + spAlias);
      end;
      result := spAlias;
    end
    else result := '';
    ...
    begin
      ...
      then
      begin
        LogString(baseLog + 'Unable to find alias at SP (local db) so looking at SEGA');
      end;
      ISProxy := GetRemoteSession;
      if not VarIsEmpty(ISProxy)
      then
        try
          ...
          then
          begin
            LogString(baseLog + ' So alias found at sega so has been stored locally');
          end;
        finally
          ISProxy.RDOEndSession;
        end;
  ...
  try
    ...
    try
      ...
      begin
        ...
        case accStat of
          DIR_ACC_NoAuthUserA, DIR_ACC_NoAuthUserB : expDate := Now;
        else
          expDate := session.RDOReadDate('TrialExpires');
        end;
        ...
        begin
          ...
          case session.RDOLogonUser(alias, passwrd) of
            DIR_NOERROR, DIR_ERROR_Unknown : result := 0;
...
begin
  case error of
    DIR_SEGA_NOERROR : result := DIR_NOERROR;
    DIR_SEGA_ERROR_UserNotFound : result := DIR_ERROR_UnexistingAccount;
    DIR_SEGA_ERROR_BadUserIDFormat : result := DIR_ERROR_Unknown;
    DIR_SEGA_ERROR_UnknownDomain : result := DIR_ERROR_Unknown;
    DIR_SEGA_ERROR_InternalError : result := DIR_ERROR_Unknown;
    DIR_SEGA_ERROR_UserNotAuthorized : result := DIR_ERROR_InvalidPassword;
    DIR_SEGA_ERROR_SystemDown : result := DIR_ERROR_Unknown;
    DIR_SEGA_ERROR_NotInitialized : result := DIR_ERROR_Unknown;
    DIR_SEGA_ERROR_TimeOut : result := DIR_ERROR_Unknown;
  else
    result := error;
  end;
end;
