...
interface
  ...
  uses
    VoyagerInterfaces, Config, Classes, Controls, Registry;
  ...
  const
    VOYAGER_ROOT = '\SOFTWARE\Oceanus\Star Peace\Client\';

implementation

  uses
    Windows, Events, URLParser, SysUtils;
        ...
        if System
          then
            begin
              ...
              KeyName := VOYAGER_ROOT + 'System\' + DataOwner
            end
          else
            begin
              result.RootKey := HKEY_CURRENT_USER;
              KeyName := VOYAGER_ROOT + 'Users\' + DataOwner;
            end;
  ...
  procedure TConfigHandler.setMasterURLHandler( URLHandler : IMasterURLHandler );
    begin
