...
implementation
  ...
  function TMapIsoHandler.HandleURL( URL : TURL ) : TURLHandlingResult;
    ...
    const
      ZoneColors : array[0..7] of TColor =
        ($00595959, clMaroon, clTeal, $00BBFFC0, $0043A34F, $001E4823, $0088D9D7, $00D87449);
    ...
    function DefineZone( URL : TURL ) : TURLHandlingResult;
      ...
      begin
        ...
        fControl.MapView.StartAreaSelection( ZoneId, ZoneColors[ZoneId], [axWater] );
        result := urlHandled;
      end;
    ...
    function Build( URL : TURL ) : TURLHandlingResult;
      ...
      begin
        ...
        fControl.ShowSurface( 1 );
        fControl.MapView.StartFacilityBuild( FacClass, FacVisualClass );
        result := urlHandled;
      end;
          ...
          then
            begin
              ...
              if (x <> 0) or (y <> 0)
                then
                  if select
                    then fControl.MapView.MoveAndSelect( y, x )
    ...
    function BuildRoad : TURLHandlingResult;
      begin
        ...
        fControl.MapView.StartCircuitBuild( cirRoads, 20000000 );
        result := urlHandled;
      end;
    ...
    function BuildRailroad : TURLHandlingResult;
      begin
        fStatusEnabled := false;
        fControl.MapView.StartCircuitBuild( cirRailroads, 20000000 );
        result := urlHandled;
      end;
  ...
  function TMapIsoHandler.HandleEvent( EventId : TEventId; var info ) : TEventHandlingResult;
    ...
    procedure InitMapViewOptions;
      begin
        with fControl do
          begin
            MapView.SoundsEnabled := fConfigHolder.ReadBoolean(false, fClientView.getUserName, 'EnableSounds', true);
            MapView.GlassBuildings := fConfigHolder.ReadBoolean(false, fClientView.getUserName, 'UseTransparency', true);
            MapView.AnimateBuildings := fConfigHolder.ReadBoolean(false, fClientView.getUserName, 'AnimateBuildings', true);
            MapView.CarsEnabled := fConfigHolder.ReadBoolean(false, fClientView.getUserName, 'ShowCars', true);
            MapView.PlanesEnabled := fConfigHolder.ReadBoolean(false, fClientView.getUserName, 'ShowPlanes', true);
            MapView.TranspOverlays := fConfigHolder.ReadBoolean(false, fClientView.getUserName, 'TransparentOverlays', true);
            MapView.SoundsVolume := StrToFloat(fConfigHolder.ReadString(false, fClientView.getUserName, 'SoundFxVolume', '1'));
          end;
      end;
  ...
  procedure TMapIsoHandler.setMasterURLHandler( URLHandler : IMasterURLHandler );
    var
      CachePath : string;
    begin
        ...
        then
          begin
            if not fControl.MapOptionsPanel.Visible
              then fControl.MapBtn.Click;
            //fControl.Tabs.CurrentTab := 1;
            Fork( threadedDefineZone, priNormal, [jmin, imin, jmax, imax, round(value)] );
          end
  ...
  procedure TMapIsoHandler.syncBuildFacility( const parms : array of const );
    ...
    begin
      ...
      case ErrorCode of
        NOERROR : ;
        ERROR_ZoneMissmatch :
          ShowMsgBox( GetLiteral('Literal225'),
            GetLiteral('Literal226'),
            0, true, false );
        ERROR_AreaNotClear :
          ShowMsgBox( GetLiteral('Literal230'),
            GetLiteral('Literal231'),
            0, true, false );
        ERROR_TooManyFacilities :
          ShowMsgBox( GetLiteral('Literal232'),
            GetLiteral('Literal233'),
            0, true, false );
        else
          ShowMsgBox( GetLiteral('Literal234'),
            GetLiteral('Literal235'),
            0, true, false );
      end;
    end;
