...
type
  ...
  TSupplySheetViewer = class(TVisualControl)
...
implementation
  ...
  uses
    ...
    Threads, MathUtils, InputOptionsViewer, SheetUtils, Literals, ClientMLS;
...
{$R *.DFM}
  ...
  procedure TSupplyHandler.Clear;
    begin
      ...
      fControl.xfer_FluidValue.Caption   := NA;
      fControl.xfer_LastCostPerc.Caption := NA;
      fControl.xfer_minK.Value           := 0;
      fControl.xfer_minK.Enabled         := false;
      fControl.lbMinQ.Caption            := NA;
      fControl.xfer_MaxPrice.Enabled     := false;
      fControl.xfer_MaxPrice.Value       := 0;
        ...
        if not VarIsEmpty(Proxy) and Proxy.SetPath(GateInfo.StrValue[tidFluidPath]) and (Update = fLastUpdate) and (Finger = fCurFinger)
          then
            try
              ...
              Lock;
              try
                SheetUtils.GetPropertyArray(Proxy, [tidFluidId, tidFluidValue, tidLastCost, tidKmin, tidPmax, tidCnxCount], GateInfo.Values);
                    ...
                    while (i < count) and (Update = fLastUpdate) do
                      begin
                        ...
                        if (fCurFinger = Finger) and (Update = fLastUpdate)
                          then Join(fControl.threadedRenderRow, [GateInfo, i, Update, Finger]);
                        inc(i);
                      end;
                    result := i = count;
                  end
  ...
  procedure TSupplyHandler.threadedRenderProperties(const parms : array of const);
    ...
    begin
      try
        ...
        if fLastUpdate = parms[1].vInteger
          then fControl.ftSupplies.CurrentFinger := 0;
      except
      end;
    end;
  ...
  procedure TSupplySheetViewer.lvConnectionsMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
    begin
      if Button = mbRight
