...
implementation
  ...
  uses
    HTMLHandler, ServerCnxHandler, URLParser, ChatHandler, ChatListHandler, CrimeHandler,
    MsgComposerHandler, Events, Protocol, MapIsoHandler, SoundHandler, MP3Handler, JukeBox,
    SHDocVw, VisualClassesHandler, ToolbarHandler, CircuitsHandler, Shutdown, ConfigHandler, Config,
    ...
    begin
      ...
      case EventId of
        ...
        evnAnswerPrivateCache :
          PathInfo := ExtractFilePath( paramstr(0) ) + 'Cache\';
  ...
  procedure TVoyagerWin.FormCreate(Sender: TObject);
    ...
    begin
      ...
      fFrameSet.RegisterMetaHandler( TMP3Handler.Create );
      ...
      fFrameSet.RegisterMetaHandler( TConfigHandler.Create );
      ...
      VoyagerTrains.RegisterMetaData;
    end;
  ...
  procedure TVoyagerWin.OnVoyagerRestore(Sender: TObject);
    begin
      //fFrameSet.HandleURL( '?frame_Id=SoundPlayer&frame_Action=Resume&MediaId=MainSoundTrack' );
    end;

  procedure TVoyagerWin.OnVoyagerMinimize(Sender: TObject);
    begin
      //fFrameSet.HandleURL( '?frame_Id=SoundPlayer&frame_Action=Pause&MediaId=MainSoundTrack' );
    end;
  ...
  procedure TVoyagerWin.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
    begin
      CanClose := true;
      try
        fKeyMapper.Free;
  ...
  procedure TVoyagerWin.OnLogonTimer(Sender: TObject);
    ...
    begin
      LogonTimer.Enabled := false;
      if not CoverForm.Visible
        then //fFrameSet.HandleURL( '?frame_Action=AutoLogon&frame_Id=CnxHandler' )
        else
          begin
            for i := 99 downto 50 do
              begin
                fFrameSet.HandleURL( '?frame_Id=MP3Handler&frame_Action=SetVolume&MediaId=MainSoundTrack&Volume=' + IntToStr(i) );
                Application.ProcessMessages;
              end;
            CoverForm.Close;
            {
            WinBtnsView.Top  := VoyagerWin.Height - WinBtnsView.Height;
