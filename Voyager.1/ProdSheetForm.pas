...
type
  ...
  TProdSheetViewer = class(TVisualControl)
...
implementation
  ...
  uses
    ...
    Threads, MathUtils, SheetUtils, Literals, ClientMLS;
...
{$R *.DFM}
  ...
  procedure TProductSheetHandler.Clear;
    begin
      inherited;
      fControl.PricePc.Value            := 0;
      fControl.Price.Caption            := GetLiteral('Literal68');
      fControl.btnHireSuppliers.Enabled := false;
      fControl.PricePc.Enabled          := false;
      Lock;
      try
        fControl.ftProducts.ClearFingers;
        ...
        if Update = fLastUpdate
          then
            begin
              ...
              if Update = fLastUpdate
                then Join(threadedRenderProperties, [fLastUpdate, Prop])
                else Prop.Free;
            end;
      except
        Prop.Free;
    ...
    begin
      try
        if Update = fLastUpdate
          then
            begin
              Prop := TStringList(parms[1].vPointer);
              try
                RenderProperties(Prop);
              finally
                Prop.Free;
              end;
              fControl.ftProducts.CurrentFinger := 0;
            end;
      except
      end;
    end;
