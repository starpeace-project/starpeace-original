...
implementation
  ...
  constructor TCar.Create(Id : integer; const IniMapPos : TPoint; BlocksToMove : integer; const Manager : ICarManager; CarClass : PCarClass);
    ...
    begin
      ...
      if roadblockid in NorthPointingBlocks
        then fRoadSide := rsEast
        else
          if roadblockid in SouthPointingBlocks
            then fRoadSide := rsWest
            else
              if roadblockid in EastPointingBlocks
  ...
  function TCar.GetBlockY(const View : IGameView) : integer;
    begin
      ...
      fRotation := View.Rotation;
      if fBlockRoutes[fRotation].Nodes <> nil
        then Result := fBlockRoutes[fRotation].Nodes[fCurNode].y*cZoomFactors[fZoom].m div cZoomFactors[fZoom].n - GetHeight(View)
        else Result := 0;
    end;
  ...
  function TCar.GetOldBlockY(const View : IGameView) : integer;
    begin
      ...
      fRotation := View.Rotation;
      if fOldBlockRoutes[fRotation].Nodes <> nil
        then Result := fOldBlockRoutes[fRotation].Nodes[fOldNode].y*cZoomFactors[fZoom].m div cZoomFactors[fZoom].n - GetHeight(View)
        else Result := 0;
    end;
  ...
  function TCarClassSoundTarget.GetSoundName : string;
    begin
      Result := fCarClass.SoundData.wavefile;
    end;
      ...
      while (i < fCarInstCount) and (carinstidx = carNone) do
        if fCarInstances[i] = nil
          then
            begin
              carinstidx := i;
              fCarInstances[carinstidx] := pointer(Sprite);
              Sprite._AddRef;
            end
          else inc(i);
      ...
      if (fCarInstances <> nil) and (ICar(Sprite).InstanceIdx < fCarInstCount)
        then
          begin
            ...
            fCarInstances[ICar(Sprite).InstanceIdx] := nil;
            dec(fAliveCars);
          end;
    end;
  ...
  function TCarManager.GetNextDir(const Car : ICar; out nextdir : TCarDir) : boolean;
    ...
    const
      cValidDir : array[TCarDir] of TRoadBlockIdSet = (NorthPointingBlocks, SouthPointingBlocks, EastPointingBlocks, WestPointingBlocks);
    const
