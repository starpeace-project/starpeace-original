                      ...
                      if ((fInstances[idx].fEfxs.efxs[k].x <> low(integer)) and (fInstances[idx].fEfxs.efxs[k].y <> low(integer))) or CalcEffectCoords(idx, k)
                        then
                          begin
                            ...
                            item.Overlays.objects[item.Overlays.Count].x := round((fInstances[idx].fEfxs.efxs[k].x - effectcls.XHook)*FocusObj.fZoomFactor);
                            item.Overlays.objects[item.Overlays.Count].y := round((fInstances[idx].fEfxs.efxs[k].y - effectcls.YHook)*FocusObj.fZoomFactor);
