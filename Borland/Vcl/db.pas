...
{ TFieldDef }
  ...
  published
    ...
    property Size: Word read GetSize write SetSize default 0;
  end;
...
begin
  if not (csLoading in DataSet.ComponentState) then OnUpdate(AItem);
end;
...
function TFieldDef.GetSize: Word;
begin
...
procedure TFieldDef.SetSize(Value: Word);
var
  FClass: TFieldClass;
begin
  FSize := Value;
...
begin
  if (Value < 1) or (Value > dsMaxStringSize) then
    DatabaseError(SInvalidFieldSize);
end;
...
begin
  ...
  if Result then
    case DataType of
      ftDate: Value := Data.Date - DateDelta;
      ftTime: Value := Data.Time / MSecsPerDay;
    else
      Value := (Data.DateTime / MSecsPerDay) - DateDelta;
    end;
end;
...
begin
  case DataType of
    ftDate: Data.Date := Integer(Trunc(Value)) + DateDelta;
    ftTime: Data.Time := Integer(Round(Frac(Value) * MSecsPerDay));
  else
    Data.DateTime := (Value + DateDelta) * MSecsPerDay;
  end;
  SetData(@Data);
end;
...
function TADTField.GetSize: Word;
...
procedure TArrayField.SetSize(Value: Word);
begin
  CheckInactive;
  FSize := Value;
end;
...
begin
  inherited Bind(Binding);
  if Assigned(FNestedDataSet) and not Binding then
...
function TReferenceField.GetDataSize: Word;
begin
  Result := FSize + 2;
end;
...
begin
  ...
  begin
    FActive := Value;
    try
      if not (csLoading in ComponentState) and (DataSet <> nil) 
        and DataSet.Active then 
...
begin
  ...
  case DataType of
    ...
    ftString, ftFixedChar, ftMemo: Result := Length(FData) + 1;
...
begin
  case DataType of
    ...
    ftBCD: DataSet.CurrToBCD(AsBCD, Buffer, 32, 4);
    ftCurrency,
...
begin
  case DataType of
    ...
    ftBCD:
        if DataSet.BCDToCurr(Buffer, Value) then
          AsBCD := Value else
          AsBCD := 0;
...
procedure TDataSet.GetProviderAttributes(List: TList);
begin
end;
...
const
  ...
  BaseFieldTypes: array[TFieldType] of TFieldType = (
    ...
    ftFloat, ftBCD, ftDate, ftTime, ftDateTime, ftBytes, ftVarBytes, ftInteger,
    ftBlob, ftBlob, ftBlob, ftBlob, ftBlob, ftBlob, ftBlob, ftUnknown,
    ftString, ftUnknown, ftLargeInt, ftADT, ftArray, ftReference, ftDataSet);

  CheckTypeSizes = [ftString, ftBytes, ftVarBytes, ftBCD, ftFixedChar, ftReference];
