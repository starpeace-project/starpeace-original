              ...
              while I < GlobalList.Count do
                ...
                begin
                  ...
                  begin
                    if Root <> nil then
                    begin
                      Reference := Root.FindComponent(FName);
                      if (Reference = nil) and (CompareText(FName, 'OWNER') = 0) then { Do not translate }
                        Reference := Root;
...
begin
  if (CompareText(Name, FPropName) = 0) and Assigned(ReadData) then
  begin
    ReadData(Self);
    FPropName := '';
  end;
    ...
    try
      for I := 0 to FFixups.Count - 1 do
        ...
        begin
          ...
          Reference := FRoot.FindComponent(CompName);
          if (Reference = nil) and (CompareText(CompName, 'OWNER') = 0) then    { Do not translate }
            Reference := FRoot;
          SetOrdProp(FInstance, FPropInfo, Longint(Reference));
        end;
    finally
...
var
  ...
  begin
    try
      Result := FindAncestorComponent(CompName, FindFieldClass(Root, CompClass));
...
begin
  case NextValue of
    ...
    vaString, vaLString: Writer.WriteStr(ReadStr);
...
var
  ...
  var
    ...
    Ident: string;
  begin
    // IntConstList protected by GlobalNameSpace, lock implied by TWriter
    with IntConstList.LockList do
    try
      for I := 0 to Count - 1 do
        with TIntConst(Items[I]) do
  ...
  var
    ...
    begin
      ...
      else Result := Component.Name + '.Owner';                                 { Do not translate }
    end;
...
begin
  ...
  begin
    ...
    if FVCLComObject = nil then
      raise EComponentError.CreateFmt(SNoComSupport, [ClassName]);
  end;
...
var
  Buffer: Pointer;
  Size, Read, Written: Longint;
begin
  try
    Size := Longint(cb);    //!! cb > MaxLongint = exception
    GetMem(Buffer, Size);   //!! cb could be > available memory
    try
      Read := FStream.Read(Buffer^, Size);
      if @cbRead <> nil then cbRead := Read;
      Result := stm.Write(Buffer, Size, @Written);
    finally
      FreeMem(Buffer, Size);
    end;
    if (Result = S_OK) and (@cbWritten <> nil) then cbWritten := Written;
  except
    Result := E_UNEXPECTED;
  end;
end;
