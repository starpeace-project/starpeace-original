...
{ TApplication }
  ...
  private
    ...
    function IsIdleMessage(const Msg: TMsg): Boolean;
...
begin
  ...
  begin
    ...
    if Ctl3DHandle = 0 then Ctl3DHandle := INVALID_HANDLE_VALUE
    else begin
      ...
      begin
        ...
        Ctl3DHandle := INVALID_HANDLE_VALUE;
      end;
    end;
  end;
end;
...
procedure DoneCtl3D;
begin
  if (Ctl3DHandle <> 0) and (Ctl3DHandle <> INVALID_HANDLE_VALUE) then
  begin
...
begin
  Result := [];
  if KeyboardState[VK_SHIFT] <> 0 then Include(Result, ssShift);
  if KeyboardState[VK_CONTROL] <> 0 then Include(Result, ssCtrl);
  if KeyboardState[VK_MENU] <> 0 then Include(Result, ssAlt);
  if KeyboardState[VK_LBUTTON] <> 0 then Include(Result, ssLeft);
  if KeyboardState[VK_RBUTTON] <> 0 then Include(Result, ssRight);
  if KeyboardState[VK_MBUTTON] <> 0 then Include(Result, ssMiddle);
end;
    ...
    if IsScrollBarVisible then
      ...
      begin
        if FControl.HandleAllocated then
          UninitializeFlatSB(FControl.Handle);
        Position := 0;
      end
      else
      begin
        if FControl.HandleAllocated then
          InitializeFlatSB(FControl.Handle);
        Position := Range;
      end;
end;
...
var
  ...
  begin
    if Control.Visible then
      case Control.Align of
        alLeft, alNone: NewRange := Max(NewRange, Position + Control.Left + Control.Width);
  ...
  begin
    if Control.Visible then
      case Control.Align of
        alTop, alNone: NewRange := Max(NewRange, Position + Control.Top + Control.Height);
...
//! Anchors are problematic here since AlignControls assumes children aren't
//! resized when aligned - anchored children are, causing the scroll range to
//! be incorrect.  Need to update scroll range prior to calling AlignControls. [rbr]
//! calcautorange;//!
...
var
  FocusHandle: HWND;
  Rgn1, Rgn2: HRGN;
  BorderX, BorderY, SaveIndex: Integer;
  MenuItem: TMenuItem;
  Canvas: TCanvas;
  DC: HDC;
            ...
            begin
              ...
              try
                ...
                try
                  ...
                  TMenuItemAccess(MenuItem).DrawItem(Canvas, rcItem,
...
begin
  ...
  if Control.ContainsControl(FActiveControl) and (Control <> FActiveControl) then
...
begin
  if (Menu <> nil) then
    Menu.ProcessMenuChar(Message) else
    inherited;
end;
...
begin
  ...
  while P <> nil do
  begin
    if ((P^.Index >= crSqlWait) and (P^.Index <= crDrag)) or (P^.Index > 0) then
...
begin
  if SystemParametersInfo(SPI_GETICONTITLELOGFONT, SizeOf(LogFont), @LogFont, 0) then
    FIconFont.Handle := CreateFontIndirect(LogFont) else
...
var
  ...
  begin
    ...
    begin
      ...
      if (Form.Align = AAlign) and not (csDesigning in Form.ComponentState) and
...
begin
  ...
  try
    ...
    if Done and IsIdleMessage(Msg) then DoActionIdle;
  except
