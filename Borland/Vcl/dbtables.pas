...
procedure CreateAttribute(List: TList; const AName: string; const AValue: Variant;
  InDelta: Boolean);
var
...
procedure FreeTimer;
begin
  if (TimerID <> 0) and (GetTickCount - StartTime > SQLDelay) then
  begin
    KillTimer(0, TimerID);
    TimerID := 0;
    StartTime := 0;
        ...
        try
          ...
          try
            ...
            begin
              ...
              // (TransIsolation <> tiDirtyRead) and not IsSQLBased. jmt.
...
begin
  ...
  try
    ...
    if High(Buffer) < Result then
      SetLength(Buffer, Trunc(Result * 1.25));
...
function TBDEDataSet.GetStatementLocale(StmtHandle: HDBIStmt): TLocale;
var
  DrvName: DBINAME;
  NumBytes: Word;
begin
...
procedure TBDEDataSet.FreeStatementLocale(var Value: TLocale);
begin
  if Value <> nil then OsLdUnloadObj(Value);
  Value := nil;
end;

procedure TBDEDataSet.GetParamData(Param: TParam; Buffer: Pointer;
  const DrvLocale: TLocale);
  ...
  begin
    ...
    begin
      ...
      AnsiToNativeBuf(DrvLocale, PChar(S), PChar(Result), Length(S));
    end else
      Result := Param.Value;
  end;
...
begin
  with Param do
    ...
    begin
      S := string(Value);
      if (Length(S) > 255) or (DataType = ftMemo) then
      begin
        BlobParamDesc(Buffer^).ulBlobLen := Length(S);
        BlobParamDesc(Buffer^).pBlobBuffer := PChar(GetNativeStr);
      end else
      begin
        if (DrvLocale <> nil) then
          AnsiToNativeBuf(DrvLocale, PChar(S), Buffer, Length(S) + 1) else
          GetData(Buffer);
      end;
    end
...
function TBDEDataSet.GetParamDataSize(Param: TParam): Integer;
begin
  with Param do
    if ((DataType in [ftString, ftFixedChar]) and (Length(Value) > 255)) or
...
begin
  ...
  begin
    ...
    begin
      ...
      if FBlockReadBuf = nil then
        FBlockReadBuf := AllocMem(DEFBLOCKSIZE);
      FBlockBufSize := Value;
      if FBlockBufSize > DEFBLOCKSIZE div FRecordSize then
        FBlockBufSize := DEFBLOCKSIZE div FRecordSize;
...
begin
  Result := False;
  if CaseInsensitive <> FCaseInsIndex then Exit;
...
begin
  if Event = deParentScroll then First;
...
begin
  ...
  if (DataSetField.DataType = ftDataSet) and (FParentDataSet.State = dsInsert) then
    FParentDataSet.Post;
  if (DataSetField.DataType = ftReference) and
     (TReferenceField(DataSetField).ReferenceTableName = '') then
...
procedure TDBDataSet.GetProviderAttributes(List: TList);
...
var
  ...
  QuoteChar: string;
begin
  inherited GetProviderAttributes(List);
  ...
  if Locale <> nil then
    CreateAttribute(List, 'LCID', Integer(TOSBLObj(Locale^).LdLCID), False);
  if not Database.IsSQLBased then
    CreateAttribute(List, 'SQL_BASED', False, True);
  QuoteChar := GetQuoteChar;
  if QuoteChar <> '' then
    CreateAttribute(List, 'QUOTE_CHAR', QuoteChar, True);
end;
...
begin
  ...
  begin
    if Assigned(CreateProviderProc) then
      ...
      DatabaseError(SNoProvider, Self);
  end;
  Result := FProvIntf;
end;
...
procedure TTable.GetProviderAttributes(List: TList);
begin
  inherited GetProviderAttributes(List);
  CreateAttribute(List, 'TABLE_NAME', TableName, True);
end;
...
begin
  ...
  Filer.DefineProperty('ParamData', ReadParamData, WriteParamData, Params.Count > 0);
end;
    ...
    for I := 0 to FParams.Count - 1 do
      ...
      begin
        ...
        begin
          ...
          else if IntPtr^ = IndTrunc then DatabaseErrorFmt(STruncationError, [Name], Self)
          else SetData(CurPtr);
        end;
...
var
  ...
  DrvLocale: TLocale;
begin
  SetLength(FieldDescs, FParams.Count);
  ...
  try
    ...
    begin
      ...
      if RequestLive and Constrained then
        SetBoolProp(StmtHandle, stmtCONSTRAINED, False);
...
procedure TQuery.GetProviderAttributes(List: TList);
var
  TableName, S: string;
  SReq: PSQLRequest;
  IsDDL: Boolean;
...
begin
  ...
  TableName := '';
  AttrReadOnly := False;
  if DbiGetSQLRequest(Database.Handle, PByte(S), IsDDL, SReq) = DBIERR_NONE then
  try
    if (not IsDDL) and (SReq.plRelation <> nil) and
       (SReq.plRelation.car <> nil) and (SReq.plRelation.cdr = nil) then
      with PSQLRelation(SReq.plRelation.car)^ do
        if szRelOwner[0] <> #0 then
          TableName := StrPas(szRelOwner) + '.' + StrPas(szRelName) else
          TableName := StrPas(szRelName);
    AttrReadOnly := (SReq.iDistinct <> 0) or (SReq.plGroupBy <> nil) or
      (not (SReq.erType in [REQSELECT, REQSELECTALL, REQSELECTDEC]));
  finally
    DbiFreeSQLRequest(SReq);
  end;
  if TableName <> '' then
    CreateAttribute(List, 'TABLE_NAME', TableName, True);
  if AttrReadOnly then
    CreateAttribute(List, 'READ ONLY', True, True);
end;
...
begin
  ...
  begin
    Prepare;
    ExecSQL;
    if RowsAffected <> 1 then DatabaseError(SUpdateFailed, FDataSet);
  end;
end;
