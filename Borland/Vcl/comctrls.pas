...
procedure TTabSheet.SetImageIndex(Value: Integer);
begin
    ...
    for I := 0 to Parent.ControlCount - 1 do // is control already associated
      if (Parent.Controls[I] is TCustomUpDown) and (Parent.Controls[I] <> Self) then
        if TCustomUpDown(Parent.Controls[I]).Associate = Value then
          raise Exception.CreateFmt(sUDAssociated,
...
procedure TListColumn.SetImageIndex(Value: Integer);
begin
  if FImageIndex <> Value then
  begin
    FImageIndex := Value;
...
begin
  with Owner.Owner do
    if not OwnerData and HandleAllocated then
      Result := Checkboxes and (ListView_GetCheckState(Handle, Index) <> 0)
    else
      Result := Checkboxes and FChecked;
end;
...
begin
  ...
  begin
    ...
    ListView_SetExtendedListViewStyle(LV.Handle, Styles or LVS_EX_CHECKBOXES);
...
procedure TCustomListView.CreateWnd;
  ...
  begin
    ...
    begin
      ...
      ListView_InsertColumn(Handle, Columns[I].Index, Column);
      UpdateColumn(I);
    end;
  end;
    ...
    case NMHdr^.code of
      NM_CUSTOMDRAW:
        ...
        begin
          ...
          else if dwDrawStage and CDDS_ITEMPREPAINT <> 0 then
          begin
            ...
            else if IsCustomDrawn(dtSubItem, cdPostPaint) then
                Result := Result or CDRF_NOTIFYPOSTPAINT;
          end;
        end;
      ...
      LVN_ENDLABELEDIT:
        with PLVDispInfo(NMHdr)^ do
          if (item.pszText <> nil) and (item.IItem <> -1) then
            Edit(item);
      LVN_COLUMNCLICK: ColClick(Column[PNMListView(NMHdr)^.iSubItem]);
...
begin
  ...
  begin
    if Target = dtSubItem then
      Result := Assigned(FOnCustomDrawSubItem)
...
begin
  Result := True;
  if Assigned(FOnCustomDrawItem) then FOnCustomDrawItem(Self, Item, State, Result);
end;
...
begin
  SetOpen(True);
  if not Open then raise Exception.Create(SCannotOpenAVI);
end;
...
begin
  if (FUpdateCount = 0) and not (csLoading in ComponentState) and
    (FToolBar <> nil) then
  begin
    Pos := Index;
        ...
        begin
          ...
          begin
            try
              ...
              PeekMessage(Msg, Handle, WM_LBUTTONDOWN, WM_LBUTTONDOWN, PM_REMOVE);
...
begin
  ...
  try
    ...
    try
      with Button.ClientToScreen(Point(0, Button.ClientHeight)) do
        FTempMenu.Popup(X, Y);
    finally
...
procedure TCoolBand.SetImageIndex(Value: Integer);
begin
  if FImageIndex <> Value then
  begin
    FImageIndex := Value;
...
begin
  ...
  begin
    FKind := Value;
    SetComCtlStyle(Self, DTS_TIMEFORMAT, Value = dtkTime);
  end;
end;
