...
type
  ...
  (*$HPPEMIT '{'}*)
  (*$HPPEMIT '  enum TCursor {crMin=0x7fff-1, crMax=0x7fff};'}*)
  (*$HPPEMIT '}'*)
...
begin
  ...
  begin
    I := (L + H) shr 1;
    C := PSiteInfoRec(List[I])^.TopParent - ParentWnd;
...
type
  ...
  TSiteInfo = record
    Sites: TSiteList;
    Found, First, Last: Integer;
  end;

function FindSiteWindow(Handle: Hwnd; Info: Pointer): Bool; stdcall;
var
  I: Integer;
begin
  with PSiteInfo(Info)^ do
  begin
    for I := First to Last do
      if Handle = PSiteInfoRec(Sites[I])^.Site.Handle then
      begin
        Found := I;
        Sites.Move(I, First);
{        Result := False;
...
function TSiteList.FindSite(Index: Integer): TWinControl;
var
  Top: HWnd;
  I, Last: Integer;
  SiteInfo: TSiteInfo;
begin
  { Make sure we're looking at the first of the children within the TopParent }
  Top := PSiteInfoRec(List[Index])^.TopParent;
  Last := -1;
  for I := 0 to Count - 1 do
    if Last < 0 then
    begin
      if PSiteInfoRec(List[I])^.TopParent = Top then
      begin
        Index := I;
        Last := Count - 1;
      end;
    end
    else if PSiteInfoRec(List[I])^.TopParent <> Top then
    begin
      Last := I - 1;
      Break;
    end;

  { Find child with highest z order }
  SiteInfo.Sites := Self;
  SiteInfo.First := Index;
  SiteInfo.Last := Last;
  SiteInfo.Found := -1;
  EnumChildWindows(Top, @FindSiteWindow, Longint(@SiteInfo));
{  if SiteInfo.Found >= 0 then
    Result := PSiteInfoRec(List[SiteInfo.Found])^.Site else}
    Result := PSiteInfoRec(List[Index])^.Site;

{  repeat
    Result := PSiteInfoRec(List[Index])^.Site;
    with PSiteInfoRec(List[Index])^ do
    begin
      if TopParent <> Top then Exit;
      Result := Site;
    end;
    Inc(Index);
  until Index >= Count - 1;}
  ...
  if Count = 0 then Exit
    ...
    begin
      if Find(CurrentWnd, Index) then
        Result := PSiteInfoRec(List[Index])^.Site//!FindSite(Index)
      else
...
begin
  ...
  begin
    ...
    else if Window = PCheckTargetInfo(Data)^.CurrentWnd then
    begin
      Result := False;
...
var
  ...
  begin
    ...
    begin
      ...
      Info.ClientWnd := 0;
    if Info.CurrentWnd = Info.ClientWnd then Exit;
    ...
    begin
      ...
      GetWindowRect(Info.TargetWnd, R2);
...
begin
  ...
  begin
    ...
    if (Site <> Client) and Site.Showing and Site.Enabled and
      ...
      ((Client.HostDockSite <> Site) or (Site.DockClientCount > 1)) then
    begin
      CanDock := True;
...
begin
  ...
  DragSave := nil;
  Accepted := False;
  if (DragObject <> nil) and DragObject.Cancelling then Exit;  // recursion control
  try
    if DragObject = nil then Exit;
...
begin
  if Control = nil then Exit;
  if SysLocale.MiddleEast and Control.UseRightToLeftReading then
...
begin
  ...
  begin
    CalcDockSizes;
    Result := Floating or (HostDockSite <> nil) and
      HostDockSite.DoUndock(NewDockSite, Self);
    if Result then
    begin
  ...
  begin
    ...
    begin                                 
      if AAlign = alNone then
        with Control do
        begin
          NewWidth := Parent.FLastResize.X + FLastWidth;
          NewHeight := Parent.FLastResize.Y + FLastHeight;
          R := BoundsRect;
        ...
        alTop:
          begin
            if akBottom in Control.Anchors then
              Inc(NewHeight, FLastResize.Y);
            Inc(Top, NewHeight);
          end;
        alBottom:
          begin
            if akTop in Control.Anchors then
              { Compensate for range being greater than client }
              Inc(NewHeight, FLastResize.Y + Bottom - ClientHeight);
        ...
        alRight:
          begin
            if akLeft in Control.Anchors then
              { Compensate for range being greater than client }
              Inc(NewWidth, FLastResize.X + Right - ClientWidth);
            Dec(Right, NewWidth);
            NewLeft := Right;
          end;
      end;
...
begin
  ...
  else if AlignWork then
//  if (not (FDockSite and FUseDockManager)) and AlignWork then
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if Msg = WM_SETTEXT then SendDockNotification(Msg, WParam, LParam);
    end;
  end
  else
...
begin
  if (FDockManager = nil) and DockSite and UseDockManager then
    Result := TDockTree.Create(Self) else
    Result := FDockManager;
...
begin
  if csDesigning in ComponentState then
    ...
    Result := GetDC(Handle);
  if Result = 0 then raise EOutOfResources.Create(SWindowDCError);
...
procedure TWinControl.UpdateLastResize(NewWidth, NewHeight: Integer);
begin
  inherited UpdateLastResize(NewWidth, NewHeight);
  if ([csLoading, csReading] * ComponentState = []) and HandleAllocated then
  begin
    if NewWidth - FWidth <> 0 then
      Inc(FLastResize.X, NewWidth - FWidth)
    else if (FLastClientWidth > 0) and (FLastClientWidth <> ClientWidth) then
      Inc(FLastResize.X, ClientWidth - (FLastClientWidth + FLastResize.X));

    if NewHeight - FHeight <> 0 then
      Inc(FLastResize.Y, NewHeight - FHeight)
    else if (FLastClientHeight > 0) and (FLastClientHeight <> ClientHeight) then
      Inc(FLastResize.Y, ClientHeight - (FLastClientHeight + FLastResize.Y));

    FLastClientWidth := ClientWidth;
    FLastClientHeight := ClientHeight;
  end;
end;
...
begin
  ...
  UpdateLastResize(Rect.Right - Rect.Left, Rect.Bottom - Rect.Top);
  FLeft := Rect.Left;
  FTop := Rect.Top;
      ...
      with Control do
        ...
        begin
          ...
          case WidthScale of
            ...
            sStretch:
              begin
                Inc(TotalMinWidth, ControlMinWidth);
                if ControlMaxWidth > 0 then
                  Inc(TotalMaxWidth, ControlMaxWidth)
                else
          ...
          case HeightScale of
            ...
            sStretch:
              begin
                Inc(TotalMinHeight, ControlMinHeight);
                if ControlMaxHeight > 0 then
                  Inc(TotalMaxHeight, ControlMaxHeight)
                else
...
begin
  ...
  case TDockOrientation(Orient) of
    doVertical: Result := R.Top;
    doHorizontal: Result := R.Left;
  else
    Result := 0;
  end;
end;
...
begin
  ...
  begin
    if FChildControl.Name = '' then
      raise Exception.Create(SDockedCtlNeedsName);
    Result := FChildControl.Name;
  end;
end;

procedure TDockZone.SetControlName(const Value: string);
var
  Client: TControl;
begin
  Client := nil;
...
begin
  ResultZone := nil;
  HTFlag := HTNOWHERE;
  CtlAtPos := FDockSite.ControlAtPos(MousePos, False);
  ...
  try
    ...
    begin
      ...
      begin
        ...
        ReadControlName(CompName);
        if CompName <> '' then Zone.SetControlName(CompName);
      end;
...
var
  ...
  procedure DrawGrabber(OnLeft: Boolean);
  begin
    with Canvas, R do
    begin
  ...
  procedure DrawCloseBtn(OnLeft: Boolean);
  begin
    if OnLeft then
    begin
      Dec(R.Left, 5);
...
begin
  ...
  begin
    R := Rect(ARect.Left + 2, ARect.Top + Offset, ARect.Right + 1,
      ARect.Top + Offset + 2);
    Dec(R.Right, 15);
    DrawGrabber(False);
    OffsetRect(R, 0, 3);
  ...
  begin
    ...
    with DockRect do
      case FDockSite.Align of
        alLeft: Right := Client.LRDockWidth;
        alRight:
          begin
            Left := FDockSite.ClientWidth - Client.LRDockWidth;
            Right := Left + Client.LRDockWidth;
          end;
        alTop: Bottom := Client.TBDockHeight;
        alBottom:
          begin
            Top := FDockSite.ClientHeight - Client.TBDockHeight;
...
begin
  ...
  if FSizingZone.FParentZone.FOrientation = doHorizontal then
    FSizingZone.FZoneLimit := FSizePos.y
  else
    FSizingZone.FZoneLimit := FSizePos.x;
  ...
  begin
    ...
    begin
      ...
      if CRect.Top >= ((DRect.Top + DRect.Bottom) div 2) then
        Result := alBottom;
      if (CRect.Top <= DRect.Top) and (CRect.Right >= DRect.Right) then
        Result := alTop
      else if (CRect.Left <= DRect.Left) and (CRect.Bottom >= DRect.Bottom) then
        Result := alLeft;
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      DropCtlZone := InternalHitTest(Point(Control.Top, Control.Left), HitTest);
...
procedure TDockTree.WindowProc(var Message: TMessage);
  ...
  begin
    if (FSizingZone.FParentZone.FOrientation = doHorizontal) then
    begin
      TestLimit := FSizingZone.Left + FBorderWidth;
      if FSizePos.y <= TestLimit then FSizePos.y := TestLimit;
      TestLimit := GetNextLimit(FSizingZone) - FBorderWidth;
      if FSizePos.y >= TestLimit then FSizePos.y := TestLimit;
    end
    else begin
      TestLimit := FSizingZone.Top + FBorderWidth;
      ...
      begin
        ...
        with TWMSetCursor(Message) do
          if (Smallint(HitTest) = HTCLIENT) and (CursorWnd = FDockSite.Handle)
            and (FDockSite.DockClientCount > 0) then
          begin
            TempZone := InternalHitTest(P, HitTestValue);
